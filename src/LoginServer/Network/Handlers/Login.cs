...
namespace Aura.Login.Network.Handlers
{
    public partial class LoginServerHandlers : PacketHandlerManager<LoginClient>
    {
        ...
        public void Login(LoginClient client, Packet packet)
        {
            ...
            {
                Send.LoginR_Msg(client,
                    "Unfortunately Aura doesn't support your client, please use the latest, updated NA client.\nIf you're the admin, you can disable this check in 'login.conf'.");
                return;
            }
            ...
            switch (loginType)
            {
                ...
                case LoginType.CmdLogin:
                    ...
                    // Create new account
                    if (LoginServer.Instance.Conf.Login.NewAccounts &&
                        (accountId.StartsWith("new//") || accountId.StartsWith("new__")))
                    {
                ...
                default:
                    Send.LoginR_Msg(client,
                        Localization.Get("Unsupported password encryption, please use the latest NA client."));
                    return;
            }
            ...
            if (account.BannedExpiration > DateTime.Now)
            {
                Send.LoginR_Msg(client, Localization.Get("You've been banned till {0}.\nReason: {1}"),
                    account.BannedExpiration, account.BannedReason);
                return;
            }
