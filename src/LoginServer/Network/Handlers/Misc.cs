...
namespace Aura.Login.Network.Handlers
{
    public partial class LoginServerHandlers : PacketHandlerManager<LoginClient>
    {
        ...
        public void TradeCard(LoginClient client, Packet packet)
        {
            ...
            if (character == null)
            {
                Log.Warning("TradeCard: User '{0}' tried to trade a card with an invalid character.",
                    client.Account.Name);
                goto L_Fail;
            }
            ...
            if (cardData == null)
            {
                Log.Warning("TradeCard: User '{0}' tried to trade a card that's not in the database.",
                    client.Account.Name);
                goto L_Fail;
            }
            ...
            // Add goods
            LoginServer.Instance.Database.TradeCard(client.Account, character, cardData);
