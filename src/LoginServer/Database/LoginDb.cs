...
namespace Aura.Login.Database
{
    public class LoginDb : AuraDb
    {
        ...
        public void UpdateAccountSecondaryPassword(Account account)
        {
            ...
            using (var mc =
                new MySqlCommand(
                    "UPDATE `accounts` SET `secondaryPassword` = @secondaryPassword WHERE `accountId` = @accountId",
                    conn))
            {
        ...
        public void UpdateAccount(Account account)
        {
            using (var conn = Connection)
            using (var cmd =
                new UpdateCommand("UPDATE `accounts` SET `lastLogin` = @lastLogin WHERE `accountId` = @accountId",
                    conn))
            {
        ...
        public List<Card> GetCharacterCards(string accountId)
        {
            ...
            using (var mc =
                new MySqlCommand(
                    "SELECT `cardId`, `type` FROM `cards` WHERE `accountId` = @accountId AND race = 0 AND !`isGift`",
                    conn))
            {
                mc.Parameters.AddWithValue("@accountId", accountId);
        ...
        public List<Card> GetPetCards(string accountId)
        {
            ...
            using (var mc =
                new MySqlCommand(
                    "SELECT `cardId`, `type`, `race` FROM `cards` WHERE `accountId` = @accountId AND race > 0 AND !`isGift`",
                    conn))
            {
                mc.Parameters.AddWithValue("@accountId", accountId);
        ...
        public List<Character> GetCharacters(string accountId)
        {
            using (var conn = Connection)
            {
                ...
                GetCharacters(accountId, "characters", CharacterType.Character, ref result, conn);
        ...
        public List<Character> GetPetsAndPartners(string accountId)
        {
            ...
            {
                var result = new List<Character>();
                GetCharacters(accountId, "pets", CharacterType.Pet, ref result, conn);
                GetCharacters(accountId, "partners", CharacterType.Partner, ref result, conn);
        ...
        private void GetCharacters(string accountId, string table, CharacterType type, ref List<Character> result,
        ...
        {
            return Create("characters", accountId, character, items);
        }
        ...
        {
            return Create("pets", accountId, pet, null);
        }
        ...
        {
            return Create("partners", accountId, partner, items);
        }
        ...
        private bool Create(string table, string accountId, Character character, List<Item> items)
        {
            ...
            using (var transaction = conn.BeginTransaction())
            {
                ...
                {
                    // Creature
                    character.CreatureId = CreateCreature(character, conn, transaction);
                    ...
                    // Items
                    if (items != null)
                        AddItems(character.CreatureId, items, conn, transaction);
        ...
        public void UpdateDeletionTime(Character character)
        {
            using (var conn = Connection)
            {
                if (character.DeletionFlag == DeletionFlag.Delete)
                    ...
                    using (var cmd = new UpdateCommand("UPDATE `creatures` SET {0} WHERE `creatureId` = @creatureId",
                        conn))
                    {
        ...
        public void TradeCard(Account account, Character targetCharacter, CharCardData charCard)
        {
            ...
            using (var conn = Connection)
            using (var cmd =
                new InsertCommand("UPDATE `accounts` SET `points` = `points` + @points WHERE `accountId` = @accountId",
                    conn))
            {
                cmd.Set("accountId", account.Name);
