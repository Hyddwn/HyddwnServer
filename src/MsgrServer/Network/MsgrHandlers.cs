...
using Group = Aura.Msgr.Database.Group;

namespace Aura.Msgr.Network
{
    public class MsgrServerHandlers : PacketHandlerManager<MsgrClient>
    {
        private readonly Regex _receiverRegex =
            new Regex(@"^[a-z0-9]+@[a-z0-9_]+$", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        public void Login(MsgrClient client, Packet packet)
        {
            ...
            client.User =
                MsgrServer.Instance.Database.GetOrCreateContact(accountId, entityId, entityName, server, channelName);
            if (client.User == null)
            {
        ...
        public void SendNote(MsgrClient client, Packet packet)
        {
            ...
            {
                Log.Warning("User '{0}' tried to send a message that's longer than 200 characters.",
            ...
            {
                Log.Warning("User '{0}' tried to send a message to invalid receiver '{1}'.", client.User.AccountId,
                    receiver);
                return;
            }
            ...
            if (prevStatus != status)
            {
                ...
                if (guild != null)
                {
                    ...
                    GuildManager.ForOnlineMembers(guild,
                        memberUser => Send.GuildMemberState(memberUser.Client, guild, member, user, status));
                }
            }
        ...
        public void FriendListRequest(MsgrClient client, Packet packet)
        {
            ...
            if (guild != null)
            {
                var member = guild.GetMember(user.CharacterId);
                GuildManager.ForOnlineMembers(guild,
                    memberUser => Send.GuildMemberState(memberUser.Client, guild, member, user, user.Status));
            }
        }
        ...
        public void ChangeGroup(MsgrClient client, Packet packet)
        {
            ...
            if (friend == null)
            {
                Log.Warning("ChangeGroup: User '{0}' tried to change group of non-existent friend.",
                    client.User.AccountId);
                client.Kill();
                return;
            }
            ...
            {
                Log.Warning("ChangeGroup: User '{0}' tried to change friend's group to an invalid one.",
                    client.User.AccountId);
                client.Kill();
                return;
            }
        ...
        public void FriendInvite(MsgrClient client, Packet packet)
        {
            ...
            if (friendUser != null)
            {
                ...
                {
                    Log.Error(
                        "FriendInvite: Failed to query user as friend for new friend's friends. Friends friends friends.");
                }
                else
                {
        ...
        public void ChatBegin(MsgrClient client, Packet packet)
        {
            ...
            if (list == 1)
            {
                Send.GuildChatMsg(client.User, Localization.Get("<SERVER>"),
                    Localization.Get("This feature hasn't been implemented yet."));
                return;
            }
            ...
            //if (list == 0)
            {
                ...
                {
                    Log.Warning("ChatBegin: User '{0}' tried to start chat without being friends.",
                        client.User.AccountId);
                    return;
                }
            }
        ...
        public void Chat(MsgrClient client, Packet packet)
        {
            ...
            {
                Log.Warning("Chat: User '{0}' sent message that was longer than allowed by the client.",
                    client.User.AccountId);
                return;
            }
        ...
        public void ChatInvite(MsgrClient client, Packet packet)
        {
            ...
            if (session == null)
            {
                Log.Warning("ChatInvite: User '{0}' tried to invite someone into invalid chat session.",
                    client.User.AccountId);
                return;
            }
        ...
        public void FriendBlock(MsgrClient client, Packet packet)
        {
            ...
            MsgrServer.Instance.Database.SetFriendshipStatusOneSided(client.User.Id, contactId,
        ...
        public void GuildMemberList(MsgrClient client, Packet packet)
        {
            ...
            if (guild == null)
            {
                Log.Warning("GuildMemberList: User '{0}' requested guild list without being in a guild.",
                    client.User.AccountId);
                return;
            }
