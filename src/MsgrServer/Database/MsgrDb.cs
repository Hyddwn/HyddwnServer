...
namespace Aura.Msgr.Database
{
    public class MsgrDb : AuraDb
    {
        ...
        /// <returns></returns>
        public User GetOrCreateContact(string accountId, long characterEntityId, string characterName, string server,
            string channelName)
        {
            using (var conn = Connection)
            {
                ...
                using (var mc =
                    new MySqlCommand("SELECT * FROM `contacts` WHERE `characterEntityId` = @characterEntityId", conn))
                {
                    ...
                    using (var reader = mc.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            if (!Enum.IsDefined(typeof(ContactStatus), user.Status) ||
                                user.Status == ContactStatus.None)
                                user.Status = ContactStatus.Online;
        ...
        {
            using (var conn = Connection)
            using (var mc = new MySqlCommand("SELECT * FROM `contacts` WHERE `characterEntityId` = @characterEntityId",
                conn))
            {
                mc.Parameters.AddWithValue("@characterEntityId", characterEntityId);
        ...
        public Friend GetFriendFromUser(string characterName, string server)
        {
            ...
            using (var mc =
                new MySqlCommand(
                    "SELECT * FROM `contacts` WHERE `characterName` = @characterName AND `server` = @server", conn))
            {
        ...
        {
            using (var conn = Connection)
            using (var mc = new MySqlCommand("DELETE FROM `notes` WHERE `receiver` = @receiver AND `noteId` = @noteId",
                conn))
            {
        ...
        public Note GetLatestUnreadNote(string receiver, long noteId)
        {
            ...
            using (var mc =
                new MySqlCommand(
                    "SELECT * FROM `notes` WHERE `receiver` = @receiver AND `noteId` > @noteId AND `read` = 0 ORDER BY `noteId` DESC LIMIT 1",
                    conn))
            {
                mc.Parameters.AddWithValue("@receiver", receiver);
                mc.Parameters.AddWithValue("@noteId", noteId);
        ...
        public void RenameGroup(User user, int groupId, string groupName)
        {
            using (var conn = Connection)
            using (var cmd =
                new UpdateCommand("UPDATE `groups` SET {0} WHERE `groupId` = @groupId AND `contactId` = @contactId",
                    conn))
            {
        ...
        public void ChangeGroup(User user, int friendContactId, int groupId)
        {
            using (var conn = Connection)
            using (var cmd =
                new UpdateCommand("UPDATE `friends` SET {0} WHERE `userId1` = @userId1 AND `userId2` = @userId2", conn))
            {
        ...
        public void DeleteGroup(User user, int groupId)
        {
            using (var conn = Connection)
            {
                // Move friends
                using (var cmd =
                    new UpdateCommand("UPDATE `friends` SET {0} WHERE `userId1` = @userId1 AND `groupId` = @oldGroupId",
                        conn))
                {
                    cmd.Set("groupId", -1);
                    cmd.AddParameter("@userId1", user.Id);
                ...
                // Delete group
                using (var mc =
                    new MySqlCommand("DELETE FROM `groups` WHERE `contactId` = @contactId AND `groupId` = @groupId",
                        conn))
                {
                    mc.Parameters.AddWithValue("@contactId", user.Id);
        ...
        public bool IsBlacklisted(int userId, int contactId)
        {
            ...
            using (var mc =
                new MySqlCommand(
                    "SELECT `friendId` FROM `friends` WHERE `userId1` = @userId1 AND `userId2` = @userId2 AND `status` = 7",
                    conn))
            {
        ...
        public void DeleteFriend(int contactId1, int contactId2)
        {
            ...
            using (var mc =
                new MySqlCommand(
                    "DELETE FROM `friends` WHERE (`userId1` = @userId1 AND `userId2` = @userId2) OR (`userId2` = @userId1 AND `userId1` = @userId2)",
                    conn))
            {
        ...
        public void SetFriendshipStatus(int contactId1, int contactId2, FriendshipStatus status)
        {
            ...
            using (var cmd =
                new UpdateCommand(
                    "UPDATE `friends` SET {0} WHERE (`userId1` = @userId1 AND `userId2` = @userId2) OR (`userId2` = @userId1 AND `userId1` = @userId2)",
                    conn))
            {
        ...
        public void SetFriendshipStatusOneSided(int contactId1, int contactId2, FriendshipStatus status)
        {
            using (var conn = Connection)
            using (var cmd =
                new UpdateCommand("UPDATE `friends` SET {0} WHERE (`userId1` = @userId1 AND `userId2` = @userId2)",
                    conn))
            {
                cmd.Set("status", (byte) status);
                cmd.AddParameter("@userId1", contactId1);
