...
namespace Aura.Data
{
            ...
            using (var reader = new JsonTextReader(fs))
            {
                try
                {
                    ...
                    foreach (var entry in array)
                    {
                        ...
                        {
                            Warnings.Add(new MandatoryValueException(path, ex.Key, obj));
                        }
                        catch (DatabaseWarningException ex)
                        {
                            Warnings.Add(new DatabaseWarningException(ex.Message + ", in " + Environment.NewLine + obj,
                                path));
                        }
                        catch (OverflowException)
                        {
                            Warnings.Add(new DatabaseWarningException(
                                "Number too big or too small for variable, in " + Environment.NewLine + obj, path));
                        }
                        catch (Exception ex)
                        {
                            throw new DatabaseErrorException(
                                "Exception: " + ex + Environment.NewLine + "Entry: " + Environment.NewLine + obj, path);
                        }
                    }
                }
    ...
    /// <typeparam name="TInfo">Data type</typeparam>
    public abstract class DatabaseJsonIndexed<TIndex, TInfo> : DatabaseJsonBase<Dictionary<TIndex, TInfo>, TInfo>
        where TInfo : class, new()
    {
        public override IEnumerator<TInfo> GetEnumerator()
        {
    ...
    public class MandatoryValueException : DatabaseWarningException
    {
        public MandatoryValueException(string path, string key, JObject obj)
            : base("Missing mandatory value '" + key + "', in " + Environment.NewLine + obj, path)
        {
            Key = key;
        }
    ...
    {
        internal static bool ReadBool(this JObject obj, string key, bool def = false)
        {
            return (bool) (obj[key] ?? def);
        }

        internal static byte ReadByte(this JObject obj, string key, byte def = 0)
        {
            return (byte) (obj[key] ?? def);
        }

        internal static sbyte ReadSByte(this JObject obj, string key, sbyte def = 0)
        {
            return (sbyte) (obj[key] ?? def);
        }

        internal static short ReadShort(this JObject obj, string key, short def = 0)
        {
            return (short) (obj[key] ?? def);
        }

        internal static ushort ReadUShort(this JObject obj, string key, ushort def = 0)
        {
            return (ushort) (obj[key] ?? def);
        }

        internal static int ReadInt(this JObject obj, string key, int def = 0)
        {
            return (int) (obj[key] ?? def);
        }

        internal static uint ReadUInt(this JObject obj, string key, uint def = 0)
        {
            return (uint) (obj[key] ?? def);
        }

        internal static float ReadFloat(this JObject obj, string key, float def = 0)
        {
            return (float) (obj[key] ?? def);
        }

        internal static double ReadDouble(this JObject obj, string key, double def = 0)
        {
            return (double) (obj[key] ?? def);
        }

        internal static string ReadString(this JObject obj, string key, string def = "")
        {
            return (string) (obj[key] ?? def);
        }
