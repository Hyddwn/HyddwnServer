...
namespace Aura.Data.Database
{
    public class ProductionData
    {
        ...
        {
            Materials = new List<ProductionMaterialData>();

            SuccessRates = new Dictionary<SkillRank, int>();
            for (var i = 0; i <= 15; ++i)
                SuccessRates.Add((SkillRank) i, 0);
        }
    ...
    public class ProductionDb : DatabaseJson<ProductionData>
    {
        ...
        {
            return Entries.Where(a => a.Category == category && a.Id == productId).ToArray();
        }

        protected override void ReadEntry(JObject entry)
        {
            entry.AssertNotMissing("category", "id", "title", "itemId", "amount", "exp", "rank", "materials",
                "successRates", "rainBonus");
            ...
            ratesObj.AssertNotMissing("Novice", "F", "E", "D", "C", "B", "A", "9", "8", "7", "6", "5", "4", "3", "2",
                "1");
            for (var rank = SkillRank.Novice; rank <= SkillRank.R1; ++rank)
