...
namespace Aura.Data.Database
{
    ...
    public class ItemDataProductionBonus
    {
        ...
        {
            return skillId == SkillId && itemId == ItemId;
        }
    }
    ...
    public class ItemDb : DatabaseJsonIndexed<int, ItemData>
    {
        ...
        {
            name = name.ToLower();
            return Entries.FirstOrDefault(a => a.Value.Name.ToLower() == name).Value;
        }
        ...
        {
            foreach (var entry in Entries.Values.Where(a => a.StackItemId != 0))
            {
                entry.StackItem = Find(entry.StackItemId);
