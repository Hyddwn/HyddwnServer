...
namespace Aura.Data.Database
{
    ...
    public class CharCardSetDb : DatabaseJsonIndexed<int, Dictionary<int, List<CharCardSetData>>>
    {
        ...
        {
            var set = Entries.GetValueOrDefault(setId);
        ...
        protected override void ReadEntry(JObject entry)
        {
            ...
            info.Pocket = entry.ReadInt("pocket");
            info.Color1 = entry.ReadUInt("color1");
            info.Color3 = entry.ReadUInt("color2");
            info.Color2 = entry.ReadUInt("color3");

            if (!Entries.ContainsKey(info.SetId))
                Entries[info.SetId] = new Dictionary<int, List<CharCardSetData>>();
            if (!Entries[info.SetId].ContainsKey(info.Race))
                Entries[info.SetId][info.Race] = new List<CharCardSetData>();

            Entries[info.SetId][info.Race].Add(info);
        }
    }
}