...
namespace Aura.Data.Database
{
    ...
    public class SkillData
    {
        ...
        public SkillRankData GetRankData(SkillRank rank, int raceId)
        {
            ...
            var skill = RankData.GetValueOrDefault(raceId);
            if (skill == null)
                if ((skill = RankData.GetValueOrDefault(0)) == null)
                    return null;

            // Cap at max rank
            SkillRankData rankData;
            if (rank > MaxRank)
                rankData = skill.GetValueOrDefault(MaxRank);
            else
    ...
    public class SkillDb : DatabaseJsonIndexed<int, SkillData>
    {
        ...
        protected override void ReadEntry(JObject entry)
        {
            ...
            // Locks
            skillInfo.PrepareLock = ReadLocks(entry, "prepare", "lock");
            skillInfo.PrepareUnlock = ReadLocks(entry, "prepare", "unlock");
            skillInfo.ReadyLock = ReadLocks(entry, "ready", "lock");
            skillInfo.ReadyUnlock = ReadLocks(entry, "ready", "unlock");
            skillInfo.UseLock = ReadLocks(entry, "use", "lock");
            skillInfo.UseUnlock = ReadLocks(entry, "use", "unlock");
            skillInfo.CompleteLock = ReadLocks(entry, "complete", "lock");
            skillInfo.CompleteUnlock = ReadLocks(entry, "complete", "unlock");
            ...
            {
                rank.AssertNotMissing("rank", "ap", "cp", "range", "stack", "stackMax", "loadTime", "newLoadTime",
                    "coolDown", "staminaCost", "staminaPrepare", "staminaWait", "staminaActive", "manaCost",
                    "manaPrepare", "manaWait", "manaActive", "life", "mana", "stamina", "str", "int", "dex", "will",
                    "luck", "var1", "var2", "var3", "var4", "var5", "var6", "var7", "var8", "var9", "training");
            ...
            Entries[skillInfo.Id] = skillInfo;
        }
        ...
        private Locks ReadLocks(JObject entry, string name, string category)
        {
            ...
            var split = cat.ReadString(category, "").Split(new[] {'|'}, StringSplitOptions.RemoveEmptyEntries);
        ...
        protected override void AfterLoad()
        {
            ...
            foreach (var skillList in Entries.Values.Select(a => a.RankData))
