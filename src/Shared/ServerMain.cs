...
namespace Aura.Shared
{
    ...
    public abstract class ServerMain
    {
        ...
        public void LoadData(DataLoad toLoad, bool reload)
        {
            ...
            try
            {
                if ((toLoad & DataLoad.Features) != 0)
                    LoadDb(AuraData.FeaturesDb, "db/features.txt", reload);
                ...
                if ((toLoad & DataLoad.StatsAge) != 0)
                    LoadDb(AuraData.StatsAgeUpDb, "db/stats_ageup.txt", reload);
                ...
                {
                    LoadDb(AuraData.WeatherTableDb, "db/weathertables.txt", reload);
                    LoadDb(AuraData.WeatherDb, "db/weather.txt", reload);
                }

                if ((toLoad & DataLoad.Keywords) != 0)
                    LoadDb(AuraData.KeywordDb, "db/keywords.txt", reload);

                if ((toLoad & DataLoad.Titles) != 0)
                    LoadDb(AuraData.TitleDb, "db/titles.txt", reload);

                if ((toLoad & DataLoad.ItemUpgrades) != 0)
                {
                    LoadDb(AuraData.ItemUpgradesDb, "db/itemupgrades.txt", reload);
                    LoadDb(AuraData.ArtisanUpgradesDb, "db/artisanupgrades.txt", reload);
                }

                if ((toLoad & DataLoad.Props) != 0)
                {
                    LoadDb(AuraData.PropsDb, "db/props.txt", reload);
                    LoadDb(AuraData.PropDefaultsDb, "db/prop_defaults.dat", reload);
                }

                if ((toLoad & DataLoad.Collecting) != 0)
                    LoadDb(AuraData.CollectingDb, "db/collecting.txt", reload);

                if ((toLoad & DataLoad.Fishing) != 0)
                {
                    LoadDb(AuraData.FishDb, "db/fish.txt", reload);
                    LoadDb(AuraData.FishingGroundsDb, "db/fishing_grounds.txt", reload);
                }

                if ((toLoad & DataLoad.Dungeons) != 0)
                {
                    LoadDb(AuraData.DungeonBlocksDb, "db/dungeon_blocks.txt", reload);
                    LoadDb(AuraData.DungeonDb, "db/dungeons.txt", reload);
                }

                if ((toLoad & DataLoad.Cutscenes) != 0)
                {
                    LoadDb(AuraData.CutscenesDb, "db/cutscenes.txt", reload);
                    LoadDb(AuraData.ActorDb, "db/actors.txt", reload);
                }

                if ((toLoad & DataLoad.Production) != 0)
                {
                    LoadDb(AuraData.ProductionDb, "db/production.txt", reload);
                    LoadDb(AuraData.ManualDb, "db/manuals.txt", reload);
                }

                if ((toLoad & DataLoad.Cooking) != 0)
                    LoadDb(AuraData.CookingDb, "db/cooking.txt", reload);

                if ((toLoad & DataLoad.Portals) != 0)
                    LoadDb(AuraData.PortalDb, "db/portals.txt", reload);

                if ((toLoad & DataLoad.CharacterStyles) != 0)
                    LoadDb(AuraData.CharacterStyleDb, "db/characterstyles.txt", reload);

                if ((toLoad & DataLoad.OptionSets) != 0)
                    LoadDb(AuraData.OptionSetDb, "db/optionsets.txt", reload);

                if ((toLoad & DataLoad.Banks) != 0)
                    LoadDb(AuraData.BankDb, "db/banks.txt", reload);

                if ((toLoad & DataLoad.ShopLicenses) != 0)
                    LoadDb(AuraData.ShopLicenseDb, "db/shop_licenses.txt", reload);
            }
        ...
        private void LoadDb(IDatabase db, string path, bool reload, bool log = true)
        {
            ...
            db.Load(new[] {systemPath, userPath}, cachePath, reload);
