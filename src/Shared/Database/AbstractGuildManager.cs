...
namespace Aura.Shared.Database
{
    ...
    {
        private readonly Dictionary<long, Guild> _guilds = new Dictionary<long, Guild>();
        ...
        public void SynchronizeGuilds()
        {
            lock (_syncLock)
            {
                var dbGuilds = Database.GetGuilds();
                ...
                foreach (var guild in _guilds.Values)
                {
                    ...
                    Guild dbGuild;
                    if (!dbGuilds.TryGetValue(guild.Id, out dbGuild) || dbGuild.Disbanded)
                    {
                    ...
                    foreach (var member in members)
                    {
                        ...
                        if (dbMember == null || member.Rank != GuildMemberRank.Applied &&
                            dbMember.Rank == GuildMemberRank.Declined)
                        {
                ...
                {
                    var guild = _guilds[member.GuildId];
                    OnSyncGuildMemberAccepted(guild, member);
                }
                ...
                {
                    var guild = _guilds[member.GuildId];
                    OnSyncGuildMemberDeclined(guild, member);
                }
                ...
                {
                    var guild = _guilds[member.GuildId];
                    OnSyncGuildMemberUpdated(guild, member);
                }
            }
        }
