...
namespace Aura.Shared.Database
{
    public class AuraDb
    {
        ...
        private readonly Regex _nameCheckRegex = new Regex(@"^[a-zA-Z][a-z0-9]{2,15}$", RegexOptions.Compiled);
        ...
        public void Init(string host, int port, string user, string pass, string db)
        {
            _connectionString =
                string.Format(
                    "server={0}; port={1}; database={2}; uid={3}; password={4}; pooling=true; min pool size=0; max pool size=100; ConvertZeroDateTime=true",
        ...
        public bool AccountIsLoggedIn(string accountId)
        {
            using (var conn = Connection)
            {
                var mc = new MySqlCommand(
                    "SELECT `accountId` FROM `accounts` WHERE `accountId` = @accountId AND `loggedIn` = true", conn);
                mc.Parameters.AddWithValue("@accountId", accountId);
        ...
        public void SetAccountLoggedIn(string accountId, bool loggedIn)
        {
            using (var conn = Connection)
            using (var mc =
                new MySqlCommand("UPDATE `accounts` SET `loggedIn` = @loggedIn WHERE `accountId` = @accountId", conn))
            {
                mc.Parameters.AddWithValue("@accountId", accountId);
        ...
        public Card AddCard(string accountId, int type, int race)
        {
            ...
            {
                var mc = new MySqlCommand(
                    "INSERT INTO `cards` (`accountId`, `type`, `race`) VALUES (@accountId, @type, @race)", conn);
                mc.Parameters.AddWithValue("@accountId", accountId);
        ...
        public NameCheckResult NameOkay(string name, string serverName)
        {
            ...
            {
                var mc = new MySqlCommand(
                    "SELECT `creatureId` FROM `creatures` WHERE `name` = @name AND `server` = @serverName", conn);
        ...
        public void SetAccountPassword(string accountName, string password)
        {
            using (var conn = Connection)
            using (var mc =
                new MySqlCommand("UPDATE `accounts` SET `password` = @password WHERE `accountId` = @accountId", conn))
            {
        ...
        public long CreateSession(string accountId)
        {
            using (var conn = Connection)
            using (var mc =
                new MySqlCommand("UPDATE `accounts` SET `sessionKey` = @sessionKey WHERE `accountId` = @accountId",
                    conn))
            {
        ...
        public bool CheckSession(string accountId, long sessionKey)
        {
            ...
            using (var mc =
                new MySqlCommand(
                    "SELECT `sessionKey` FROM `accounts` WHERE `accountId` = @accountId AND `sessionKey` = @sessionKey",
                    conn))
            {
                mc.Parameters.AddWithValue("@accountId", accountId);
                mc.Parameters.AddWithValue("@sessionKey", sessionKey);
        ...
        public void UninitializeCreature(long creatureId)
        {
            using (var conn = Connection)
            using (var mc =
                new MySqlCommand("UPDATE `creatures` SET `state` = `state` & ~1 WHERE `creatureId` = @creatureId",
                    conn))
            {
            ...
            using (var conn = Connection)
            {
                    ...
                    , conn))
                {
                    using (var reader = mc.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            var member = ReadGuildMember(reader);
        ...
        public Guild GetGuild(long guildId)
        {
            ...
            using (var mc = new MySqlCommand("SELECT * FROM `guilds` WHERE `guildId` = @id", conn))
            {
                ...
                using (var reader = mc.ExecuteReader())
                {
                    ...
                    {
                        var guild = ReadGuild(reader);
                        var members = GetGuildMembers(guild.Id);
        ...
        public void UpdateGuildLeader(Guild guild)
        {
            ...
            using (var transaction = conn.BeginTransaction())
            {
                ...
                foreach (var member in guild.GetMembers())
                    using (var cmd =
                        new UpdateCommand("UPDATE `guild_members` SET {0} WHERE `characterId` = @characterId", conn,
                            transaction))
                    {
        ...
        public void RemoveGuildMember(GuildMember member)
        {
            ...
            using (var cmd =
                new MySqlCommand(
                    "DELETE FROM `guild_members` WHERE `guildId` = @guildId && `characterId` = @characterId", conn))
            {
        ...
        public void UpdateGuildMemberRank(GuildMember member)
        {
            ...
            using (var cmd =
                new UpdateCommand(
                    "UPDATE `guild_members` SET {0} WHERE `guildId` = @guildId AND `characterId` = @characterId", conn))
            {
        ...
        /// <returns></returns>
        public List<GuildSearchResult> GetGuildList(string serverName, GuildSortBy sortBy, GuildSortType sortType,
            GuildSearchLevel guildLevel, GuildSearchMembers memberAmount, GuildType guildType, string searchText)
        {
            ...
            var query = string.Format(
                ...
                " WHERE `g`.`server` = '{0}' AND `g`.`name` LIKE '%{1}%' AND `g`.`guildId` > {2}", serverName,
                searchText, MabiId.Guilds);
            ...
            if (guildLevel != GuildSearchLevel.All)
            {
                ...
                {
                    default:
                    case GuildSearchLevel.Beginner:
                        level = GuildLevel.Beginner;
                        break;
                    case GuildSearchLevel.Basic:
                        level = GuildLevel.Basic;
                        break;
                    case GuildSearchLevel.Advanced:
                        level = GuildLevel.Advanced;
                        break;
                    case GuildSearchLevel.Great:
                        level = GuildLevel.Great;
                        break;
                    case GuildSearchLevel.Grand:
                        level = GuildLevel.Grand;
                        break;
                }
            ...
            if (memberAmount != GuildSearchMembers.All)
            {
                ...
                {
                    default:
                    case GuildSearchMembers.Lv1_5:
                        min = 1;
                        max = 5;
                        break;
                    case GuildSearchMembers.Lv6_10:
                        min = 6;
                        max = 10;
                        break;
                    case GuildSearchMembers.Lv11_20:
                        min = 11;
                        max = 20;
                        break;
                    case GuildSearchMembers.Lv21_50:
                        min = 21;
                        max = 50;
                        break;
                    case GuildSearchMembers.Lv51_X:
                        min = 21;
                        max = 999999;
                        break;
                }
            ...
            {
                string by;
                var type = sortType.ToString().ToUpper();
                switch (sortBy)
                {
                    default:
                    case GuildSortBy.Level:
                        by = "`g`.`level`";
                        break;
                    case GuildSortBy.Members:
                        by = "`memberCount`";
                        break;
                    case GuildSortBy.Type:
                        by = "`g`.`type`";
                        break;
                    case GuildSortBy.Name:
                        by = "`g`.`name`";
                        break;
                }
                ...
                while (reader.Read())
                {
                    ...
                    switch ((GuildLevel) reader.GetInt32("level"))
                    {
                        ...
                        case GuildLevel.Great:
                            guild.LevelIndex = 50;
                            break;
                        case GuildLevel.Grand:
                            guild.LevelIndex = 250;
                            break;
                    }
