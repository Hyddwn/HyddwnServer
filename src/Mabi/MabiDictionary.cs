...
namespace Aura.Mabi
{
    ...
    public class MabiDictionary
    {
        ...
        public static MabiDictionary Empty => new MabiDictionary();
        ...
        public void SetUShort(string key, ushort val)
        {
            Set(key, (short) val);
        }
        ...
        public void SetLong(string key, DateTime val)
        {
            SetLong(key, val.Ticks / 10000);
        }
        ...
        public void SetString(string key, string format, params object[] args)
        {
            Set(key, string.Format(format, args));
        }
        ...
        public void SetBin(string key, byte[] val)
        {
            Set(key, val);
        }
        ...
        {
            return (ulong) Get<long>(key);
        }

        public DateTime GetDateTime(string key)
        {
            return new DateTime(Get<long>(key) * 10000);
        }
        ...
        private string ValToTypeStr(object val)
        {
            ...
            if (val is ushort || val is short) return "2";
            if (val is uint || val is int) return "4";
            if (val is ulong || val is long) return "8";
            if (val is float) return "f";
            if (val is string) return "s";
            if (val is bool) return "b";
        ...
        public override string ToString()
        {
            ...
            {
                var sType = ValToTypeStr(tag.Value);

                if (sType == "b")
                    sb.AppendFormat("{0}:{1}:{2};", tag.Key, sType, (bool) tag.Value ? "1" : "0");
                else if (sType == "s")
                    sb.AppendFormat("{0}:{1}:{2};", tag.Key, sType,
                        ((string) tag.Value).Replace(";", "%S").Replace(":", "%C"));
                else if (sType == "f")
                    sb.AppendFormat("{0}:{1}:{2};", tag.Key, sType,
                        ((float) tag.Value).ToString(CultureInfo.InvariantCulture));
                else if (sType == "B")
        ...
        public void Parse(string str)
        {
            ...
            foreach (Match match in Regex.Matches(str, "([^:]+):([^:]+):([^;]*);"))
            {
                ...
                {
                    case "1":
                        Set(key, Convert.ToByte(val));
                        break;
                    case "2":
                        Set(key, Convert.ToInt16(val));
                        break;
                    case "4":
                        Set(key, Convert.ToInt32(val));
                        break;
                    case "8":
                        Set(key, Convert.ToInt64(val));
                        break;
                    case "f":
                        Set(key, Convert.ToSingle(val, CultureInfo.InvariantCulture));
                        break;
                    case "s":
                        Set(key, val.Replace("%S", ";").Replace("%C", ":"));
                        break;
                    case "b":
                        Set(key, val == "1");
                        break;
                    case "B":
                        Set(key, Convert.FromBase64String(val));
                        break;
                }
            }
        }
