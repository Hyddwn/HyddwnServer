    ...
    public static class MabiZip
    {
        ...
        public static string Compress(string str)
        {
            ...
            using (var mout = new MemoryStream())
            {
                ...
                // Flags
                var flg = 2 << 6; // Compression level
                var n = (cmf * 256 + flg) % 31;
                if (n != 0)
                    flg += 31 - n; // Check bits

                // <length>;<cmf><flg><data><checksum>
                return string.Format("{0};{1:x02}{2:x02}{3}{4:x08}", barr.Length, cmf, flg,
                    BitConverter.ToString(mout.ToArray()).Replace("-", "").ToLower(), ComputeAdler32(barr));
            }
        }
