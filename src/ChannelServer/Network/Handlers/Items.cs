...
namespace Aura.Channel.Network.Handlers
{
    public partial class ChannelServerHandlers : PacketHandlerManager<ChannelClient>
    {
        ...
        public void ItemMove(ChannelClient client, Packet packet)
        {
            ...
            var untrustedSource =
                (Pocket) packet
                    .GetInt(); // Discard this, NA does too // [200200, NA233 (2016-08-12)] Changed from byte to int
            ...
            if (target == creature.Inventory.LeftHandPocket && !item.IsShieldLike && creature.RightHand != null &&
                !AuraData.FeaturesDb.IsEnabled("TwinSword"))
            {
                ...
                Send.Notice(creature, NoticeType.MiddleSystem,
                    Localization.Get("The Dual Wielding feature hasn't been enabled yet."));
                goto L_Fail;
            }

            // Check extra slots' availability
            if (target.IsExtraSlot() && !creature.CanUseExtraEquipmentPocket(target) ||
                source.IsExtraSlot() && !creature.CanUseExtraEquipmentPocket(source))
            {
            ...
            {
                Log.Warning(
                    "ItemMove: User '{0}' tried to move something to or from the trade window without being in a trade.",
                    client.Account.Id);
                goto L_Fail;
            }

            // Stop moving when changing weapons
            if (target >= Pocket.RightHand1 && target <= Pocket.Magazine2 ||
                source >= Pocket.RightHand1 && source <= Pocket.Magazine2)
                creature.StopMove();
        ...
        public void ItemDrop(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning("ItemDrop: Player '{0}' ({1:X16}) tried to drop from inaccessible pocket.", creature.Name,
                    creature.EntityId);
                Send.ItemDropR(creature, false, 0);
                return;
            }
            ...
            if (item.IsBag && item.OptionInfo.LinkedPocketId != Pocket.None &&
                creature.Inventory.CountItemsInPocket(item.OptionInfo.LinkedPocketId) > 0)
            {
                Log.Warning("ItemDrop: Player '{0}' ({1:X16}) tried to drop filled item bag.", creature.Name,
                    creature.EntityId);
                Send.ItemDropR(creature, false, 0);
                return;
            }
            ...
            // Check droppability
            if (item.HasTag("/not_dropable/") || item.Data.Action == ItemAction.ImportantItem ||
                item.Data.Action == ItemAction.Important2Item)
            {
        ...
        public void ItemPickUp(ChannelClient client, Packet packet)
        {
            ...
            if (!creature.CanPickUp(item))
                if (creature.IsDev)
                {
                    Send.Notice(creature,
                        Localization.Get(
                            "You stole an innocent player's loot, feeling like a big, strong devCAT now? Shame on you."));
                }
                else
                {
            ...
            if (item.HasTag("/key/"))
                Send.Effect(creature, Effect.PickUpItem, (byte) 1, item.Info.Id, item.Info.Color1, item.Info.Color2,
                    item.Info.Color3);
        ...
        public void ItemStateChange(ChannelClient client, Packet packet)
        {
            ...
            var unkPocket =
                packet.GetInt(); // [200200, NA233 (2016-08-12)] Changed from byte to int (apparently it's a pocket as well?)
            ...
            foreach (var target in new[] {firstTarget, secondTarget})
            {
                ...
                if (target != Pocket.Head && target != Pocket.Robe && target != Pocket.Armor &&
                    target != Pocket.HeadStyle && target != Pocket.RobeStyle && target != Pocket.ArmorStyle)
                {
                    Log.Warning(
                        "ItemStateChange: Creature '{0:X16}' tried to change state of invalid pocket's item ({1}).",
                        creature.EntityId, target);
                    continue;
                }
        ...
        public void BeginnerWarpBook(ChannelClient client, Packet packet)
        {
            ...
            if (creature.TotalLevel >= 1000)
            {
                // Unofficial
                Send.Notice(creature,
                    Localization.Get("You can only use this item with a cumulative level of less than 1000."));
                return;
            }
        ...
        public void CollectionAddItem(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning(
                    "CollectionAddItem: User '{0}' tried to add an item to an item that isn't a collection book ({1}).",
                    client.Account.Id, book.Info.Id);
            ...
            {
                Log.Warning("CollectionAddItem: Invalid index '{0}' for '{1}' in '{2}'.", itemIndex, item.Info.Id,
                    book.Info.Id);
        ...
        public void CollectionGetReward(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning(
                    "CollectionGetReward: User '{0}' tried to claim a reward for an item that isn't a collection book ({1}).",
                    client.Account.Id, book.Info.Id);
        ...
        public void SwitchExtraEquipment(ChannelClient client, Packet packet)
        {
            ...
            if (currentSet == EquipmentSet.Original && newSet != EquipmentSet.Original ||
                currentSet != EquipmentSet.Original && newSet == EquipmentSet.Original)
            {
