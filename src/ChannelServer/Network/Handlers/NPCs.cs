...
namespace Aura.Channel.Network.Handlers
{
    public partial class ChannelServerHandlers : PacketHandlerManager<ChannelClient>
    {
        ...
        public void NpcTalkStart(ChannelClient client, Packet packet)
        {
            ...
            if (disallow)
                throw new ModerateViolation("Tried to talk to NPC 0x{0:X} ({1}) without permission.", npcEntityId,
                    target.Name);
            ...
            if (target.ScriptType == null)
            {
                ...
                Log.Warning("NpcTalkStart: Creature '{0}' tried to talk to NPC '{1}', that doesn't have a script.",
                    creature.Name, target.Name);
                return;
            }

            // Check distance
            if (!bypassDistanceCheck && (creature.RegionId != target.RegionId ||
                                         target.GetPosition().GetDistance(creature.GetPosition()) > 1000))
            {
                Send.MsgBox(creature, Localization.Get("You're too far away."));
                Send.NpcTalkStartR_Fail(creature);

                Log.Warning("NpcTalkStart: Creature '{0}' tried to talk to NPC '{1}' out of range.", creature.Name,
                    target.Name);
                return;
            }
        ...
        ///     001 [................] String :
        ///     <result session='1837'>
        ///         <this type="character">4503599627370498</this><return type="string">@end</return>
        ///     </result>
        ...
        public void NpcTalkSelect(ChannelClient client, Packet packet)
        {
            ...
            if (client.NpcSession.Script.ConversationState != ConversationState.Select)
                Log.Debug("Received Select without being in Select mode ({0}).",
                    client.NpcSession.Script.GetType().Name);
        ...
        public void NpcTalkKeyword(ChannelClient client, Packet packet)
        {
            ...
            if (!creature.Keywords.Has(keyword))
            {
                ...
                Log.Warning("NpcTalkKeyword: Player '{0}' tried using keyword '{1}', without knowing it.",
                    creature.Name, keyword);
                return;
            }
        ...
        public void NpcShopSellItem(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning("NpcShopSellItem: Failed to remove item '{0:X16}' from '{1}'s inventory.", entityId,
                    creature.Name);
                goto L_End;
            }
        ...
        public void CheckDirectBankSelling(ChannelClient client, Packet packet)
        {
            ...
            // Check space
            var goldMax = Math.Min(int.MaxValue,
                client.Account.Characters.Count * (long) ChannelServer.Instance.Conf.World.BankGoldPerCharacter);
        ...
        public void RequestBankTabs(ChannelClient client, Packet packet)
        {
            ...
            Send.OpenBank(creature, client.Account.Bank, race, creature.Temp.CurrentBankId,
                creature.Temp.CurrentBankTitle);
        }
        ...
        public void BankDepositGold(ChannelClient client, Packet packet)
        {
            ...
            if (creature.Inventory.Gold < amount)
                throw new ModerateViolation("BankDepositGold: '{0}' ({1:X16}) tried to deposit more than he has.",
                    creature.Name, creature.EntityId);
            ...
            {
                Send.MsgBox(creature,
                    Localization.Get("The maximum amount of gold you may store in the bank is {0:n0}."), goldMax);
        ...
        public void BankDepositItem(ChannelClient client, Packet packet)
        {
            ...
            tabName = string.Format("{0}@{1}", tabName,
                creature.Client.Account.Characters.FirstOrDefault(c => c.CreatureId == packet.Id).Server);

            // Check premium
            if (!client.Account.PremiumServices.CanUseAllBankTabs && tabName != string.Format("{0}@{1}", creature.Name,
                    creature.Client.Account.Characters.FirstOrDefault(c => c.CreatureId == packet.Id).Server))
            {
                // Unofficial
                Send.MsgBox(creature,
                    Localization.Get("Inventory Plus is required to access other character's bank tabs."));
            ...
            // Deposit item
            var success = client.Account.Bank.DepositItem(creature, itemEntityId, creature.Temp.CurrentBankId, tabName,
                posX, posY);
        ...
        public void BankPostLicenseInquiryDeposit(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning(
                    "BankPostLicenseInquiryDeposit: User '{0}' tried to post-license-inquiry-deposit invalid item.",
                    client.Account.Id);
            ...
            // Deposit item
            var success = client.Account.Bank.DepositItem(creature, itemEntityId, creature.Temp.CurrentBankId,
                creature.Name, 0, 0);
        ...
        public void NpcTalkEgo(ChannelClient client, Packet packet)
        {
            ...
            if (items.Count(item => item.EgoInfo.Race == egoRace) > 1)
            {
                Send.ServerMessage(creature,
                    Localization.Get("Multiple egos of the same type are currently not supported."));
                Send.NpcTalkEgoR(creature, false, 0, null, null);
                return;
            }
            ...
            {
                case EgoRace.SwordM:
                    npcName = "ego_male_sword";
                    break;
                case EgoRace.SwordF:
                    npcName = "ego_female_sword";
                    break;
                case EgoRace.BluntM:
                    npcName = "ego_male_blunt";
                    break;
                case EgoRace.BluntF:
                    npcName = "ego_female_blunt";
                    break;
                case EgoRace.WandM:
                    npcName = "ego_male_wand";
                    break;
                case EgoRace.WandF:
                    npcName = "ego_female_wand";
                    break;
                case EgoRace.BowM:
                    npcName = "ego_male_bow";
                    break;
                case EgoRace.BowF:
                    npcName = "ego_female_bow";
                    break;
                case EgoRace.CylinderM:
                    npcName = "ego_male_cylinder";
                    break;
                case EgoRace.CylinderF:
                    npcName = "ego_female_cylinder";
                    break;
            }
