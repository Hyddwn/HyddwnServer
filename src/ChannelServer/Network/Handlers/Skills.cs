...
namespace Aura.Channel.Network.Handlers
{
    public partial class ChannelServerHandlers : PacketHandlerManager<ChannelClient>
    {
        ...
        public void SkillAdvance(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning("SkillAdvance: Player '{0:X16}' tried to advance skill '{1}' to unknown rank '{2}'.",
                    creature.EntityId, skill.Info.Id, nextRank);
                goto L_Fail;
            }
        ...
        public void SkillStart(ChannelClient client, Packet packet)
        {
            ...
            try
            {
                ...
                if (skill.RankData.ManaPrepare != 0)
                    creature.Regens.Add(Stat.Mana, creature.GetAdjustedManaCost(skill.RankData.ManaPrepare),
                        creature.ManaMax, 1000);
                ...
                if (skill.RankData.ManaActive != 0)
                    creature.Regens.Add("SkillInUse" + skill.Info.Id, Stat.Mana,
                        creature.GetAdjustedManaCost(skill.RankData.ManaActive), creature.ManaMax);
                if (skill.RankData.StaminaActive != 0)
                    creature.Regens.Add("SkillInUse" + skill.Info.Id, Stat.Stamina, skill.RankData.StaminaActive,
                        creature.StaminaMax);
        ...
        public void SkillPrepare(ChannelClient client, Packet packet)
        {
            ...
            try
            {
                ...
                creature.Lock(Locks.ChanceStance | Locks.ChangeEquipment | Locks.UseItem | Locks.PrepareSkills |
                              Locks.Attack | Locks.TalkToNpc | Locks.Gesture | Locks.StartSkills);
                ...
                else
                {
                    ...
                    if (skill.RankData.ManaPrepare != 0)
                        ...
                        else
                        {
                            ...
                            var perSecond = (float) (cost / Math.Ceiling(castTime / 1000f) *
                                                     (skill.RankData.NewLoadTime / 1000f));
                    ...
                    if (skill.RankData.StaminaPrepare != 0)
                        ...
                        {
                            var perSecond = (float) (skill.RankData.StaminaPrepare / Math.Ceiling(castTime / 1000f) *
                                                     (skill.RankData.NewLoadTime / 1000f));
                            var seconds = (int) (Math.Ceiling(castTime / 1000f) * 1000);
        ...
        public void SkillReady(ChannelClient client, Packet packet)
        {
            ...
            try
            {
                ...
                if (skill.RankData.ManaWait != 0)
                    creature.Regens.Add("ActiveSkillWait", Stat.Mana,
                        creature.GetAdjustedManaCost(skill.RankData.ManaWait), creature.ManaMax);
                if (skill.RankData.StaminaWait != 0)
                    creature.Regens.Add("ActiveSkillWait", Stat.Stamina, skill.RankData.StaminaWait,
                        creature.StaminaMax);
        ...
        public void SkillUse(ChannelClient client, Packet packet)
        {
            ...
            {
                creature.Unlock(Locks.All);
                creature.Lock(Locks.ChanceStance | Locks.Walk | Locks.Run | Locks.ChangeEquipment | Locks.UseItem |
                              Locks.Attack | Locks.PickUpAndDrop | Locks.TalkToNpc | Locks.Gesture);
                ...
                if (skill.RankData.ManaActive != 0)
                    creature.Regens.Add(Stat.Mana, creature.GetAdjustedManaCost(skill.RankData.ManaActive),
                        creature.ManaMax, 1000);
                if (skill.RankData.StaminaActive != 0)
        ...
        public void CombatSetAim2(ChannelClient client, Packet packet)
        {
            ...
            if (activeSkill == null || activeSkill.Data.Type != SkillType.RangedCombat || creature.Target == null)
            {
                ...
                Log.Debug("CombatSetAim: No active (ranged) skill or invalid target.. ({0}, {1}, {2})", skillId, type,
                    targetId);
                Send.CombatSetAimR(creature, 0, SkillId.None, 0);
                return;
            }
        ...
        public void ProductionSuccessRequest(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning(
                    "ProductionSuccessRequest: Creature '{0:X16}' tried to request success rate without having the skill.",
                    creature.EntityId);
                return;
            }
        ...
        public void TransferSkillExp(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning("TransferSkillExp: User '{0}' tried to transfer exp from '{1}'.", client.Account.Id,
                    fromSkillId);
                goto L_End;
            }
            ...
            {
                Log.Warning("TransferSkillExp: User '{0}' tried to transfer exp to '{1}'.", client.Account.Id,
                    toSkillId);
                goto L_End;
            }
            ...
            {
                Log.Warning("TransferSkillExp: User '{0}' tried to transfer invalid exp amount '{1}'.",
                    client.Account.Id, amount);
                goto L_End;
            }
            ...
            {
                Log.Warning(
                    "TransferSkillExp: User '{0}' tried to transfer exp from or to a skill they don't have '{1}'->'{2}'.",
                    client.Account.Id, fromSkillId, toSkillId);
                goto L_End;
            }

            if (fromSkill.Info.ConditionCount1 < amount)
            {
                Log.Warning("TransferSkillExp: User '{0}' tried to transfer more exp than the skill has.",
                    client.Account.Id);
                goto L_End;
            }
            ...
            {
                Log.Warning("TransferSkillExp: User '{0}' tried to transfer exp to a fully trained skill.",
                    client.Account.Id);
                goto L_End;
            }
