...
namespace Aura.Channel.Network.Handlers
{
    public partial class ChannelServerHandlers : PacketHandlerManager<ChannelClient>
    {
        ...
        public void HomesteadInfoRequest(ChannelClient client, Packet packet)
        {
            ...
            // Re-open GMCP
            if (creature.Vars.Perm.GMCP != null &&
                client.Account.Authority >= ChannelServer.Instance.Conf.World.GmcpMinAuth)
        ...
        {
            Log.Warning(
                "A client seems to be incompatible with the server, the latest version of Aura only supports the latest NA update. (Account id: {0})",
                client.Account.Id);
        }
        ...
        public void SpinColorWheel(ChannelClient client, Packet packet)
        {
            ...
            if (!creature.Skills.IsActive(SkillId.NameColorChange) &&
                !creature.Skills.IsActive(SkillId.UseItemChattingColorChange))
            {
                Log.Warning(
                    "SpinColorWheel: Creature '{0:X16}' tried to spin color wheel without the necesseray skill being active.",
                    creature.EntityId);
                return;
            }
        ...
        public void ChangeNameColor(ChannelClient client, Packet packet)
        {
            ...
            switch (creature.Temp.ColorWheelResult)
            {
                ...
                case 32:
                    ...
                    break;
                default:
                    Log.Warning("ChangeNameColor: Calculating color failed, result: {0}.",
                        creature.Temp.ColorWheelResult);
                    return;
            }
            ...
            // Notice
            if (bothChange)
                Send.Notice(creature, NoticeType.Middle,
                    Localization.Get("Your name and chat text colors have changed."));
            else if (nameChange)
