...
namespace Aura.Channel.Network.Handlers
{
    public partial class ChannelServerHandlers : PacketHandlerManager<ChannelClient>
    {
        ...
        public void ChannelLogin(ChannelClient client, Packet packet)
        {
            ...
            if (!secondaryLogin || client.State == ClientState.LoggingIn)
            {
                ...
                else
                {
                    ...
                    if (character.RegionId == 0 || Math2.Between(character.RegionId, 35000, 40000) ||
                        Math2.Between(character.RegionId, 10000, 11000))
        ...
        public void EnterRegionRequest(ChannelClient client, Packet packet)
        {
            ...
            if (region == null)
            {
                Log.Warning("EnterRegionRequest: Player '{0}' tried to enter unknown region '{1}'.", creature.Name,
                    regionId);
                return;
            }
        ...
        public void SwitchChannel(ChannelClient client, Packet packet)
        {
            ...
            // Get channel
            var channel =
                ChannelServer.Instance.ServerList.GetChannel(ChannelServer.Instance.Conf.Channel.ChannelServer,
                    channelName);
            if (channel == null)
            {
                Log.Debug("Warning: Player '{0}' tried to switch to non-existent channel '{1}'.", creature.Name,
                    channelName);
