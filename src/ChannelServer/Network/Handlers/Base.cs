    ...
    public partial class ChannelServerHandlers : PacketHandlerManager<ChannelClient>
    {
        ...
        public override void Handle(ChannelClient client, Packet packet)
        {
            ...
            catch (AggregateException ex)
            {
                ...
                var unhandled = ex.Flatten().InnerExceptions.Where(e => !CheckInnerSecurityException(client, e))
                    .ToList();
            ...
            catch (Exception ex)
            {
                ...
                if (!CheckInnerSecurityException(client, ex))
                {
                    ...
                    ChannelServer.Instance.Database.LogSecurityIncident(client, IncidentSeverityLevel.Moderate,
                        "Unhandled exception while processing packet: " + ex, null);
                    throw;
                }
            }
        }
        ...
        private void HandleSecurityException(ChannelClient client, SecurityViolationException ex)
        {
            ...
            ChannelServer.Instance.Events.OnSecurityViolation(
                new SecurityViolationEventArgs(client, ex.Level, ex.Message, ex.StackReport));
            ...
            Log.Debug("Client '{0}' : Account {1} (Controlling {2}) just committed a {3} offense. Incident report: {4}",
                client.Address, accName, charName, ex.Level, ex.Message);
        }
    }
}