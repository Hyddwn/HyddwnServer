...
namespace Aura.Channel.Network.Handlers
{
    public partial class ChannelServerHandlers : PacketHandlerManager<ChannelClient>
    {
        ...
        public void PersonalShopChangeTitle(ChannelClient client, Packet packet)
        {
            ...
            if (shop == null)
            {
                Log.Warning("PersonalShopChangeTitle: User '{0}' tried to change title of non-existent shop.",
                    client.Account.Id);
                return;
            }
            ...
            {
                Log.Warning("PersonalShopChangeTitle: User '{0}' tried to set invalid title '{1}'.", client.Account.Id,
                    title);
                return;
            }
        ...
        public void PersonalShopChangeDescription(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning(
                    "PersonalShopChangeDescription: User '{0}' tried to change description of non-existent shop.",
                    client.Account.Id);
                return;
            }
            ...
            {
                Log.Warning("PersonalShopChangeDescription: User '{0}' tried to set invalid description '{1}'.",
                    client.Account.Id, description);
                return;
            }
        ...
        public void PersonalShopTakeDown(ChannelClient client, Packet packet)
        {
            ...
            if (shop == null)
            {
                Log.Warning("PersonalShopTakeDown: User '{0}' tried to take down non-existent shop.",
                    client.Account.Id);
        ...
        public void PersonalShopSetPrice(ChannelClient client, Packet packet)
        {
            ...
            if (shop == null)
            {
                Log.Warning("PersonalShopSetPrice: User '{0}' tried to set price for an item in a non-existent shop.",
                    client.Account.Id);
                Send.PersonalShopTakeDownR(creature, false);
                return;
            }
            ...
            if (!success)
                Log.Warning("PersonalShopSetPrice: User '{0}' tried to set price for an invalid item.",
                    client.Account.Id);
        }
        ...
        public void PersonalShopOpen(ChannelClient client, Packet packet)
        {
            ...
            if (shop == null || shop.Region != creature.Region ||
                !creature.GetPosition().InRange(shop.Prop.GetPosition(), 1000))
            {
                Send.MsgBox(creature, Localization.Get("Personal Shop not found."));
                Send.PersonalShopOpenR(creature, null);
                return;
