...
namespace Aura.Channel.Network.Handlers
{
    public partial class ChannelServerHandlers : PacketHandlerManager<ChannelClient>
    {
        ...
        public void RequestRebirth(ChannelClient client, Packet packet)
        {
            ...
            var hairStyleData = AuraData.CharacterStyleDb.Find(
                raceData.Gender == Gender.Male ? CharacterStyleType.MaleHairStyle : CharacterStyleType.FemaleHairStyle,
                hairItemId);
            var faceData = AuraData.CharacterStyleDb.Find(
                raceData.Gender == Gender.Male ? CharacterStyleType.MaleFace : CharacterStyleType.FemaleFace,
                faceItemId);
            ...
            if (hairItemId != hair.Info.Id)
            {
                ...
                if (!raceData.HasTag(hairStyleData.Races))
                    throw new ModerateViolation(
                        "Player tried to rebirth with hair not available to their race: {0}, {1}", creature.RaceId,
                        hairItemId);
            ...
            if (faceItemId != face.Info.Id)
            {
                ...
                if (!raceData.HasTag(faceData.Races))
                    throw new ModerateViolation(
                        "Player tried to rebirth with face not available to their race: {0}, {1}", creature.RaceId,
                        faceItemId);
            ...
            if (hairColor != (byte) hair.Info.Color1)
            {
                ...
                if (!raceData.HasTag(hairColorData.Races))
                    throw new ModerateViolation(
                        "Player tried to rebirth with hair color not available to their race: {0}, {1}",
                        creature.RaceId, hairColor);
            ...
            if (skinColor != creature.SkinColor)
            {
                ...
                if (!raceData.HasTag(skinColorData.Races))
                    throw new ModerateViolation(
                        "Player tried to rebirth with skin color not available to their race: {0}, {1}",
                        creature.RaceId, skinColor);
            ...
            if (eyeColor != creature.EyeColor)
            {
                ...
                if (!raceData.HasTag(eyeColorData.Races))
                    throw new ModerateViolation(
                        "Player tried to rebirth with eye color not available to their race: {0}, {1}", creature.RaceId,
                        eyeColor);
            ...
            if (eyeType != creature.EyeType)
            {
                ...
                if (!raceData.HasTag(eyeTypeData.Races))
                    throw new ModerateViolation(
                        "Player tried to rebirth with eye type not available to their race: {0}, {1}", creature.RaceId,
                        eyeType);
            ...
            if (creature.Points < totalPrice)
                throw new ModerateViolation(
                    "Player tried to rebirth without being able to pay for the selected options, total price: {0}, Points: {1}",
                    totalPrice, creature.Points);
            ...
            {
                // Tir beginner area
                case RebirthLocation.Tir:
                    creature.SetLocation(125, 21489, 76421);
                    break;

                // Iria beginner area (Rano)
                case RebirthLocation.Iria:
                    creature.SetLocation(3001, 164533, 161862);
                    break;
            }
