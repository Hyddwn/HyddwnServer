...
namespace Aura.Channel.Network.Handlers
{
    public partial class ChannelServerHandlers : PacketHandlerManager<ChannelClient>
    {
        ...
        public void PartyCreate(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning("PartyCreate: User '{0}' tried to create a party while already being in one.",
                    client.Account.Id);
            ...
            if (maxSize > ChannelServer.Instance.Conf.World.PartyMaxSize)
                Send.MsgBox(creature, Localization.Get("The maximum party size allowed on this server is {0}."),
                    ChannelServer.Instance.Conf.World.PartyMaxSize);
        ...
        public void PartyChangeFinish(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning("PartyChangeFinish: Creature '{0:X16}' tried to use unknown rule '{1}'.", creature.EntityId,
                    rule);
            ...
            if (!creature.IsInParty || creature != party.Leader)
            {
                Log.Warning("PartyLeave: User '{0}' tried to change party finish setting illigitly.",
                    client.Account.Id);
                Send.PartyChangeFinishR(creature, false);
                return;
            }
        ...
        public void PartyChangeExp(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning("PartyChangeExp: Creature '{0:X16}' tried to use unknown rule '{1}'.", creature.EntityId,
                    rule);
                Send.PartyChangeFinishR(creature, false);
                return;
            }
        ...
        public void PartySetQuest(ChannelClient client, Packet packet)
        {
            ...
            if (!creature.IsInParty)
            {
                Log.Warning("PartySetQuest: Creature '{0:X16}' tried to set party quest without being in a party.",
                    creature.EntityId);
            ...
            if (creature.Party.MemberCount < min)
            {
                Send.MsgBox(creature,
                    Localization.Get("Your party must consist of {0} or more members\nto register a party quest."),
                    min);
                Send.PartySetQuestR(creature, false);
                return;
            }
        ...
        public void PartyUnsetQuest(ChannelClient client, Packet packet)
        {
            ...
            if (!creature.IsInParty)
            {
                Log.Warning("PartyUnsetQuest: Creature '{0:X16}' tried to unset party quest without being in a party.",
                    creature.EntityId);
                Send.PartySetQuestR(creature, false);
                return;
            }
