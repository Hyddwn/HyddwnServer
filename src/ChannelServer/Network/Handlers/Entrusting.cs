...
namespace Aura.Channel.Network.Handlers
{
    public partial class ChannelServerHandlers : PacketHandlerManager<ChannelClient>
    {
        ...
        public void Entrustment(ChannelClient client, Packet packet)
        {
            ...
            if (partner == null)
            {
                Send.Notice(creature,
                    Localization.Get("There is no party member available to ask for an Enchantment."));
                Send.EntrustmentR(creature, false);
                return;
            }
            ...
            var items = creature.Inventory.GetItems(a => a.HasTag("/enchant/powder/") && !a.HasTag("/powder05/"),
                StartAt.BottomRight);
            if (items.Count == 0)
            {
                // Unofficial
        ...
        public void EntrustmentCancel(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning("EntrustmentCancel: User '{0}' tried to cancel entrustment without being in one.",
                    client.Account.Id);
                return;
            }
        ...
        public void EntrustmentRefuse(ChannelClient client, Packet packet)
        {
            ...
            if (creature.Temp.ActiveEntrustment == null)
            {
                Log.Warning("EntrustmentRefuse: User '{0}' tried to cancel entrustment without being in one.",
                    client.Account.Id);
                return;
            }
        ...
        public void EntrustmentFinalizeRequest(ChannelClient client, Packet packet)
        {
            ...
            if (creature.Temp.ActiveEntrustment == null)
            {
                Log.Warning("EntrustmentFinalizeRequest: User '{0}' tried to cancel entrustment without being in one.",
                    client.Account.Id);
