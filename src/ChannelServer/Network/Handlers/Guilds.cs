    ...
    public partial class ChannelServerHandlers : PacketHandlerManager<ChannelClient>
    {
        ...
        public void GuildDonate(ChannelClient client, Packet packet)
        {
            ...
            if (checkId != 0)
            {
                ...
                if (!check.HasTag("/check/"))
                {
                    Log.Warning("GuildDonate: User '{0}' tried to donate invalid check ({1}).", client.Account.Id,
                        check.Info.Id);
                    Send.GuildDonateR(creature, false);
                    return;
                }
        ...
        public void GuildDestroyStone(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning("GuildDestroyStone: User '{0}' tried to destroy stone without being leader.",
                    client.Account.Id);
                Send.GuildDonateR(creature, false);
                return;
            }
        ...
        ///     002 [................] String :
        ///     aaaaaaaaaabbbbbbbbbbccccccccccddddddddddeeeeeeeeeeffffffffffgggggggggghhhhhhhhhhjjjjjjjjjjiiiiiiiiii
        /// </example>
        ...
        public void GuildApply(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning("GuildApply: User '{0}' sent an application that was >100 characters, truncated.",
                    client.Account.Id);
            ...
            Send.GuildMessage(creature, guild,
                Localization.Get(
        ...
        public void GuildChangeStone(ChannelClient client, Packet packet)
        {
            ...
            switch (stoneType)
            {
                case GuildStoneType.Hope:
                    level = GuildLevel.Advanced;
                    break;
                case GuildStoneType.Courage:
            ...
            {
                Log.Warning(
                    "GuildChangeStone: User '{0}' tried to change stone without having the necessary guild level.",
                    client.Account.Id);
                return;
            }
            ...
            if (guild.Points < points || guild.Gold < gold)
            {
                Send.MsgBox(creature,
                    Localization.Get("You need {0:n0} Guild Points and {1:n0} Gold to change the guild stone."), points,
                    gold);
                return;
            }
        ...
        public void GuildWithdrawGold(ChannelClient client, Packet packet)
        {
            ...
            if (guild.WithdrawMaxAmount == 0)
            {
                Send.MsgBox(creature,
                    Localization.Get(
                        "A total agreememt from the Guild is required to withdraw the Gold from the guild safe. Please set up a poll in the guild homepage."));
                Send.GuildWithdrawGoldR(creature, false);
                return;
            }
            ...
            if (DateTime.Now > guild.WithdrawDeadline)
            {
                Send.MsgBox(creature,
                    Localization.Get(
                        "Withdrawal period expired. Guild funds may only be withdrawn for up to one week after winning the majority vote."));
                Send.GuildWithdrawGoldR(creature, false);
                return;
            }
            ...
            if (maxAmount < amount)
            {
                Log.Warning("GuildWithdrawGold: User '{0}' requested more than they should be able to.",
                    client.Account.Id);
                Send.GuildWithdrawGoldR(creature, false);
                return;
            }
        ...
        public void GuildCreateRequest(ChannelClient client, Packet packet)
        {
            ...
            {
                Log.Warning("GuildCreateRequest: User '{0}' sent invalid type or visibility ({1}, {2}).",
                    client.Account.Id, type, visibility);
                Send.GuildCreateRequestR(creature, false);
                return;
            }
            ...
            var size = 5;
            if (!creature.IsInParty || party.MemberCount != size || party.GetMembers().Any(a => a.GuildId != 0) ||
                party.Leader != creature)
            {
                Send.MsgBox(creature,
                    Localization.GetPlural("You must be the leader of a party of {0} to form a guild.",
                        "You must be the leader of a party of {0} to form a guild.", size), size);
                Send.GuildCreateRequestR(creature, false);
                return;
            }
        ...
        public void GuildNameVote(ChannelClient client, Packet packet)
        {
            ...
            if (party.Leader.EntityId != leaderEntityId)
            {
        ...
        public void GuildCreationConfirmation(ChannelClient client, Packet packet)
        {
            ...
            var size = 5;
            if (!creature.IsInParty || party.MemberCount != size || party.GetMembers().Any(a => a.GuildId != 0) ||
                party.Leader != creature || party.GuildNameVoteCount != party.MemberCount)
            {
                Send.MsgBox(creature,
                    Localization.GetPlural("You must be the leader of a party of {0} to form a guild.",
                        "You must be the leader of a party of {0} to form a guild.", size), size);
        ...
        public void GuildCreateGuildRobe(ChannelClient client, Packet packet)
        {
            ...
            {
                Send.MsgBox(creature,
                    Localization.Get("Your guild needs 1,000 GP and 50,000 Gold in order to design a Guild Robe."));
                goto L_Fail;
            }
            ...
            // Response
            Send.GuildCreateGuildRobeUpdate(creature, emblemMark, emblemOutline, stripes, robeColor, badgeColor,
                emblemMarkColor, emblemOutlineColor, stripesColor, true);
            Send.GuildCreateGuildRobeR(creature, true);
            return;

            L_Fail:
            Send.GuildCreateGuildRobeUpdate(creature, emblemMark, emblemOutline, stripes, robeColor, badgeColor,
                emblemMarkColor, emblemOutlineColor, stripesColor, false);
