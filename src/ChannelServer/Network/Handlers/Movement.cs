...
namespace Aura.Channel.Network.Handlers
{
    public partial class ChannelServerHandlers : PacketHandlerManager<ChannelClient>
    {
        ...
        public void EventInform(ChannelClient client, Packet packet)
        {
            ...
            if (clientEvent == null)
            {
                Log.Warning("EventInform: Player '{0:X16}' triggered unknown event '{1:X16}'.", creature.EntityId,
                    eventId);
                return;
            }
            ...
            if (saveTownParam != null && signalType == SignalType.Enter && clientEvent.IsInside(pos.X, pos.Y))
                if (saveTownParam.XML != null)
                {
                    ...
                    {
                        Log.Warning("EventInform: Failed to get globalname for SaveTown '{0:X16}'.",
