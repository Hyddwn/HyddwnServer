...
namespace Aura.Channel.Util
{
    ...
    {
        public SecurityViolationEventArgs(ChannelClient offender, IncidentSeverityLevel level, string report,
            string stacktrace)
        {
            Client = offender;
            Level = level;
            Report = report;
            StackReport = stacktrace;
        }

        public ChannelClient Client { get; }
        public string Report { get; }
        public string StackReport { get; }
        public IncidentSeverityLevel Level { get; }
    }
    ...
    public abstract class SecurityViolationException : Exception
    {
        ...
        protected SecurityViolationException(IncidentSeverityLevel lvl, string report)
        {
            ...
            Message = string.Format("{0}: {1}", stacktrace.GetFrame(0).GetMethod().Name, report);
        }
    ...
    public static class Autoban
    {
        ...
        public static void Incident(ChannelClient client, IncidentSeverityLevel level, string report,
            string stacktrace = null)
        {
            ...
            switch (level)
            {
                case IncidentSeverityLevel.Mild:
                    client.Account.AutobanScore += ChannelServer.Instance.Conf.Autoban.MildAmount;
                    break;
                case IncidentSeverityLevel.Moderate:
                    client.Account.AutobanScore += ChannelServer.Instance.Conf.Autoban.ModerateAmount;
                    break;
                case IncidentSeverityLevel.Severe:
                    client.Account.AutobanScore += ChannelServer.Instance.Conf.Autoban.SevereAmount;
                    break;
                default:
            ...
            switch (ChannelServer.Instance.Conf.Autoban.LengthIncrease)
            {
                ...
                case AutobanLengthIncrease.Linear:
                    banLength = TimeSpan.FromMinutes(
                        (long) (ChannelServer.Instance.Conf.Autoban.InitialBanTime.TotalMinutes * autobanCount));
                    break;

                case AutobanLengthIncrease.Exponential:
                    banLength = TimeSpan.FromMinutes(ChannelServer.Instance.Conf.Autoban.InitialBanTime.TotalMinutes *
                                                     (long) Math.Pow(autobanCount, 2));
                    break;

                default:
                    Log.Warning("Unknown AutobanLengthIncrease: {0}",
                        ChannelServer.Instance.Conf.Autoban.LengthIncrease);
