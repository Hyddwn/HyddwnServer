...
namespace Aura.Channel.Util.Configuration.Files
{
    public class AutobanConfFile : ConfFile
    {
        ...
        public void Load()
        {
            Require("system/conf/autoban.conf");

            Enabled = GetBool("enabled", false);
            BanAt = GetInt("ban_at", 10);
            MildAmount = GetInt("mild_amount", 1);
            ModerateAmount = GetInt("moderate_amount", 5);
            SevereAmount = GetInt("severe_amount", 10);
            ReductionTime = GetTimeSpan("reduction_time", TimeSpan.FromDays(7));
            InitialBanTime = GetTimeSpan("initial_ban_time", TimeSpan.FromHours(6));
            LengthIncrease = GetEnum("length_increase", AutobanLengthIncrease.Exponential);
            ResetScoreOnBan = GetBool("reset_on_ban", false);
        }
    }
}