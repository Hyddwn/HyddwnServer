...
namespace Aura.Channel.Util
{
    public class GmCommandManager : CommandManager<GmCommand, GmCommandFunc>
    {
        public GmCommandManager()
        {
            ...
            Add(00, -1, "help", "[command]", Localization.Get("Displays available commands and their usage."),
                HandleHelp);
            Add(00, -1, "equipset", "<set (0, 1, 2, 3, ...)>", Localization.Get("Switches to the given equip set."),
                HandleEquipSet);
            ...
            Add(01, 50, "haircolor", "<hex color>", Localization.Get("Changes character's hair color."),
                HandleHairColor);
            ...
            Add(01, 50, "nccolor", "<id (0-32)>", Localization.Get("Changes name and chat color."),
                HandleNameChatColor);

            // GMs
            Add(50, 50, "warp", "<region> [x] [y]", Localization.Get("Warps to a specific region and position."),
                HandleWarp);
            Add(50, 50, "jump", "[x] [y]", Localization.Get("Warps to a specific position in the current region."),
                HandleJump);
            Add(50, 50, "goto", "<target name>", Localization.Get("Warps to a specific creature."), HandleGoTo);
            Add(50, 50, "item", "<id|name> [amount|color1 [color2 [color 3]]]", Localization.Get("Spawns item."),
                HandleItem);
            ...
            Add(50, 50, "ego", "<item id> <ego name> <ego race> [color1 [color2 [color 3]]]",
                Localization.Get("Creates spirit weapon."), HandleEgo);
            ...
            Add(50, 50, "msg", "<message>", Localization.Get("Broadcasts a system message to all logged in players."),
                HandleMsg);
            Add(50, 50, "broadcast", "<message>", Localization.Get("Broadcasts message on all channels."),
                HandleBroadcast);
            Add(50, 50, "allskills", "", Localization.Get("Adds all supported skills on their max rank."),
                HandleAllSkills);
            ...
            Add(50, 50, "favor", "<npc name> [amount]", Localization.Get("Changes favor of an NPC towards the player."),
                HandleFavor);
            Add(50, 50, "stress", "<npc name> [amount]",
                Localization.Get("Changes stress of an NPC towards the player."), HandleStress);
            Add(50, 50, "memory", "<npc name> [amount]",
                Localization.Get("Changes how well an NPC remembers the player."), HandleMemory);
            Add(50, 50, "weather", "[0.0~2.0|clear|rain|storm|type1~type12]",
                Localization.Get("Changes weather in player's region."), HandleWeather);
            Add(50, 50, "telewalk", "", Localization.Get("Enables/disables teleportation in place of walking."),
                HandleTeleWalk);
            Add(50, 50, "points", "<modificator>", Localization.Get("Modificates account's points (Pon)."),
                HandlePoints);
            Add(50, 50, "fillpotions", "", Localization.Get("Fills all potion stacks in inventory."),
                HandleFillPotions);
            ...
            Add(50, 50, "quest", "<id> [finish objective]",
                Localization.Get("Starts a quest or sets the specified objective to be finished."), HandleQuest);
            Add(50, 50, "prof",
                "[equipflags=all|armor|glove|shoe|head|robe|rhand1|rhand2|lhand1|lhand2|accessory1|accessory2] [amount=101000]",
                Localization.Get("Sets proficiency of equipped item(s)."), HandleProf);
            Add(50, 50, "resetcd", "", Localization.Get("Resets all cool downs."), HandleResetCoolDown);

            // Admins
            Add(99, 99, "dynamic", "[variant]", Localization.Get("Creates dynamic region, based on the current one."),
                HandleDynamic);
            Add(99, 99, "dungeon", "<dungeon name>",
                Localization.Get("Creates a new dungeon instance and warps there."), HandleDungeon);
            Add(99, -1, "reloaddata", "", Localization.Get("Reloads file data (items, skills, etc)."),
                HandleReloadData);
            Add(99, -1, "reloadscripts", "", Localization.Get("Reloads scripts (NPCs, monsters, AIs, etc)."),
                HandleReloadScripts);
            ...
            Add(99, 99, "nosave", "",
                Localization.Get("Marks creature's controlled by the target's client to not be saved on logout."),
                HandleNoSave);
            Add(99, -1, "dbgregion", "[scale=20] [entityIds|propIds]",
                Localization.Get("Creates an image of the current region and its and client events."),
                HandleDebugRegion);
        ...
        {
            Add(new GmCommand(auth, charAuth, name, usage, "", func));
        }
        ...
        {
            Add(new GmCommand(auth, charAuth, name, usage, description, func));
        }
        ...
        {
            if (message.Length < 2 ||
                !message.StartsWith(ChannelServer.Instance.Conf.Commands.Prefix.ToString(CultureInfo.InvariantCulture)))
                return false;

            // Parse arguments
            var args = ParseLine(message);
            ...
            var commandConf =
                ChannelServer.Instance.Conf.Commands.GetAuth(command.Name, command.Auth, command.CharAuth);

            // Check auth
            if (!isCharCommand && client.Account.Authority < commandConf.Auth ||
                isCharCommand && client.Account.Authority < commandConf.CharAuth)
            {
            ...
            {
                Send.ServerMessage(creature, Localization.Get("Command '{0}' cannot be used on another character."),
                    args[0]);
                return true;
            }
            ...
            if (result == CommandResult.InvalidArgument)
            {
                ...
                if (command.CharAuth <= client.Account.Authority && command.CharAuth > 0)
                    Send.ServerMessage(creature, Localization.Get("Usage: {0} <target> {1}"), command.Name,
                        command.Usage);
        ...
        private CommandResult HandleWhere(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            ...
            Send.ServerMessage(sender, msg, target.RegionId, pos.X, pos.Y, target.Name, target.Direction, areaId,
                MabiMath.ByteToRadian(target.Direction));

            return CommandResult.Okay;
        }

        private CommandResult HandleWarp(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
        ...
        private CommandResult HandleJump(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            var regionId = target.RegionId;
            int x = -1, y = -1;
        ...
        private CommandResult HandleGo(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
            {
            ...
            if (destination.StartsWith("tir"))
            {
                regionId = 1;
                x = 12801;
                y = 38397;
            }
            else if (destination.StartsWith("dugald"))
            {
                regionId = 16;
                x = 23017;
                y = 61244;
            }
            else if (destination.StartsWith("dun"))
            {
                regionId = 14;
                x = 38001;
                y = 38802;
            }
            else if (destination.StartsWith("gairech"))
            {
                regionId = 30;
                x = 39295;
                y = 53279;
            }
            else if (destination.StartsWith("bangor"))
            {
                regionId = 31;
                x = 12904;
                y = 12200;
            }
            else if (destination.StartsWith("emain"))
            {
                regionId = 52;
                x = 39818;
                y = 41621;
            }
            else if (destination.StartsWith("tail"))
            {
                regionId = 300;
                x = 212749;
                y = 192720;
            }
            else if (destination.StartsWith("tara"))
            {
                regionId = 401;
                x = 99793;
                y = 91209;
            }
            else if (destination.StartsWith("cobh"))
            {
                regionId = 23;
                x = 28559;
                y = 37693;
            }
            else if (destination.StartsWith("ceo"))
            {
                regionId = 56;
                x = 8987;
                y = 9611;
            }
            else if (destination.StartsWith("neko"))
            {
                regionId = 600;
                x = 114430;
                y = 79085;
            }
            else if (destination.StartsWith("gm"))
            {
                regionId = 22;
                x = 2500;
                y = 2500;
            }
            else if (destination.StartsWith("alby"))
            {
                regionId = 13;
                x = 3200;
                y = 3200;
            }
            else if (destination.StartsWith("ciar"))
            {
                regionId = 11;
                x = 3200;
                y = 3200;
            }
            else if (destination.StartsWith("rabbie"))
            {
                regionId = 24;
                x = 3200;
                y = 3425;
            }
            else if (destination.StartsWith("math"))
            {
                regionId = 25;
                x = 3200;
                y = 3425;
            }
            else if (destination.StartsWith("barri"))
            {
                regionId = 32;
                x = 3200;
                y = 2880;
            }
            else if (destination.StartsWith("fiodh"))
            {
                regionId = 49;
                x = 3530;
                y = 7150;
            }
            else if (destination.StartsWith("albey"))
            {
                regionId = 44;
                x = 3200;
                y = 3450;
            }
            else if (destination.StartsWith("coil"))
            {
                regionId = 54;
                x = 3520;
                y = 7180;
            }
            else if (destination.StartsWith("runda"))
            {
                regionId = 64;
                x = 5600;
                y = 8190;
        ...
        private CommandResult HandleItem(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
            ...
            if (!int.TryParse(args[1], out itemId))
            {
                ...
                if (all.Count > 0)
                {
                    ...
                    if (all.Count > 1 && score != 0)
                    {
                        ...
                        Send.ServerMessage(sender,
                            Localization.Get("No exact match found for '{0}', using best result, '{1}' ({2:0.0}%)."),
                            args[1], itemData.Name, perc);
                    }
                }
            }
            else
            ...
            {
                if (sender != target)
                    Send.ServerMessage(target, Localization.Get("Item '{0}' has been spawned by '{1}'."), itemData.Name,
                        sender.Name);
        ...
        private CommandResult HandleEnchant(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandleManual(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandleDynamic(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            // Get variant
            var variant = "";
            ...
            Send.ServerMessage(sender, Localization.Get("Created new region based on region {0}, new region's id: {1}"),
        ...
        private CommandResult HandleItemInfo(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandleSkillInfo(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandleRaceInfo(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandleSkill(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
            ...
            {
                Send.ServerMessage(sender, Localization.Get("Skill '{0}' doesn't have rank '{1}'."), args[1],
                    (SkillRank) rank);
                return CommandResult.Fail;
            }
            ...
            if (target != sender)
                Send.ServerMessage(sender, Localization.Get("Skill '{0}' added by '{1}'."), (SkillId) skillId,
                    sender.Name);

            return CommandResult.Okay;
        }

        private CommandResult HandleBody(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandleCp(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (sender == target)
                Send.ServerMessage(sender, Localization.Get("Your combat power: {0:0.0}"), target.CombatPower);
            else
                Send.ServerMessage(sender, Localization.Get("{0}'s combat power: {1:0.0}"), target.Name,
                    target.CombatPower);

            return CommandResult.Okay;
        }

        private CommandResult HandleHairColor(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
            ...
            Send.ServerMessage(sender, Localization.Get("Change successful, new value: {0}"),
                "0x" + color.ToString("X8"));
            if (sender != target)
                Send.ServerMessage(target, Localization.Get("Your appearance has been changed by {0}."), sender.Name);
        ...
        private CommandResult HandleTitle(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandleSpeed(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            short speed = 0;
            ...
            if (sender != target)
                Send.ServerMessage(target, Localization.Get("Your speed has been changed to +{0}% by {1}."), speed,
                    sender.Name);

            return CommandResult.Okay;
        }

        private CommandResult HandleSpawn(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
            ...
            for (var i = 0; i < amount; ++i)
            {
                ...
                var creature =
                    ChannelServer.Instance.World.SpawnManager.Spawn(raceId, target.RegionId, x, y, true, true);
        ...
        private CommandResult HandleDie(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            target.Kill(sender);
        ...
        private CommandResult HandleReloadData(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
        ...
        private CommandResult HandleReloadScripts(ChannelClient client, Creature sender, Creature target,
            string message, IList<string> args)
        {
            Send.ServerMessage(sender,
                Localization.Get(
                    "Beware, reloading should only be used during development, it's not guaranteed to be safe."));
            Send.ServerMessage(sender, Localization.Get("Reloading, this might take a moment."));
        ...
        private CommandResult HandleReloadConf(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            Send.ServerMessage(sender,
                Localization.Get(
        ...
        private CommandResult HandleAp(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
            ...
            if (target != sender)
                Send.ServerMessage(target,
                    Localization.GetPlural("{0} gave you {1} AP.", "{0} gave you {1} AP.", amount), sender.Name,
                    amount);

            return CommandResult.Okay;
        }

        private CommandResult HandleCloseNpc(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (!target.Client.NpcSession.IsValid())
                return CommandResult.Fail;

            Send.NpcTalkEndR(target.Client.NpcSession.Script.Player, target.Client.NpcSession.Script.NPC.EntityId,
                "Ended by closenpc command.");
        ...
        private CommandResult HandleGmcp(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
        ...
        private CommandResult HandleCard(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandleHeal(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            target.FullHeal();
        ...
        private CommandResult HandleClean(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            ...
            if (target != sender)
                Send.ServerMessage(target,
                    Localization.Get("{0} removed all items that were lying on the floor in your region."),
                    sender.Name);

            return CommandResult.Okay;
        }

        private CommandResult HandleCondition(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            ...
            for (var i = 1; i < args.Count; ++i)
                if (!ulong.TryParse(args[i].Replace("0x", ""), NumberStyles.HexNumber, CultureInfo.InvariantCulture,
                    out conditions[i - 1]))
                {
            ...
            // Apply conditions
            target.Conditions.Deactivate(ConditionsA.All);
            target.Conditions.Activate((ConditionsA) conditions[0]);
            target.Conditions.Deactivate(ConditionsB.All);
            target.Conditions.Activate((ConditionsB) conditions[1]);
            target.Conditions.Deactivate(ConditionsC.All);
            target.Conditions.Activate((ConditionsC) conditions[2]);
            target.Conditions.Deactivate(ConditionsD.All);
            target.Conditions.Activate((ConditionsD) conditions[3]);
            target.Conditions.Deactivate(ConditionsE.All);
            target.Conditions.Activate((ConditionsE) conditions[4]);
            target.Conditions.Deactivate(ConditionsF.All);
            target.Conditions.Activate((ConditionsF) conditions[5]);
            target.Conditions.Deactivate(ConditionsG.All);
            target.Conditions.Activate((ConditionsG) conditions[6]);
        ...
        private CommandResult HandleEffect(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
        ...
        private CommandResult HandleProp(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandleWho(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            ...
            if (regionId != 0)
            {
                ...
                Send.ServerMessage(sender, Localization.Get("Players online in region {0} ({1}):"), regionId,
                    players.Count);
            }
            else
            {
        ...
        private CommandResult HandleMotion(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 3)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandleGesture(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandleBroadcast(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandleMsg(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandleAllSkills(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            // List of "working", normal player skills
            var listOfSkills = new[]
            {
                SkillId.Counterattack, SkillId.CriticalHit, SkillId.Defense, SkillId.FinalHit, SkillId.MagnumShot,
                SkillId.RangedAttack,
                SkillId.SharpMind, SkillId.Smash, SkillId.Windmill, SkillId.SupportShot, SkillId.ArrowRevolver2,

                SkillId.Blacksmithing, SkillId.Campfire, SkillId.FirstAid, SkillId.Fishing, SkillId.Handicraft,
                SkillId.Herbalism, SkillId.PotionMaking,
                SkillId.ProductionMastery, SkillId.Refining, SkillId.Rest, SkillId.Tailoring, SkillId.Weaving,

                SkillId.Firebolt, SkillId.Healing, SkillId.Icebolt, SkillId.Lightningbolt, SkillId.ManaShield,
                SkillId.Meditation, SkillId.SilentMove,
            ...
            foreach (var sid in listOfSkills)
            {
                ...
                target.Skills.Give(sid, skill.MaxRank);
            }
            ...
            if (target != sender)
                Send.ServerMessage(target,
                    Localization.Get("{0} gave you all skills the server supports on their max rank."), sender.Name);

            return CommandResult.Okay;
        }

        private CommandResult HandleAllTitles(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            ...
            if (target != sender)
                Send.ServerMessage(target,
                    Localization.Get("{0} enabled all available titles for you, please relog to use them."),
                    sender.Name);

            return CommandResult.Okay;
        }

        private CommandResult HandleDistance(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            ...
            if (distancePos == null)
            {
                ...
                Send.ServerMessage(sender,
                    Localization.Get("Position 1 saved, use command again to calculate distance."));
            }
            else
            {
                ...
                Send.ServerMessage(sender, Localization.Get("Distance between '{0}' and '{1}': {2}"), distancePos, pos2,
                    distance);
        ...
        private CommandResult HandleGold(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
            ...
            if (sender != target)
                Send.SystemMessage(target,
                    string.Format(Localization.GetPlural("{0} gave you {1:n0}g.", "{0} gave you {1:n0}g.", amount),
                        sender.Name, amount));

            return CommandResult.Okay;
        }

        private CommandResult HandleFavor(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
            ...
            if (sender != target)
                Send.SystemMessage(target, Localization.Get("{2} changed {0}'s favor towards you, new value: {1}"),
                    name, favor, sender.Name);

            return CommandResult.Okay;
        }

        private CommandResult HandleStress(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
            ...
            if (sender != target)
                Send.SystemMessage(target, Localization.Get("{2} changed {0}'s stress towards you, new value: {1}"),
                    name, stress, sender.Name);

            return CommandResult.Okay;
        }

        private CommandResult HandleMemory(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
            ...
            if (sender != target)
                Send.SystemMessage(target, Localization.Get("{2} changed how well {0} remembers you, new value: {1}"),
                    name, memory, sender.Name);

            return CommandResult.Okay;
        }

        private CommandResult HandleEgo(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 4)
            {
            ...
            // Get ego race
            EgoRace egoRace;
            if (!Enum.TryParse(args[3], out egoRace) || egoRace <= EgoRace.None || egoRace > EgoRace.CylinderF)
            {
            ...
            {
                if (sender != target)
                    Send.ServerMessage(target, Localization.Get("Ego '{0}' has been spawned by '{1}'."), itemData.Name,
                        sender.Name);
        ...
        private static bool TryParseColorsFromArgs(IList<string> args, int offset, out uint color1, out uint color2,
            out uint color3)
        {
            color1 = 0;
            color2 = 0;
            ...
            for (var i = 0; i < 3; ++i)
            {
                ...
                {
                    if (!uint.TryParse(sColor.Substring(2), NumberStyles.HexNumber, CultureInfo.InvariantCulture,
                        out color))
                        return false;
                }
                else
        ...
        private CommandResult HandleWeather(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            ...
            if (args[1].StartsWith("type"))
            {
                ...
                ChannelServer.Instance.Weather.SetProviderAndUpdate(target.RegionId,
                    new WeatherProviderTable(target.RegionId, args[1]));
            ...
            ChannelServer.Instance.Weather.SetProviderAndUpdate(target.RegionId,
                new WeatherProviderConstant(target.RegionId, val));

            return CommandResult.Okay;
        }

        private CommandResult HandleTeleWalk(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            // Toggle telewalk
        ...
        private CommandResult HandleDungeon(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
            ...
            if (sender != target)
                Send.ServerMessage(target, Localization.Get("{0} warped you into {1}."), sender.Name,
                    dungeonData.EngName);

            return CommandResult.Okay;
        }

        private CommandResult HandleLastTown(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
        ...
        private CommandResult HandleCutscene(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandlePartySize(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
            ...
            if (sender != target)
                Send.SystemMessage(target, Localization.Get("Party size changed to {0} by {1}."), target.Party.MaxSize,
                    sender.Name);

            return CommandResult.Okay;
        }

        private CommandResult HandleOpenShop(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandleNameChatColor(ChannelClient client, Creature sender, Creature target,
            string message, IList<string> args)
        {
            var idx = 0;
            ...
            if (sender == target)
                ...
                Send.SystemMessage(target, Localization.Get("Your name/chat color has been changed by {0}."),
                    sender.Name);

            return CommandResult.Okay;
        }

        private CommandResult HandlePoints(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            ...
            {
                if (sender == target)
                    Send.ServerMessage(sender, Localization.GetPlural("You have {0} Pon.", "You have {0} Pon.", oldVal),
                        oldVal);
                else
                    Send.ServerMessage(sender, Localization.GetPlural("{1} has {0} Pon.", "{1} has {0} Pon.", oldVal),
                        oldVal, target.Name);
            ...
            if (sender != target)
                Send.ServerMessage(target, Localization.Get("Your Pon have been modificated by {2}: {0} -> {1}."),
                    oldVal, newVal, sender.Name);

            return CommandResult.Okay;
        }

        private CommandResult HandleShutdown(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandleNoSave(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            ...
            Send.ServerMessage(sender,
                Localization.GetPlural("Marked {0} creature to *not* be saved.",
                    "Marked {0} creatures to *not* be saved.", creatures.Length), creatures.Length);
            if (sender != target)
                Send.ServerMessage(sender, Localization.Get("{0} marked your creatures to *not* be saved on logout."),
                    sender.Name);

            return CommandResult.Okay;
        }

        private CommandResult HandleFillPotions(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            var count = 0;
            ...
            Send.ServerMessage(sender,
                Localization.GetPlural("Filled {0} potion stack.", "Filled {0} potion stacks.", count), count);
            if (target != sender)
                Send.ServerMessage(sender,
                    Localization.GetPlural("{0} filled {1} of your potion stacks.",
                        "{0} filled {1} of your potion stacks.", count), sender.Name, count);

            return CommandResult.Okay;
        }

        private CommandResult HandleDebugRegion(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            var scale = 20;
            var padding = 60;
            ...
            try
            {
                ...
                using (var gfx = Graphics.FromImage(bmp))
                {
                    ...
                    foreach (var entity in events)
                    {
                        ...
                        {
                            gfx.DrawLine(pen, points[0].X / scale + padding, bmp.Height - points[0].Y / scale - padding,
                                points[1].X / scale + padding, bmp.Height - points[1].Y / scale - padding);
                            gfx.DrawLine(pen, points[1].X / scale + padding, bmp.Height - points[1].Y / scale - padding,
                                points[2].X / scale + padding, bmp.Height - points[2].Y / scale - padding);
                            gfx.DrawLine(pen, points[2].X / scale + padding, bmp.Height - points[2].Y / scale - padding,
                                points[3].X / scale + padding, bmp.Height - points[3].Y / scale - padding);
                            gfx.DrawLine(pen, points[3].X / scale + padding, bmp.Height - points[3].Y / scale - padding,
                                points[0].X / scale + padding, bmp.Height - points[0].Y / scale - padding);
                        }
                    }
            ...
            {
                Send.ServerMessage(sender,
                    Localization.Get("Failed to create debug image, try to use a larger scale."));
                return CommandResult.Fail;
            }
        }

        private CommandResult HandleKeyword(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
            ...
            if (!success)
            {
                if (remove)
                    Send.ServerMessage(sender,
                        Localization.Get("Failed to remove keyword. Maybe the target doesn't have it?"));
                else
                    Send.ServerMessage(sender,
                        Localization.Get("Failed to add keyword. Maybe the target already has it?"));
            }
            else
            {
                if (remove)
                {
                    ...
                    if (sender != target)
                        Send.ServerMessage(sender, Localization.Get("{0} removed your '{1}' keyword."), sender.Name,
                            keyword);
                }
                else
                {
                    ...
                    if (sender != target)
                        Send.ServerMessage(sender, Localization.Get("{0} gave you the keyword '{1}'."), sender.Name,
                            keyword);
                }
            }
        ...
        private CommandResult HandleHelp(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            ...
            if (args.Count == 1)
            {
                var commands = string.Join(", ",
                    _commands.Values.Distinct().Where(a => a.Auth <= auth).Select(a => a.Name).OrderBy(a => a));
            ...
            var aliases = string.Join(", ",
                _commands.Where(a => a.Value == command && commandName != a.Key).Select(a => a.Key).OrderBy(a => a));
        ...
        private CommandResult HandleGoTo(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
        ...
        private CommandResult HandlePtj(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 3)
                return CommandResult.InvalidArgument;
            ...
            if (sender != target)
                Send.ServerMessage(target, Localization.Get("{2} has changed your '{0}' PTJ level to '{1}'."), type,
                    level, sender.Name);

            return CommandResult.Okay;
        }

        private CommandResult HandleSyncGuilds(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
        ...
        private CommandResult HandleRolePlay(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            ...
            Send.ServerMessage(sender,
                Localization.Get("Role-play started, use same command without actor argument to stop."));
            if (sender != target)
        ...
        private CommandResult HandleQuest(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
                return CommandResult.InvalidArgument;
            ...
            if (objective == null)
            {
                ...
                if (target != sender)
                    Send.ServerMessage(target, Localization.Get("The '{0}' quest was started for you by {1}."),
                        quest.Name, sender.Name);
            }
            else
            {
                ...
                Send.ServerMessage(sender, Localization.Get("Finished quest objective '{0}' on '{1}' quest."),
                    objective, quest.Name);
                if (target != sender)
                    Send.ServerMessage(target,
                        Localization.Get("Quest objective '{0}' on '{1}' quest was finished for you by {2}."),
                        objective, quest.Name, sender.Name);
            }

            return CommandResult.Okay;
        }

        private CommandResult HandleProf(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            const int
                EquipFlag_Armor = 1 << 0,
            ...
            // Set default values
            var equipFlags = EquipFlag_All;
            var profAmount = MaxProficiency;
            ...
            if (args.Count >= 2)
            {
                ...
                foreach (var flag in args[1].ToLower().Split(new[] {'|'}, StringSplitOptions.RemoveEmptyEntries))
                    ...
                    {
                        case "all":
                            equipFlags |= EquipFlag_All;
                            break;
                        case "armor":
                            equipFlags |= EquipFlag_Armor;
                            break;
                        case "glove":
                            equipFlags |= EquipFlag_Glove;
                            break;
                        case "shoe":
                            equipFlags |= EquipFlag_Shoe;
                            break;
                        case "head":
                            equipFlags |= EquipFlag_Head;
                            break;
                        case "robe":
                            equipFlags |= EquipFlag_Robe;
                            break;
                        case "rhand1":
                            equipFlags |= EquipFlag_RHand1;
                            break;
                        case "rhand2":
                            equipFlags |= EquipFlag_RHand2;
                            break;
                        case "lhand1":
                            equipFlags |= EquipFlag_LHand1;
                            break;
                        case "lhand2":
                            equipFlags |= EquipFlag_LHand2;
                            break;
                        case "accessory1":
                            equipFlags |= EquipFlag_Accessory1;
                            break;
                        case "accessory2":
                            equipFlags |= EquipFlag_Accessory2;
                            break;
                        default:
                            Send.ServerMessage(sender,
                                Localization.Get("Unrecognised token '{0}' in 'equipflags' argument."), flag);
                            return CommandResult.InvalidArgument;
                    }
            }
            ...
            if (target != sender)
                Send.ServerMessage(target, Localization.Get("Your equipment proficiency has been set by {0}."),
                    sender.Name);

            return CommandResult.Okay;
        }

        private CommandResult HandleResetCoolDown(ChannelClient client, Creature sender, Creature target,
            string message, IList<string> args)
        {
            ...
            if (target != sender)
                Send.ServerMessage(target, Localization.Get("All your cool downs have been reset by {0}."),
                    sender.Name);

            return CommandResult.Okay;
        }

        private CommandResult HandleEquipSet(ChannelClient client, Creature sender, Creature target, string message,
            IList<string> args)
        {
            if (args.Count < 2)
            {
    ...
    public class GmCommand : Command<GmCommandFunc>
    {
        ...
        public int Auth { get; }
        public int CharAuth { get; }
    }

    public delegate CommandResult GmCommandFunc(ChannelClient client, Creature sender, Creature target, string message,
        IList<string> args);
}