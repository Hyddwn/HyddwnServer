    ...
    public class Gathering : IPreparable, ICompletable, ICancelable
    {
        ...
        public void Complete(Creature creature, Skill skill, Packet packet)
        {
            ...
            if (collectData == null)
            {
                ...
                DoComplete(creature, entityId, collectId, false, 1);
                return;
            }

            // Check tools
            if (!CheckHand(collectData.RightHand, creature.RightHand) ||
                !CheckHand(collectData.LeftHand, creature.LeftHand))
            {
            ...
            if (creature.RightHand != null && creature.RightHand.IsBreakable)
            {
                ...
                {
                    DoComplete(creature, entityId, collectId, false, 4);
                    return;
                }
            ...
            var targetEntity = GetTargetEntity(creature.Region, entityId);
            ...
            {
                DoComplete(creature, entityId, collectId, false, 3);
                return;
            }

            // Determine success
            var successChance = GetSuccessChance(creature, collectData);
            ...
            if (targetEntity is Prop)
            {
                ...
                {
                    DoComplete(creature, entityId, collectId, false, 2);
                    return;
                }

                // Regenerate resources
                targetProp.Resource += (float) ((DateTime.Now - targetProp.LastCollect).TotalMinutes *
                                                collectData.ResourceRecovering);
            ...
            if (collectSuccess)
            {
                ...
                // Product
                var itemId = receiveItemId =
                    collectData.GetRndProduct(rnd, new ProductBonus(collectionBonusProduct, collectionBonus));
                if (itemId != 0)
                {
                    ...
                    if (collectData.Source == 0)
                    {
                        ...
                        for (var i = 0; creature.Region.GetItem(a => a.GetPosition() == pos) != null && i < 10; ++i)
            ...
            // Events
            ChannelServer.Instance.Events.OnCreatureGathered(new CollectEventArgs(creature, collectData, collectSuccess,
                receiveItemId));

            // Complete
            DoComplete(creature, entityId, collectId, collectSuccess, 0);
        }
        ...
        private Entity GetTargetEntity(Region region, long entityId)
        {
            ...
            var targetEntity = isProp ? region.GetProp(entityId) : (Entity) region.GetCreature(entityId);
