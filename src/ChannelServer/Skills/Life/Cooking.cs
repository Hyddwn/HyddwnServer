...
namespace Aura.Channel.Skills.Life
{
    ...
    public class Cooking : IPreparable, ICompletable, IInitiableSkillHandler
    {
        ...
        public void Complete(Creature creature, Skill skill, Packet packet)
        {
            ...
            Item item = null;
            var success = false;
            var recipe = AuraData.CookingDb.Find(creature.Temp.CookingMethod,
                creature.Temp.CookingIngredients.Select(a => a.Item.Info.Id));
            if (recipe != null)
            {
                // Get judgement
                var judgement = JudgeQuality(recipe, creature.Temp.CookingIngredients);
                if (judgement.Quality >= SuccessMinQuality)
                {
                    var quality = (int) judgement.Quality;
                    var rating = GetRating((int) judgement.Quality);
                    ...
                    if (judgement.HelpItem != null)
                    {
                        ...
                        if (judgement.HelpAmount < 0)
                            helpmsg += Localization.Get(
                                "There may have been {1:0.0}%-{2:0.0}% less {0} than required.");
                        else
                            helpmsg += Localization.Get(
                                "There may have been {1:0.0}%-{2:0.0}% more {0} than required.");
                    ...
                    OnSuccessfulCooking(creature, skill, creature.Temp.CookingMethod, item, rating);
        ...
        public void Init()
        {
            ChannelServer.Instance.Events.PlayerUsesItem += OnPlayerUsesItem;
        }
        ...
        public bool Prepare(Creature creature, Skill skill, Packet packet)
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                if (item == null)
                {
                    Log.Warning("Cooking.Prepare: Creature '{0:X16}' tried to use non-existent item.",
                        creature.EntityId);
                    return false;
                }
            ...
            // Check tools
            if (!CheckTools(creature, method))
                return false;

            // Check rank
            if (!CheckRank(creature, method, skill.Info.Rank))
                return false;

            // Check prop
            if (!CheckProp(creature, method, propEntityId))
                return false;
            ...
            Send.SkillUse(creature, skill.Info.Id, GetTime(method));
        ...
        private bool CheckTools(Creature creature, string method)
        {
            ...
            if (right != "")
            {
                ...
                {
                    Send.Notice(creature, Localization.Get("You can't use this {0} anymore."),
                        creature.RightHand.Data.Name);
                    return false;
                }
            }
        ...
        private Judgement JudgeQuality(RecipeData recipe, List<Ingredient> ingredients)
        {
            ...
            foreach (var ingredient in ingredients)
            {
                ...
                if (ingredientData == null)
                {
                    ...
                    {
                        Log.Error(
                            "Cooking.JudgeQuality: Failed to get ingredient data for item '{0}' in recipe '{1},{2}'.",
                            ingredient.Item.Info.Id, recipe.Method, recipe.ItemId);
                        break;
                    }
                }
                ...
                var qualityAdd = ingredientData.QualityMin +
                                 rate * (ingredientData.QualityMax - ingredientData.QualityMin);
