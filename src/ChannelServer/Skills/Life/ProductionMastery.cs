...
namespace Aura.Channel.Skills.Life
{
    ...
    public class ProductionMastery : IInitiableSkillHandler, ISkillHandler
    {
        ...
        public void Init()
        {
            ...
            ChannelServer.Instance.Events.CreatureProducedItem += OnCreatureProducedItem;
            ChannelServer.Instance.Events.CreatureCreatedItem += OnCreatureCreatedItem;
            ChannelServer.Instance.Events.CreatureCookedMeal += OnCreatureCookedMeal;
        }
        ...
        private void OnCreatureProducedItem(ProductionEventArgs args)
        {
            ...
            if (skill.Info.Rank == SkillRank.R5)
            {
                if (args.Success)
                {
                    ...
                    if (args.ProductionData.Category == ProductionCategory.PotionMaking &&
                        args.ProductionData.Materials.Any(a => a.Tag.Contains("/whiteherb/")))
                        skill.Train(4); // Successfully make a potion using a White Herb.
                    else if (args.ProductionData.Category == ProductionCategory.Handicraft &&
                             args.ProductionData.Rank >= SkillRank.R7 && args.ProductionData.Rank <= SkillRank.R5)
            ...
            if (skill.Info.Rank == SkillRank.R4)
            {
                if (args.Success)
                {
                    ...
                    if (args.ProductionData.Category == ProductionCategory.PotionMaking &&
                        args.ProductionData.Materials.Any(a => a.Tag.Contains("/goldenherb/")))
                        skill.Train(4); // Successfully make a potion using a Golden Herb.
                    else if (args.ProductionData.Category == ProductionCategory.Handicraft &&
                             args.ProductionData.Rank >= SkillRank.R6 && args.ProductionData.Rank <= SkillRank.R4)
            ...
            if (skill.Info.Rank == SkillRank.R3)
            {
                if (args.Success)
                {
                    ...
                    if (args.ProductionData.Category == ProductionCategory.PotionMaking &&
                        args.ProductionData.Materials.Any(a => a.Tag.Contains("/mandrake/")))
                        skill.Train(4); // Successfully make a potion using a Mandrake.
                    else if (args.ProductionData.Category == ProductionCategory.Handicraft &&
                             args.ProductionData.Rank >= SkillRank.R5 && args.ProductionData.Rank <= SkillRank.R3)
            ...
            if (skill.Info.Rank == SkillRank.R2)
            {
                if (args.Success)
                {
                    ...
                    if (args.ProductionData.Category == ProductionCategory.PotionMaking &&
                        args.ProductionData.Materials.Any(a => a.Tag.Contains("/antidoteherb/")))
                        skill.Train(4); // Successfully make a potion using a Antidote Herb.
                    else if (args.ProductionData.Category == ProductionCategory.Handicraft &&
                             args.ProductionData.Rank >= SkillRank.R4 && args.ProductionData.Rank <= SkillRank.R2)
            ...
            if (skill.Info.Rank == SkillRank.R1)
                if (args.Success)
                {
                    ...
                    if (args.ProductionData.Category == ProductionCategory.PotionMaking &&
                        args.ProductionData.Materials.Any(a => a.Tag.Contains("/poisonherb/")))
                        skill.Train(3); // Successfully make a potion using a Poison Herb.
                    else if (args.ProductionData.Category == ProductionCategory.Handicraft &&
                             args.ProductionData.Rank >= SkillRank.R3 && args.ProductionData.Rank <= SkillRank.R1)
