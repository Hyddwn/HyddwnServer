...
namespace Aura.Channel.Skills.Life
{
    ...
    public class Blacksmithing : CreationSkill, IPreparable, ICompletable
    {
        ...
        public void Complete(Creature creature, Skill skill, Packet packet)
        {
            ...
            {
                // Materials
                if (!ReadMaterials(creature, packet, out materials))
                    goto L_Fail;
            }
            else if (stage == Stage.Finish)
            {
                // Hits
                if (!ReadHits(creature, packet, out hits))
                    goto L_Fail;
            }
            else
            {
            ...
            if (stage == Stage.Progression)
            {
                ...
                if (!GetItemsToDecrement(creature, Stage.Progression, manualData, requiredMaterials, materials,
                    out toDecrement))
                    goto L_Fail;

                // Decrement mats
                DecrementMaterialItems(creature, toDecrement, rnd);
            }
            ...
            else
            {
                ...
                if (item == null)
                {
                    Log.Warning("Blacksmithing.Complete: Creature '{0:X16}' tried to work on non-existing item.",
                        creature.EntityId);
                    goto L_Fail;
                }
                ...
                {
                    Log.Warning(
                        "Blacksmithing.Complete: Creature '{0:X16}' tried use an item with a different id than the manual.",
                        creature.EntityId);
                    goto L_Fail;
                }
                ...
                {
                    Log.Warning(
                        "Blacksmithing.Complete: Creature '{0:X16}' tried work on an item that is already finished.",
                        creature.EntityId);
                    goto L_Fail;
                }
            }
            ...
            if (progress < 1)
            {
                ...
                var chance = GetSuccessChance(creature, skill, manualData.Rank);
                ...
                OnProgress(creature, skill, item, result);

                // Event
                ChannelServer.Instance.Events.OnCreatureFinishedProductionOrCollection(creature,
                    result >= ProgressResult.Good);
            }
            else
            {
                var quality = CalculateQuality(hits, creature.Temp.BlacksmithingMiniGameDots);
                FinishItem(creature, skill, manualData, creature.Temp.CreationFinishId, item, quality);
                OnProgress(creature, skill, item, ProgressResult.Finish);

                // Creation event
                ChannelServer.Instance.Events.OnCreatureCreatedItem(new CreationEventArgs(creature,
                    CreationMethod.Blacksmithing, item, manualData.Rank));
            }
        ...
        public bool Prepare(Creature creature, Skill skill, Packet packet)
        {
            ...
            if (stage == Stage.Progression && existingItemEntityId != 0)
            {
                ...
                if (item == null)
                {
                    Log.Warning("Blacksmithing.Complete: Creature '{0:X16}' tried to work on non-existent item.",
                        creature.EntityId);
                    return false;
                }
                ...
                if (item.MetaData1.GetFloat(ProgressVar) == 1)
                {
                    ...
                    List<ProductionMaterial> toDecrement;
                    if (!GetItemsToDecrement(creature, Stage.Finish, manualData, requiredMaterials, materials,
                        out toDecrement))
                        return false;
        ...
        {
            if (creature.RightHand == null || creature.Magazine == null ||
                !creature.RightHand.HasTag("/tool/blacksmith/*/hammer/") ||
                !creature.Magazine.HasTag("/blacksmith/manual/"))
            {
                // Sanity check, client checks it as well.
                Send.Notice(creature,
                    Localization.Get("You need a Hammer in your right hand\nand a Blacksmith Manual in your left."));
                return false;
            }
        ...
        private void OnProgress(Creature creature, Skill skill, Item item, ProgressResult result)
        {
            if (skill.Info.Rank == SkillRank.RF)
            {
                if (item.HasTag("/weapon/|/tool/"))
                    switch (result)
                    {
                        case ProgressResult.VeryGood:
                            skill.Train(1);
                            break; // Achieve a great result forging a tool or a weapon.
                        case ProgressResult.Good:
                            skill.Train(2);
                            break; // Achieve a good result forging a tool or a weapon.
                        case ProgressResult.VeryBad:
                            skill.Train(3);
                            break; // Achieve a failing result forging a tool or a weapon.
                        case ProgressResult.Bad:
                            skill.Train(4);
                            break; // Achieve a bad result forging a tool or a weapon.
                        case ProgressResult.Finish:
                            skill.Train(5);
                            break; // Successfully forge a tool or a weapon.
                    }
            ...
            if (skill.Info.Rank == SkillRank.RE)
            {
                if (item.HasTag("/shield/"))
                    switch (result)
                    {
                        case ProgressResult.VeryGood:
                            skill.Train(1);
                            break; // Achieve a great result forging a shield.
                        case ProgressResult.Good:
                            skill.Train(2);
                            break; // Achieve a good result forging a shield.
                        case ProgressResult.VeryBad:
                            skill.Train(3);
                            break; // Achieve a failing result forging a shield.
                        case ProgressResult.Bad:
                            skill.Train(4);
                            break; // Achieve a bad result forging a shield.
                        case ProgressResult.Finish:
                            skill.Train(5);
                            break; // Successfully forge a shield.
                    }
            ...
            if (skill.Info.Rank == SkillRank.RD)
            {
                if (item.HasTag("/helmet/"))
                    switch (result)
                    {
                        case ProgressResult.VeryGood:
                            skill.Train(1);
                            break; // Achieve a great result forging a helmet.
                        case ProgressResult.Good:
                            skill.Train(2);
                            break; // Achieve a good result forging a helmet.
                        case ProgressResult.VeryBad:
                            skill.Train(3);
                            break; // Achieve a failing result forging a helmet.
                        case ProgressResult.Bad:
                            skill.Train(4);
                            break; // Achieve a bad result forging a helmet.
                        case ProgressResult.Finish:
                            skill.Train(5);
                            break; // Successfully forge a helmet.
                    }
            ...
            if (skill.Info.Rank == SkillRank.RC)
            {
                if (item.HasTag("/gauntlet/"))
                    switch (result)
                    {
                        case ProgressResult.VeryGood:
                            skill.Train(1);
                            break; // Achieve a great result forging gauntlets.
                        case ProgressResult.Good:
                            skill.Train(2);
                            break; // Achieve a good result forging gauntlets.
                        case ProgressResult.VeryBad:
                            skill.Train(3);
                            break; // Achieve a failing result forging gauntlets.
                        case ProgressResult.Bad:
                            skill.Train(4);
                            break; // Achieve a bad result forging gauntlets.
                        case ProgressResult.Finish:
                            skill.Train(5);
                            break; // Successfully forge gauntlets.
                    }
            ...
            if (skill.Info.Rank == SkillRank.RB)
            {
                if (item.HasTag("/armorboots/"))
                    switch (result)
                    {
                        case ProgressResult.VeryGood:
                            skill.Train(1);
                            break; // Achieve a great result forging greaves.
                        case ProgressResult.Good:
                            skill.Train(2);
                            break; // Achieve a good result forging greaves.
                        case ProgressResult.VeryBad:
                            skill.Train(3);
                            break; // Achieve a failing result forging greaves.
                        case ProgressResult.Bad:
                            skill.Train(4);
                            break; // Achieve a bad result forging greaves.
                        case ProgressResult.Finish:
                            skill.Train(5);
                            break; // Successfully forge greaves.
                    }
            ...
            if (skill.Info.Rank == SkillRank.RA)
            {
                if (item.HasTag("/armor/"))
                    switch (result)
                    {
                        case ProgressResult.VeryGood:
                            skill.Train(1);
                            break; // Greatly successful in making Armor
                        case ProgressResult.Good:
                            skill.Train(2);
                            break; // Successful in making Armors
                        case ProgressResult.VeryBad:
                            skill.Train(3);
                            break; // If unsuccessful in making Armor
                        case ProgressResult.Bad:
                            skill.Train(4);
                            break; // The result of making Armor is very poor.
                        case ProgressResult.Finish:
                            skill.Train(5);
                            break; // Completed making Armor 100%.
                    }
            ...
            if (skill.Info.Rank == SkillRank.R9)
            {
                if (item.HasTag("/armor/"))
                    ...
                    switch (result)
                    {
                        case ProgressResult.Good:
                            skill.Train(4);
                            break; // Successful in making a tool or a weapon.
                        case ProgressResult.Finish:
                            skill.Train(5);
                            break; // Completed making tool or a weapon 100%.
                    }
            ...
            if (skill.Info.Rank == SkillRank.R8)
            {
                if (item.HasTag("/armor/"))
                    ...
                    switch (result)
                    {
                        case ProgressResult.Good:
                            skill.Train(4);
                            break; // Successful in making a Shield.
                        case ProgressResult.Finish:
                            skill.Train(5);
                            break; // Completed making a Shield 100%.
                    }
            ...
            if (skill.Info.Rank == SkillRank.R7)
            {
                if (item.HasTag("/armor/"))
                    switch (result)
                    {
                        case ProgressResult.VeryGood:
                            skill.Train(1);
                            break; // Achieve a great result forging armor.
                        case ProgressResult.Good:
                            skill.Train(2);
                            break; // Achieve a good result forging armor.
                        case ProgressResult.Finish:
                            skill.Train(3);
                            break; // Successfully forge armor.
                    }
