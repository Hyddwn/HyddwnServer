...
namespace Aura.Channel.Skills.Life
{
    ...
    public class Tailoring : CreationSkill, IPreparable, ICompletable
    {
        ...
        public void Complete(Creature creature, Skill skill, Packet packet)
        {
            ...
            {
                // Stitches
                ReadStitches(creature, packet, out stitches);
            }
            else
            {
            ...
            if (existingItemEntityId != 0)
            {
                ...
                {
                    Log.Warning("Tailoring.Complete: Creature '{0:X16}' tried to work on non-existing item.",
                        creature.EntityId);
                    goto L_Fail;
                }
                ...
                {
                    Log.Warning(
                        "Tailoring.Complete: Creature '{0:X16}' tried use an item with a different id than the manual.",
                        creature.EntityId);
                    goto L_Fail;
                }
                ...
                {
                    Log.Warning(
                        "Tailoring.Complete: Creature '{0:X16}' tried work on an item that is already finished.",
                        creature.EntityId);
                    goto L_Fail;
                }
            }
            ...
            if (progress < 1)
            {
                ...
                OnProgress(creature, skill, result);
            ...
            {
                var quality = CalculateQuality(stitches, creature.Temp.TailoringMiniGameX,
                    creature.Temp.TailoringMiniGameY);
                FinishItem(creature, skill, manualData, 0, existingItem, quality);
                OnProgress(creature, skill, ProgressResult.Finish);

                // Creation event
                ChannelServer.Instance.Events.OnCreatureCreatedItem(new CreationEventArgs(creature,
                    CreationMethod.Tailoring, existingItem, manualData.Rank));
        ...
        public bool Prepare(Creature creature, Skill skill, Packet packet)
        {
            ...
            if (stage == Stage.Progression && existingItemEntityId != 0)
            {
                ...
                if (item == null)
                {
                    Log.Warning("Tailoring.Complete: Creature '{0:X16}' tried to work on non-existent item.",
                        creature.EntityId);
                    return false;
                }
        ...
        private bool CheckTools(Creature creature)
        {
            ...
            if (
                ...
                creature.Magazine == null || !creature.Magazine.HasTag("/tailor/manual/") ||
                !creature.Magazine.MetaData1.Has("FORMID")
            )
            {
                Send.MsgBox(creature,
                    Localization.Get(
            ...
            {
                Send.MsgBox(creature,
                    Localization.Get("You can't use this Sewing Pattern anymore. You'll stab your fingers!"));
                return false;
            }
            ...
            if (skill.Info.Rank >= SkillRank.RF && skill.Info.Rank <= SkillRank.R3)
            {
                ...
                switch (result)
                {
                    case ProgressResult.VeryGood:
                        skill.Train(2);
                        break; // Achieve a very good result.
                    case ProgressResult.Bad:
                        skill.Train(3);
                        break; // The result is a failure.
                    case ProgressResult.VeryBad:
                        skill.Train(4);
                        break; // The result is very bad.
                    case ProgressResult.Finish:
                        skill.Train(5);
                        break; // Clothes are finished.
                }
