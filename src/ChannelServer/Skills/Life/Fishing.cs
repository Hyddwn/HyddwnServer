...
namespace Aura.Channel.Skills.Life
{
    ...
    public class Fishing : ISkillHandler, IPreparable, IReadyable, IUseable, ICancelable
    {
        ...
        public bool Prepare(Creature creature, Skill skill, Packet packet)
        {
            ...
            // Check rod and bait
            if (!CheckEquipment(creature))
            {
                Send.MsgBox(creature,
                    Localization.Get("You need a Fishing Rod in your right hand\nand a Bait Tin in your left."));
                return false;
            }
        ...
        public void OnResponse(Creature creature, FishingMethod method, bool success)
        {
            ...
            if (!success)
            {
                Send.Notice(creature,
                    Localization.Get("I was hesistating for a bit, and it got away...")); // More responses?
            ...
            else
            {
                ...
                if (!cancel)
                    Send.Effect(creature, Effect.Fishing, (byte) FishingEffectType.ReelIn, true,
                        creature.Temp.FishingProp.EntityId, item.Info.Id, size, propName, propSize);
            }

            creature.Temp.FishingDrop = null;

            // Handle training
            Training(creature, skill, success, item);
        ...
        public async void StartFishing(Creature creature, int delay)
        {
            ...
            // Get fishing drop 
            creature.Temp.FishingDrop = GetFishingDrop(creature, rnd);
            if (creature.Temp.FishingDrop == null)
            {
        ...
        private DropData GetFishingDrop(Creature creature, Random rnd)
        {
            ...
            foreach (var fishingGround in grounds.OrderByDescending(a => a.Priority))
            {
                ...
                // Check equip
                if (fishingGround.Rod != 0 && fishingGround.Rod != rod ||
                    fishingGround.Bait != 0 && fishingGround.Bait != bait)
                    continue;
        ...
        {
            return creature.RightHand != null && creature.RightHand.HasTag("/fishingrod/") &&
                   creature.Magazine != null && creature.Magazine.HasTag("/fishing/bait/");
        }
