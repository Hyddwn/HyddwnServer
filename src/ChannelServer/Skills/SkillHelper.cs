...
namespace Aura.Channel.Skills
{
    public static class SkillHelper
    {
        ...
        public static void HandleDefenseProtection(Creature target, ref float damage, bool defense = true,
        ...
        /// <param name="defense"></param>
        /// <param name="protection"></param>
        public static void HandleMagicDefenseProtection(Creature target, ref float damage, bool defense = true,
            bool protection = true)
        {
            if (defense)
        ...
        public static void UpdateWeapon(Creature attacker, Creature target, ProficiencyGainType profGainType,
            params Item[] weapons)
        {
            if (attacker == null)
                return;
        ...
        /// <returns></returns>
        public static List<Creature> GetTargetableCreaturesInSkillArea(Creature attacker, int skillLength,
            int skillWidth)
        {
            Position discard;
        ...
        /// <returns></returns>
        public static List<Creature> GetTargetableCreaturesInSkillArea(Creature attacker, int skillLength,
            int skillWidth, out Position endPosition)
        {
            if (attacker == null)
            {
            ...
            var radian = MabiMath.ByteToRadian(attacker.Direction);
            var creatures =
                attacker.Region.GetCreaturesInRectangle(attackerPos, radian, skillLength, skillWidth, out endPosition);
            var targetableCreatures = creatures.Where(x =>
                attacker.CanTarget(x) && !attacker.Region.Collisions.Any(attackerPos, x.GetPosition()));
