...
namespace Aura.Channel.Skills.Base
{
    ...
    public abstract class CreationSkill
    {
        ...
        /// </summary>
        protected static readonly int[] SuccessTable =
        {
            96, 93, 91, 88, 87, 85, 84, 83, 81, 79, 77, 74, 72, 71, 70, 54, 39, 27, 19, 12, 7, 5, 3, 1, 1, 1, 0, 0, 0, 0
        };
        ...
        protected int GetSuccessChance(Creature creature, Skill skill, SkillRank manualRank)
        {
            ...
            if (creature.IsInParty)
            {
                ...
                var tailorsCount = members.Where(a => a != creature && a.Skills.Has(skill.Info.Id, SkillRank.RF))
                    .Count();
                if (tailorsCount != 0)
        ...
        protected void FinishItem(Creature creature, Skill skill, ManualData manualData, int finishId, Item item,
            int quality)
        {
            ...
            if (quality == 100) // god
                ...
                msg = Localization.Get(
                    "Your {0} isn't really fit for use. Maybe you could get some decent scrap wood out of it.");

            // Apply bonuses and append msgs
            var bonuses = GetBonusesFor(item, quality);
            if (bonuses.Count != 0)
            {
                msg += "\n";
        ...
        /// <returns></returns>
        protected bool GetItemsToDecrement(Creature creature, Stage stage, ManualData manualData,
            List<ProductionMaterialData> requiredMaterials, List<ProductionMaterial> materials,
            out List<ProductionMaterial> toDecrement)
        {
                ...
                if (material.Item.HasTag(reqMat.Tag))
                {
                    ...
                    if (inUse.Contains(material.Item.EntityId))
                    {
                        Send.ServerMessage(creature,
                            Localization.Get(
                                "Unable to handle request, please report, with this information: ({0}/{1}:{2})."),
                            material.Item.Info.Id, manualData.Category, manualData.Id);
                        Log.Warning(
                            "CreationSkill.GetItemsToDecrement: Item '{0}' matches multiple materials for manual '{1}:{2}'.",
                            material.Item.Info.Id, manualData.Category, manualData.Id);
                        return false;
                    }
        ...
        protected bool ReadMaterials(Creature creature, Packet packet, out List<ProductionMaterial> materials)
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                {
                    Log.Warning(
                        "CreationSkill.ReadMaterials: Creature '{0:X16}' tried to use non-existent material item.",
                        creature.EntityId);
                    return false;
                }
