...
namespace Aura.Channel.Skills.Base
{
    ...
    public abstract class ProductionSkill : IPreparable, ICompletable
    {
        ...
        /// </summary>
        protected virtual int Proficiency => 30;
        ...
        public void Complete(Creature creature, Skill skill, Packet packet)
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                {
                    Log.Warning(
                        "ProductionSkill.Prepare: Creature '{0:X16}' tried to use non-existent item as material.",
                        creature.EntityId);
                    return;
                }
            ...
            // Check prop
            if (!CheckProp(creature, propEntityId))
                goto L_Fail;

            // Check category
            if (!CheckCategory(creature, category))
            {
                Log.Warning(
                    "ProductionSkill.Complete: Creature '{0:X16}' tried to use category '{1}' with skill '{2}'.",
                    creature.EntityId, category, GetType().Name);
                goto L_Fail;
            }
            ...
            // Check tools
            if (!CheckTools(creature, skill, productData))
                goto L_Fail;

            // Check mana
            if (!CheckMana(creature, productData))
                goto L_Fail;
            ...
            foreach (var material in materials)
            {
                ...
                var match =
                    ...
                    material.Item.IsGatheringPouch && material.Item.Data.StackItem != null &&
                    material.Item.Data.StackItem.HasTag(reqMat.Tag);
                ...
                if (match)
                {
                    ...
                    if (inUse.Contains(material.Item.EntityId))
                    {
                        Send.ServerMessage(creature,
                            ...
                            material.Item.Info.Id, productData.Id);
                        Log.Warning(
                            "ProductionSkill.Complete: Item '{0}' matches multiple materials for product '{1}'.",
                            material.Item.Info.Id, productData.Id);
                        goto L_Fail;
                    }
            ...
            for (var i = 0; i < potentialProducts.Length; ++i)
            {
                var multiplier = 1f;
            ...
            if (potentialProducts.Length > 1)
            {
                ...
                if (itemId == 0)
                {
                    Log.Error("ProductionSkill.Complete: Failed to select random product item for {0}/{1}, num: {2}.",
                        category, productId, num);
            ...
            UpdateTool(creature, productData);

            // Skill training
            SkillTraining(creature, skill, productData, success);
            ...
            if (success)
            {
                ...
                ChannelServer.Instance.Events.OnCreatureProducedItem(new ProductionEventArgs(creature, productData,
                    true, productItem));
            ...
            // Material creation event
            ChannelServer.Instance.Events.OnCreatureProducedItem(new ProductionEventArgs(creature, productData, false,
                null));
        ...
        protected virtual bool CheckTools(Creature creature, Skill skill, ProductionData productData)
        {
            ...
            if (productData.Tool == "/barehand/")
            {
                ...
                {
                    Log.Warning("ProductionSkill.Complete: User '{0}' tried to produce without empty hands.",
            ...
            else
            {
                ...
                {
                    Log.Warning(
                        "ProductionSkill.Complete: Creature '{0:X16}' tried to produce without the appropriate tool.",
                        creature.EntityId);
                    return false;
                }
            }
