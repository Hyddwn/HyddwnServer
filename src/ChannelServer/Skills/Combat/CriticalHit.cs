...
namespace Aura.Channel.Skills.Combat
{
    ...
    public class CriticalHit : ISkillHandler, IInitiableSkillHandler
    {
        ...
        private void OnCreatureAttack(TargetAction tAction)
        {
            ...
            if (targetSkill != null)
                if (targetSkill.Info.Rank == SkillRank.Novice)
                    targetSkill.Train(1); // Learn from enemy's critical hit.
                else if (targetSkill.Info.Rank >= SkillRank.RF)
        ...
        /// <param name="tAction"></param>
        public static void Handle(Creature attacker, float critChance, ref float damage, TargetAction tAction,
            bool bypassNoviceCheck = false)
        {
