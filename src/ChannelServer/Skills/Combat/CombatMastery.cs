...
namespace Aura.Channel.Skills.Combat
{
    ...
    public class CombatMastery : ICombatSkill, IInitiableSkillHandler
    {
        ...
        public CombatSkillResult Use(Creature attacker, Skill skill, long targetEntityId)
        {
            ...
            targets.UnionWith(attacker.GetTargetableCreaturesInCone(mainTarget.GetPosition(),
                attacker.GetTotalSplashRadius(), attacker.GetTotalSplashAngle()));
            ...
            for (byte i = 1; i <= maxHits; ++i)
            {
                ...
                cap.Hit = i;
                cap.Type = attacker.IsDualWielding
                    ? CombatActionPackType.TwinSwordAttack
                    : CombatActionPackType.NormalAttack;
                cap.PrevId = prevId;
                prevId = cap.Id;
                ...
                foreach (var target in targets)
                {
                    ...
                    // Critical Hit
                    var critChance = i == 1
                        ? attacker.GetRightCritChance(target.Protection)
                        : attacker.GetLeftCritChance(target.Protection);
                    ...
                    if (!target.IsDead)
                    {
                        ...
                        {
                            target.Stability -= GetStabilityReduction(attacker, weapon) / maxHits;
                            ...
                            if (skill.Info.Id != SkillId.FinalHit)
                            {
                                ...
                                if (target.IsUnstable && target.Is(RaceStands.KnockBackable))
                                    ...
                                    tAction.Set(TargetOptions.KnockDown);
                                     ...
                                     target.Is(RaceStands.KnockBackable))
                            {
                    ...
                    {
                        if (target == mainTarget)
                            aAction.Stun = GetAttackerStun(attacker, weapon,
                                tAction.IsKnockBack && skill.Info.Id != SkillId.FinalHit);
                        tAction.Stun = GetTargetStun(attacker, weapon, tAction.IsKnockBack);
                    }
        ...
        public static short GetAttackerStun(Creature creature, Item weapon, bool knockback)
        {
            ...
            var speed = weapon != null ? weapon.Data.AttackSpeed : (AttackSpeed) creature.RaceData.AttackSpeed;
