...
namespace Aura.Channel.Skills.Music
{
    ...
    public class Composing : IPreparable, ICompletable, ICancelable
    {
        ...
        public bool Prepare(Creature creature, Skill skill, Packet packet)
        {
            ...
            if (mmlParts.Length > 0 && !string.IsNullOrWhiteSpace(mmlParts[0]) &&
                mmlParts[0].Length > skill.RankData.Var1) goto L_Fail; // Melody
            if (mmlParts.Length > 1 && !string.IsNullOrWhiteSpace(mmlParts[1]) &&
                mmlParts[1].Length > skill.RankData.Var2) goto L_Fail; // Harmony 1
            if (mmlParts.Length > 2 && !string.IsNullOrWhiteSpace(mmlParts[2]) &&
                mmlParts[2].Length > skill.RankData.Var3) goto L_Fail; // Harmony 2
