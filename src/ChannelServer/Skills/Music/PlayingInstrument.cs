...
namespace Aura.Channel.Skills.Music
{
        ...
        ICustomPrepareUsageSkill
    {
        ...
        /// </summary>
        protected virtual string MetaDataScoreField => "SCORE";
        ...
        {
            if (GetScore(creature) == null)
                creature.Regens.Add("PlayingInstrument", Stat.Stamina, skill.RankData.StaminaActive,
                    creature.StaminaMax);
        }
        ...
        public bool Prepare(Creature creature, Skill skill, Packet packet)
        {
            ...
            if (ChannelServer.Instance.Conf.World.PerfectPlay)
            {
                ...
                Send.ServerMessage(creature,
                    Localization.Get("Perfect play is enabled, your performance will sound perfect."));
            }
            ...
            StartPlay(creature, skill, instrumentType, effectQuality, mml, rndScore);
            OnPlay(creature, skill, quality);
            ...
            creature.Skills.Callback(skill.Info.Id, () =>
            {
                Send.Notice(creature, GetRandomQualityMessage(quality));
                AfterPlay(creature, skill, quality);
            });
        ...
        private void StartPlay(Creature creature, Skill skill, InstrumentType instrumentType, int quality,
            string compressedMml, int scoreId)
        {
        ...
        private string GetScore(Creature creature)
        {
            ...
            if (creature.Magazine == null || !creature.Magazine.MetaData1.Has(MetaDataScoreField) ||
                creature.Magazine.OptionInfo.Durability < DurabilityUse)
                return null;

            return creature.Magazine.MetaData1.GetString(MetaDataScoreField);
        }
        ...
        {
            string[] msgs = null;
            switch (GetQualityRating(quality))
            {
        ...
        {
            var playingQuality = GetQualityRating(quality);
            ...
            if (skill.Info.Rank >= SkillRank.RF && skill.Info.Rank <= SkillRank.R6 &&
                playingQuality == PlayingQuality.Bad)
            ...
            if (skill.Info.Rank >= SkillRank.RF && skill.Info.Rank <= SkillRank.RE &&
                playingQuality == PlayingQuality.VeryBad)
        ...
        protected virtual void OnCreatureAttackedByPlayer(TargetAction action)
        {
            ...
            if (action.Attacker == null || action.Attacker.RightHand == null ||
                !action.Attacker.RightHand.HasTag("/instrument/"))
                return;
