...
namespace Aura.Channel.Skills.Magic
{
    ...
    public class Fireball : IPreparable, IReadyable, IUseable, ICompletable, ICancelable, ICustomHitCanceler
    {
        ...
        public void Use(Creature attacker, Skill skill, Packet packet)
        {
            ...
            Use(attacker, skill, targetEntityId, unkInt1, unkInt2);
        }
        ...
        public void Use(Creature attacker, Skill skill, long targetEntityId, int unkIn1, int unkInt2)
        {
            if (skill.Stacks < skill.RankData.StackMax)
            {
                ...
                Log.Warning("Fireball.Use: User '{0}' tried to fire a not fully charged Fireball.",
                    attacker.Client.Account.Id);
                return;
            }
            ...
            if (target == null)
            {
                Log.Warning("Fireball.Use: Creature '0x{0}' tried to use Fireball on invalid target.",
                    attacker.EntityId);
            ...
            Send.Effect(fireballProp, Effect.FireballFly, 0, regionId, fromX, fromY, toX, toY, time, (byte) 0,
                (ushort) skill.Info.Id);
            ...
            Task.Delay(time).ContinueWith(_ => Impact(attacker, skill, fireballProp));
        }
        ...
        private void Impact(Creature attacker, Skill skill, Prop fireballProp)
        {
            ...
            var aAction = new AttackerAction(CombatActionType.SpecialHit, attacker, targetLocation.ToLocationId(),
                skill.Info.Id);
            ...
            foreach (var target in targets)
            {
                ...
                // Elements
                damage *= GetElementalDamageMultiplier(attacker, target);
