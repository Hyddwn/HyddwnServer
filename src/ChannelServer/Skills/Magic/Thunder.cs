    ...
    public class Thunder : IPreparable, IReadyable, ICombatSkill, ICompletable, ICancelable, ICustomHitCanceler
    {
        ...
        public CombatSkillResult Use(Creature attacker, Skill skill, long targetEntityId)
        {
            ...
            var potentialTargets =
                attacker.GetTargetableCreaturesAround(targetPos, AreaOfEffect, TargetableOptions.IgnoreWalls);
            ...
            // Bolts
            Bolts(attacker, skill, stacks, mainTarget, targets, thunderboltDelay);

            // Schedule callback for the Thunderbolt
            Task.Run(() => Thunderbolts(attacker, skill, stacks, damageProp, thunderboltDelay, targets));
        ...
        public bool Ready(Creature creature, Skill skill, Packet packet)
        {
            ...
            {
                var addStacks = GetStacks(creature, skill);
        ...
        /// </summary>
        private void Bolts(Creature attacker, Skill skill, int stacks, Creature mainTarget, List<Creature> targets,
            int thunderboltDelay)
        {
            // Combat action
            ...
            foreach (var target in targets)
            {
                ...
                var damage = GetBoltDamage(attacker, skill, stacks);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (delayReduction > 0)
                            stabilityReduction = (short) Math.Max(0,
                                stabilityReduction - stabilityReduction / 100 * delayReduction);
        ...
        private async void Thunderbolts(Creature attacker, Skill skill, int stacks, Prop damageProp,
            int thunderboltDelay, List<Creature> targets)
        {
            ...
            {
                Thunderbolt(attacker, skill, stacks, damageProp, targets);
                await Task.Delay(500);
            }
        ...
        private void Thunderbolt(Creature attacker, Skill skill, int stacks, Prop damageProp, List<Creature> targets)
        {
            ...
            foreach (var target in targets.Where(a => !a.IsDead))
            {
                ...
                var damage = GetThunderboltDamage(attacker, skill, stacks);
