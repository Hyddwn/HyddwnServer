...
namespace Aura.Channel.Skills.Magic
{
    ...
    public class Enchant : IPreparable, ICompletable, ICancelable
    {
        ...
        private static readonly float[] _baseChanceB00 = {69, 65, 60, 55, 51, 46, 32, 30, 27, 25, 20, 14, 10, 6, 4};
        private static readonly float[] _baseChanceB05 = {73, 68, 63, 58, 53, 48, 34, 32, 29, 26, 21, 15, 10, 6, 4};
        private static readonly float[] _baseChanceB10 = {77, 71, 66, 61, 56, 51, 35, 33, 30, 27, 22, 16, 11, 7, 5};
        private static readonly float[] _baseChanceB50 = {90, 90, 90, 85, 78, 71, 50, 47, 42, 38, 31, 22, 15, 10, 7};
        private static readonly float[] _baseChanceB60 = {90, 90, 90, 90, 84, 76, 53, 50, 45, 41, 33, 24, 16, 10, 7};
        ...
        public void Complete(Creature creature, Skill skill, Packet packet)
        {
            ...
            // Get option set id
            optionSetId = GetOptionSetid(enchant);
            ...
            {
                Log.Warning("Enchant.Complete: Creature '{0:X16}' tried to use set '{1}' on invalid item '{2}'.",
                    creature.EntityId, optionSetData.Id, item.Info.Id);
                goto L_End;
            }
            ...
            else
            {
                ...
                var durabilityLoss = GetDurabilityLoss(rnd, optionSetData.Rank, result);
                if (durabilityLoss == -1)
        ...
        public bool Prepare(Creature creature, Skill skill, Packet packet)
        {
            ...
            // Check ranks
            var optionSetId = GetOptionSetid(enchant);
            var optionSetData = AuraData.OptionSetDb.Find(optionSetId);
            if (optionSetData == null)
            {
                Log.Warning("Enchant.Prepare: Creature '{0:X16}' tried to enchant with unknown option set '{1}'.",
                    creature.EntityId, optionSetId);
                return false;
            }
            ...
            if (!optionSetData.IgnoreRank && !optionSetData.AlwaysSuccess)
                if (optionSetData.Rank >= SkillRank.R5 && skill.Info.Rank < SkillRank.R5)
                {
                    Send.Notice(creature,
                        Localization.Get("Your Enchant skill must be Rank 5 or above to use this Enchant Scroll."));
                    return false;
                }
            ...
            if (!optionSetData.IgnoreRank)
                ...
                {
                    var checkSetId = optionSetData.Type == UpgradeType.Prefix
                        ? item.OptionInfo.Prefix
                        : item.OptionInfo.Suffix;
                    var checkSetData = AuraData.OptionSetDb.Find(checkSetId);
                    if (checkSetData == null || checkSetData.Rank + 1 < optionSetData.Rank)
                    {
                        // Unofficial
                        Send.Notice(creature,
                            Localization.Get("You need to enchant Enchantments of R9 and above in sequence."));
                        return false;
                    }
                }
            ...
            if (enchant.MetaData1.Has(checkExpiration))
            {
                ...
                if (DateTime.Now > expiration)
                {
                    Send.MsgBox(creature,
                        Localization.Get("No enchantment options available.\nThis scroll is expired."));
                    return false;
                }
            }
        ...
        public bool Burn(Creature creature, Item item, Prop campfire, bool enchantersBurn)
        {
            ...
            if (enchantersBurn)
            {
                ...
                var hasEnchantBurnItems =
                    creature.Inventory.Has(51102) && creature.Inventory.Has(63016); // Mana Herb + Holy Water
                ...
                // Debug
                if (creature.Titles.SelectedTitle == TitleId.devCAT)
                    Send.ServerMessage(creature, "Debug: Chance for enchant: {0:0}, chance for powder: {1:0}",
                        enchantChance, powderChance);
                ...
                // Training
                BurnTraining(skill, enchantBurnSuccess, powderBurnSuccess);
        ...
        private int GetDurabilityLoss(Random rnd, SkillRank enchantRank, EnchantResult result)
        {
            ...
            switch (enchantRank)
            {
                ...
                case SkillRank.RF:
                    points = result == EnchantResult.Fail ? rnd.Next(0, 1) : rnd.Next(0, 2);
                    break;
                case SkillRank.RE:
                    points = result == EnchantResult.Fail ? rnd.Next(0, 2) : rnd.Next(0, 4);
                    break;
                case SkillRank.RD:
                    points = result == EnchantResult.Fail ? rnd.Next(0, 2) : rnd.Next(0, 4);
                    break;
                case SkillRank.RC:
                    points = result == EnchantResult.Fail ? rnd.Next(0, 3) : rnd.Next(0, 6);
                    break;
                case SkillRank.RB:
                    points = result == EnchantResult.Fail ? rnd.Next(0, 3) : rnd.Next(0, 7);
                    break;
                case SkillRank.RA:
                    points = result == EnchantResult.Fail ? rnd.Next(0, 4) : rnd.Next(0, 8);
                    break;
                case SkillRank.R9:
                    points = result == EnchantResult.Fail ? rnd.Next(1, 6) : rnd.Next(1, 10);
                    break;
                case SkillRank.R8:
                    points = result == EnchantResult.Fail ? rnd.Next(2, 7) : rnd.Next(2, 12);
                    break;
                case SkillRank.R7:
                    points = result == EnchantResult.Fail ? rnd.Next(2, 8) : rnd.Next(2, 14);
                    break;

                // Custom durability loss for R6+, for safe enchanting option.
                case SkillRank.R6:
                    points = result == EnchantResult.Fail ? rnd.Next(3, 9) : rnd.Next(3, 16);
                    break;
                case SkillRank.R5:
                    points = result == EnchantResult.Fail ? rnd.Next(3, 9) : rnd.Next(3, 16);
                    break;
                case SkillRank.R4:
                    points = result == EnchantResult.Fail ? rnd.Next(4, 10) : rnd.Next(4, 18);
                    break;
                case SkillRank.R3:
                    points = result == EnchantResult.Fail ? rnd.Next(4, 10) : rnd.Next(4, 18);
                    break;
                case SkillRank.R2:
                    points = result == EnchantResult.Fail ? rnd.Next(5, 11) : rnd.Next(5, 20);
                    break;
                case SkillRank.R1:
                    points = result == EnchantResult.Fail ? rnd.Next(5, 11) : rnd.Next(5, 20);
                    break;
