...
namespace Aura.Channel.Skills
{
    ...
    public class CombatActionPack
    {
        ...
        {
            Id = Interlocked.Increment(ref _actionId);
            Hit = 1;
            Type = CombatActionPackType.NormalAttack;
        ...
        {
            return Actions.Where(a => a.Category == CombatActionCategory.Target).Select(a => a.Creature).ToArray();
        }
        ...
        public void Handle()
        {
            foreach (var action in Actions)
            {
                ...
                if (action.Category == CombatActionCategory.Target)
                {
                    ...
                    if (action.Creature.IsPlayer)
                    {
                        ...
                        if (tAction.IsKnockBack && ChannelServer.Instance.Conf.World.BrokenEggs)
                        {
                            ...
                            var eggs = action.Creature.Inventory.GetItems(a =>
                                a.HasTag("/usable/food/cooking/solid/*egg/") && a.Info.Pocket != Pocket.VIPInventory);
                    ...
                    {
                        var custom =
                            ChannelServer.Instance.SkillManager.GetHandler(activeSkill.Info.Id) as ICustomHitCanceler;
                    ...
                    tAction.Creature.WasKnockedBack = tAction.Has(TargetOptions.KnockBack) ||
                                                      tAction.Has(TargetOptions.KnockDown) ||
                                                      tAction.Has(TargetOptions.Smash);
                ...
                else if (action.Category == CombatActionCategory.Attack)
                {
                    ...
                    if (action.Creature.IsPlayer && action.SkillId >= SkillId.RangedAttack &&
                        action.SkillId <= SkillId.UrgentShot && !AuraData.FeaturesDb.IsEnabled("CombatSystemRenewal"))
                        action.Stun = 0;
            ...
            // Skill used
            if (SkillId != SkillId.CombatMastery)
                Send.CombatUsedSkill(Attacker, SkillId);

            // End combat action
            Send.CombatActionEnd(Attacker, Id);
        }
    }
    ...
    public abstract class CombatAction
    {
        ...
        {
            return flags == Flags || flags != CombatActionType.None && (Flags & flags) != 0;
        }
    }
    ...
    public class AttackerAction : CombatAction
    {
        ...
        public AttackerAction(CombatActionType type, Creature creature, long targetId)
        {
            ...
            Creature = creature;
            TargetId = targetId;
            WeaponParameterType = 1;

            var active = creature.Skills.ActiveSkill;
            SkillId = active == null ? SkillId.CombatMastery : active.Info.Id;
        }
        ...
        /// </summary>
        public override bool IsKnockBack => Has(AttackerOptions.KnockBackHit2) || Has(AttackerOptions.KnockBackHit1);

        /// <summary>
        ///     Attack or Target action.
        /// </summary>
        public override CombatActionCategory Category => CombatActionCategory.Attack;
    ...
    public class TargetAction : CombatAction
    {
        ...
        public TargetAction(CombatActionType type, Creature creature, Creature attacker, SkillId attackerSkillId)
        {
            ...
            Creature = creature;
            Attacker = attacker;
            AttackerSkillId = attackerSkillId;
        ...
        /// </summary>
        public override bool IsKnockBack => Has(TargetOptions.Downed);

        /// <summary>
        ///     Attack or Target action.
        /// </summary>
        public override CombatActionCategory Category => CombatActionCategory.Target;
