...
namespace Aura.Channel.Skills.Hidden
{
    ...
    public class HiddenResurrection : IPreparable, IUseable, ICompletable, ICancelable
    {
        ...
        {
            var target = GetTarget(packet);
            var item = creature.Temp.SkillItem1;

            // Check target validity
            if (target == null || !target.IsDead || !target.DeadMenu.Has(ReviveOptions.PhoenixFeather) ||
                creature.RegionId != target.RegionId && !item.HasTag("/remote/"))
            {
                // Unofficial
            ...
            else
            {
                ...
                {
                    var targets = creature.Party.GetMembers().Where(a =>
                        a != creature && a != target && a.IsDead && a.DeadMenu.Has(ReviveOptions.PhoenixFeather));
        ...
        public bool Prepare(Creature creature, Skill skill, Packet packet)
        {
            ...
            if (item.HasTag("/party/") && !creature.Party.GetMembers()
                    .Any(a => a != creature && a.IsDead && a.DeadMenu.Has(ReviveOptions.PhoenixFeather)))
            {
