...
namespace Aura.Channel.Scripting
{
    ...
    {
        private readonly List<MagicWordsFunction> _calls;
        ...
        public void LoadCode(string script)
        {
            ...
            // TODO: Parse properly
            foreach (Match call in Regex.Matches(script,
                @"(?<function>\w+)\s*\(\s*(?<arg1>[^,\)]+)?\s*(,\s*(?<arg2>[^,\)]+)\s*)?(,\s*(?<arg3>[^,\)]+)\s*)?(,\s*(?<arg4>[^,\)]+)\s*)?\)",
                RegexOptions.Compiled))
            {
                var function = new MagicWordsFunction();
                function.Name = call.Groups["function"].Value;

                if (!string.IsNullOrWhiteSpace(call.Groups["arg1"].Value))
                    function.Arguments.Add(call.Groups["arg1"].Value);
                if (!string.IsNullOrWhiteSpace(call.Groups["arg2"].Value))
                    function.Arguments.Add(call.Groups["arg2"].Value);
                if (!string.IsNullOrWhiteSpace(call.Groups["arg3"].Value))
                    function.Arguments.Add(call.Groups["arg3"].Value);
                if (!string.IsNullOrWhiteSpace(call.Groups["arg4"].Value))
                    function.Arguments.Add(call.Groups["arg4"].Value);
            ...
            foreach (var function in _calls)
                switch (function.Name)
                {
                    ...
                    case "modify":
                    {
                        ...
                        {
                            case "str":
                                creature.StrBonus += modifier;
                                break;
                            case "int":
                                creature.IntBonus += modifier;
                                break;
                            case "dex":
                                creature.DexBonus += modifier;
                                break;
                            case "will":
                                creature.WillBonus += modifier;
                                break;
                            case "luck":
                                creature.LuckBonus += modifier;
                                break;
                            default:
                                Log.Warning("MagicWordsScript: Unknown modify stat '{0}'.", stat);
                                continue;
                        }
        ...
        private class MagicWordsFunction
        {
            ...
            public T GetArgument<T>(int index)
            {
                if (index < 0 || index > Arguments.Count - 1)
                    throw new ArgumentException("MagicWordsFunction.GetArgument: Missing argument '" + index +
                                                "' for '" + Name + "'.");

                var type = typeof(T);
                var result = (T) Convert.ChangeType(Arguments[index], type);
                if (result == null)
                    throw new ArgumentException("MagicWordsFunction.GetArgument: Invalid argument '" +
                                                Arguments[index] + "' for '" + Name + "', expected " + type.Name + ".");
