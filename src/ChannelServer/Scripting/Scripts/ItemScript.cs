...
namespace Aura.Channel.Scripting.Scripts
{
    ...
    public abstract class ItemScript : GeneralScript
    {
        ...
        {
            var attr = GetType().GetCustomAttribute<ItemScriptAttribute>();
            if (attr == null)
            {
        ...
        protected void Heal(Creature creature, double life, double mana, double stamina, double toxicity)
        {
            ...
            if (toxicityStage != ToxicityStage.Normal)
            {
                ...
                {
                    case ToxicityStage.Stage1:
                        multiplier = 1.0 + rnd.NextDouble() * 0.3;
                        break;
                    case ToxicityStage.Stage2:
                        multiplier = 1.3;
                        break;
                    case ToxicityStage.Stage3:
                        multiplier = 1.3 + rnd.NextDouble() * 0.3;
                        break;
                    case ToxicityStage.Stage4:
                        multiplier = 1.6;
                        break;
                    case ToxicityStage.Stage5:
                        multiplier = 1.6 + rnd.NextDouble() * 0.4;
                        break;
                    case ToxicityStage.Stage6:
                        multiplier = 2.0 + rnd.NextDouble() * 1.0;
                        break;
                }
            ...
            Poison(creature, diffLife, diffMana, diffStamina, toxicity);
        }
        ...
        protected void HealRate(Creature creature, double life, double mana, double stamina, double toxicity)
        {
            ...
            Heal(creature, life, mana, stamina, toxicity);
        }
        ...
        protected void HealFull(Creature creature, double toxicity)
        {
            creature.Injuries = 0;
            creature.Hunger = 0;
            HealRate(creature, 100, 100, 100, toxicity);
        }
        ...
        private void Poison(Creature creature, double life, double mana, double stamina, double toxicity)
        {
            ...
            if (stage != beforeStage)
                ...
                {
                    case ToxicityStage.Stage1:
                        Send.Notice(creature, L("It feels like the potion works better than before."));
                        break;
                    case ToxicityStage.Stage2:
                        Send.Notice(creature, L("This potion is effective!"));
                        break;
                    case ToxicityStage.Stage3:
                        Send.Notice(creature, L("This potion is clearly effective, but it feels weird somehow."));
                        break;
                    case ToxicityStage.Stage4:
                        Send.Notice(creature, L("The potion has some side effects."));
                        break;
                    case ToxicityStage.Stage5:
                        Send.Notice(creature,
                            L("The potion worked as it should, but it had some bad side-effects too."));
                        break;
                    case ToxicityStage.Stage6:
                        Send.Notice(creature, L("Anymore of this potion is dangerous!"));
                        break;
                }
        ...
        /// </remarks>
        protected void Feed(Creature creature, double hunger, double weight = 0, double upper = 0, double lower = 0,
            double str = 0, double int_ = 0, double dex = 0, double will = 0, double luck = 0, double life = 0,
            double mana = 0, double stm = 0)
        {
        ...
        protected void Treat(Creature creature, double injuries, double toxicity)
        {
            ...
            Poison(creature, diffInjuries, 0, 0, toxicity);
        }
        ...
        protected void AddGesture(Creature creature, string keyword, string name)
        {
            ...
            Send.Notice(creature, Localization.Get("The {0} Gesture has been added. Check your gestures window."),
                name);
        }
        ...
        protected void Buff(Creature creature, Item item, int timeout, double str = 0, double int_ = 0, double dex = 0,
            double will = 0, double luck = 0, double life = 0, double mana = 0, double stamina = 0,
            double lifeRecovery = 0, double manaRecovery = 0, double staminaRecovery = 0, double injuryRecovery = 0,
            int defense = 0, int protection = 0)
        {
            ...
            // Add stat buffs
            if (str != 0)
                creature.StatMods.Add(Stat.StrMod, (float) Math.Floor(str / 200f * quality), statModSource,
                    statModIdent, timeout);

            if (int_ != 0)
                creature.StatMods.Add(Stat.IntMod, (float) Math.Floor(int_ / 200f * quality), statModSource,
                    statModIdent, timeout);

            if (dex != 0)
                creature.StatMods.Add(Stat.DexMod, (float) Math.Floor(dex / 200f * quality), statModSource,
                    statModIdent, timeout);

            if (will != 0)
                creature.StatMods.Add(Stat.WillMod, (float) Math.Floor(will / 200f * quality), statModSource,
                    statModIdent, timeout);

            if (luck != 0)
                creature.StatMods.Add(Stat.LuckMod, (float) Math.Floor(luck / 200f * quality), statModSource,
                    statModIdent, timeout);

            if (defense != 0)
                creature.StatMods.Add(Stat.DefenseMod, (float) Math.Floor(defense / 200f * quality), statModSource,
                    statModIdent, timeout);

            if (protection != 0)
                creature.StatMods.Add(Stat.ProtectionMod, (float) Math.Floor(protection / 200f * quality),
                    statModSource, statModIdent, timeout);

            if (life != 0)
            {
                creature.StatMods.Add(Stat.LifeMaxMod, (float) Math.Floor(life / 200f * quality), statModSource,
                    statModIdent, timeout);
            ...
            if (mana != 0)
            {
                creature.StatMods.Add(Stat.ManaMaxMod, (float) Math.Floor(mana / 200f * quality), statModSource,
                    statModIdent, timeout);
            ...
            if (stamina != 0)
            {
                creature.StatMods.Add(Stat.StaminaMaxMod, (float) Math.Floor(stamina / 200f * quality), statModSource,
                    statModIdent, timeout);
            ...
            Send.StatUpdate(creature, StatUpdateType.Public, Stat.Life, Stat.LifeInjured, Stat.LifeMaxMod,
                Stat.LifeMax);
        }
        ...
        protected void ShootFirework(Location location, FireworkType type, string message)
        {
            ...
            if (region == null)
            {
                Log.Warning(GetType().Name + ".ShootFirework: Unknown region.");
                return;
            }
