...
namespace Aura.Channel.Scripting.Scripts
{
    ...
    public class NpcShopScript : GeneralScript, IDisposable
    {
        ...
        public NpcShopScript()
        {
            ...
            ChannelServer.Instance.Events.ErinnMidnightTick += OnErinnMidnightTick;
        }
        ...
        {
            if (ChannelServer.Instance.ScriptManager.NpcShopScripts.ContainsKey(GetType().Name))
            {
                Log.Error("NpcShopScript.Init: Duplicate '{0}'", GetType().Name);
                return false;
            }
        ...
        {
            var tab = GetOrCreateTab(tabTitle);
            ...
            switch (tab.PaymentMethod)
            {
                case PaymentMethod.Gold:
                    item.SetGoldPrice(price);
                    break;
                case PaymentMethod.Stars:
                    item.OptionInfo.StarPrice = price;
                    break;
                case PaymentMethod.Ducats:
                    item.OptionInfo.DucatPrice = price;
                    break;
                case PaymentMethod.Points:
                    item.OptionInfo.PointPrice = price;
                    break;
            }
        ...
        public void OpenFor(Creature creature, NPC owner)
        {
            ...
            Send.OpenNpcShop(creature, GetTabs(creature, owner));
        }
        ...
        public bool Buy(Creature creature, long itemEntityId, bool moveToInventory, bool directBankTransaction)
        {
            ...
            {
                case PaymentMethod.Gold:
                    price = item.OptionInfo.Price;
                    break;
                case PaymentMethod.Stars:
                    price = item.OptionInfo.StarPrice;
                    break;
                case PaymentMethod.Ducats:
                    price = item.OptionInfo.DucatPrice;
                    break;
                case PaymentMethod.Points:
                    price = item.OptionInfo.PointPrice;
                    break;
            }
            ...
            switch (paymentMethod)
            {
                ...
                case PaymentMethod.Stars:
                    canPay = creature.Inventory.Stars >= price;
                    break;
                case PaymentMethod.Ducats:
                    canPay = false;
                    break; // TODO: Implement ducats.
                case PaymentMethod.Points:
                    canPay = creature.Points >= price;
                    break;
            }
            ...
            if (!canPay)
            {
                switch (paymentMethod)
                {
                    case PaymentMethod.Gold:
                        ...
                        break;
                    case PaymentMethod.Stars:
                        Send.MsgBox(creature, Localization.Get("Insufficient amount of stars."));
                        break;
                    case PaymentMethod.Ducats:
                        Send.MsgBox(creature, Localization.Get("Insufficient amount of ducats."));
                        break;
                    case PaymentMethod.Points:
                        Send.MsgBox(creature,
                            Localization.Get("You don't have enough Pon.\nYou will need to buy more."));
                        break;
                }
            ...
            if (success)
            {
                ...
                if (item.Stock > 0)
                {
                    ...
                    if (item.Stock == 0)
                    {
                        ...
                        Send.AddToNpcShop(creature, GetTabs(creature, owner));
                    }
    ...
    public class NpcShopTab
    {
        ...
        public NpcShopTab(string title, int order, bool randomizeColors, Func<Creature, NPC, bool> display)
        {
            ...
            Title = title;
            Order = order;
            RandomizeColorsEnabled = randomizeColors;
            ShouldDisplay = display ?? ((c, n) => true);
        }
