...
namespace Aura.Channel.Scripting.Scripts
{
    ...
    public abstract class GeneralScript : IDisposable, IScript, IAutoLoader
    {
        ...
        /// </summary>
        protected ScriptVariables GlobalVars => ChannelServer.Instance.ScriptManager.GlobalVars;
        ...
        public void AutoLoad()
        {
            ...
            foreach (var method in methods)
            {
                ...
                try
                {
                    eventHandlerInfo.AddEventHandler(ChannelServer.Instance.Events,
                        Delegate.CreateDelegate(eventHandlerInfo.EventHandlerType, this, method));
                }
                catch (Exception ex)
                {
                    Log.Exception(ex, "AutoLoadEvents: Failed to subscribe '{1}.{2}' to '{0}'.", attr.Event, type.Name,
                        method.Name);
                }
            }
        }
        ...
        public virtual void Dispose()
        {
            ...
            foreach (var method in methods)
            {
                ...
                {
                    Log.Exception(ex, "Dispose: Failed to unsubscribe '{1}.{2}' from '{0}'.", attr.Event, type.Name,
                        method.Name);
                }
            }
        ...
        {
            // Calculate days
            var m1 = now.Year * 7 * 40 * 24 * 60 + now.Month * 40 * 24 * 60 + now.Day * 24 * 60 + now.Hour * 60 +
                     now.Minute;
            var m2 = future.Year * 7 * 40 * 24 * 60 + future.Month * 40 * 24 * 60 + future.Day * 24 * 60 +
                     future.Hour * 60 + future.Minute;
        ...
        public IEnumerable<int> GetLevelMatchingQuestIds(QuestLevel level, PtjType type, params int[] questIds)
        {
            ...
            var questScripts = questIds.Select(id => ChannelServer.Instance.ScriptManager.QuestScripts.Get(id))
                .Where(a => a != null);
            ...
            if (questScriptsCount != questIds.Length)
            {
                ...
                Log.Warning("Creature.RandomPtj: Some of the given quest ids are unknown (" +
                            string.Join(", ", missing) + ").");
            }
        ...
        public int GetRandomPtj(Creature creature, PtjType type, params int[] questIds)
        {
            ...
            var sameLevelQuests = GetLevelMatchingQuestIds(level, type, questIds);
        ...
        /// <param name="func"></param>
        protected void AddCommand(int auth, int charAuth, string name, string usage, string description,
            GmCommandFunc func)
        {
        ...
        protected Prop SpawnProp(int id, int regionId, int x, int y, float direction, float scale,
            PropFunc behavior = null)
        {
            var region = ChannelServer.Instance.World.GetRegion(regionId);
            if (region == null)
            {
                Log.Error("{1}.SpawnProp: Region '{0}' doesn't exist.", regionId, GetType().Name);
                return null;
            }
        ...
        {
            return SpawnProp(id, regionId, x, y, direction, 1, behavior);
        }
        ...
        protected Prop SpawnProp(Prop prop, PropFunc behavior = null)
        {
            ...
            if (region == null)
            {
                Log.Error("{1}.SpawnProp: Region '{0}' doesn't exist.", prop.RegionId, GetType().Name);
                return null;
            }
        ...
        protected Prop SetPropBehavior(long entityId, PropFunc behavior)
        {
            ...
            if (prop == null)
            {
                Log.Error("{1}.SetPropBehavior: Prop '{0:X16}' doesn't exist.", entityId, GetType().Name);
                return null;
            }
        ...
        protected int CreateSpawner(int race, int amount, int region, int delay = 0, int delayMin = 10,
            int delayMax = 20, int[] titles = null, int[] coordinates = null)
        {
        ...
        {
            return Spawn(raceId, amount, regionId, x, y, null);
        }
        ...
        {
            return Spawn(raceId, amount, regionId, x, y, radius, true, null);
        }
        ...
        {
            return Spawn(raceId, amount, regionId, x, y, radius, effect, null);
        }
        ...
        /// <returns>List of creatures spawned by this call.</returns>
        protected List<Creature> Spawn(int raceId, int amount, int regionId, int x, int y,
            Action<Creature, Creature> onDeath)
        {
            return Spawn(raceId, amount, regionId, x, y, 0, true, onDeath);
        }
        ...
        protected List<Creature> Spawn(int raceId, int amount, int regionId, int x, int y, int radius, bool effect,
            Action<Creature, Creature> onDeath)
        {
            return Spawn(raceId, amount, regionId, new Position(x, y), radius, effect, onDeath);
        }
        ...
        /// <param name="onDeath">Runs when one of the creatures dies</param>
        /// <returns>List of creatures spawned by this call.</returns>
        protected List<Creature> Spawn(int raceId, int amount, int regionId, Position pos, int radius, bool effect,
            Action<Creature, Creature> onDeath)
        {
            ...
            for (var i = 0; i < amount; ++i)
            {
                ...
                var creature =
                    ChannelServer.Instance.World.SpawnManager.Spawn(raceId, regionId, pos.X, pos.Y, true, effect);
            ...
            timer = new Timer(_ =>
                {
                    ...
                    {
                        Log.Exception(ex, "Exception during SetTimeout callback in {0}.", GetType().Name);
                    }
                    GC.KeepAlive(timer);
                }
            ...
            timer = new Timer(_ =>
                {
                    ...
                    {
                        Log.Exception(ex, "Exception during SetInterval callback in {0}.", GetType().Name);
                    }
                    GC.KeepAlive(timer);
                }
                , null, ms, ms);
        ...
        {
            if (till < from)
                Log.Warning("{0}: ScheduleEvent: Till date is earlier than from date.", GetType().Name);
