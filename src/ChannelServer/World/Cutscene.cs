...
namespace Aura.Channel.World
{
    public class Cutscene
    {
        ...
        private readonly Creature[] _viewers;
        ...
        public Cutscene(string name, Creature leader, params Creature[] viewers)
        {
            ...
            if ((Data = AuraData.CutscenesDb.Find(name)) == null)
                throw new ArgumentException("Unknown cutscene '" + name + "'.");

            Name = name;
            Leader = leader;

            Actors = new Dictionary<string, Creature>();
        ...
        public void LoadActorsFromData()
        {
            ...
            foreach (var cutsceneActorData in Data.Actors)
            {
                Creature actor = null;
                ...
                else if (actorName.StartsWith("player"))
                {
                    ...
                    if (!int.TryParse(actorName.Substring("player".Length), out idx))
                        ...
                        {
                            Log.Warning("Cutscene.FromData: Index out of party member range '{0}/{1}'.", idx,
                                _viewers.Length);
                        }
                    else
                        actor = _viewers[idx];
