...
namespace Aura.Channel.World
{
    ...
    public struct Location
    {
        ...
        {
            RegionId = (ushort) (locationId >> 32);
            X = (ushort) (locationId >> 16) * 20;
            Y = (ushort) (locationId >> 00) * 20;
        }
        ...
        public Location(string location)
        {
            ...
            if (split.Length > 2)
            {
                ...
                if (eventData == null)
                    throw new Exception("Event '" + split[2] + "' not found in area '" + areaData.Name +
                                        "' of region '" + regionData.Name + "'.");
            }
            ...
            {
                // Based on area
                X = (areaData.X1 + areaData.X2) / 2;
                Y = (areaData.Y1 + areaData.Y2) / 2;
            }
            else
            {
                // Based on region
                X = (regionData.X1 + regionData.X2) / 2;
                Y = (regionData.Y1 + regionData.Y2) / 2;
            }
        }
        ...
        public long ToLocationId()
        {
            ...
            result |= (long) (0xFFFF & RegionId) << 32;
            result |= (long) (0xFFFF & (X / 20)) << 16;
            result |= (long) (0xFFFF & (Y / 20)) << 0;
        ...
        {
            return RegionId.GetHashCode() ^ X.GetHashCode() ^ Y.GetHashCode();
        }
        ...
        {
            return string.Format("(RegionId: {0}, X: {1}, Y: {2})", RegionId, X, Y);
        }
    }
}