...
namespace Aura.Channel.World.Entities.Creatures
{
    public class CreatureQuests : IDisposable
    {
        ...
        private readonly Dictionary<int, QuestOwl> _owlQueue;
        private readonly Timer _owlTimer;
        private readonly Dictionary<PtjType, PtjTrackRecord> _ptjRecords;
        ...
        {
            return Get(a => a.Id == questId && a.State == QuestState.InProgress);
        }
        ...
        {
            return Get(a => a.UniqueId == uniqueId);
        }
        ...
        public void SendOwl(int questId, int delay)
        {
            ...
            QueueOwl(questId, DateTime.Now.AddSeconds(delay));
        }
        ...
        {
            var quest = GetFirstIncomplete(questId);
        ...
        {
            var quest = Get(a => a.Id == questId && a.State == QuestState.InProgress);
            if (quest == null)
                return false;

            return Complete(quest, owl);
        }
        ...
        public bool Complete(Quest quest, int rewardGroup, bool owl)
        {
            ...
            var success = EndQuest(quest, rewardGroup, owl);
            if (success)
            {
                ...
                for (var i = quest.Data.Objectives.Count - 1; i >= 0; --i)
                {
                    ...
                    {
                        var items = _creature.Inventory.GetItems(item => item.Info.Id == itemId && !item.IsIncomplete,
                            StartAt.BottomRight);
        ...
        public bool GiveUp(Quest quest)
        {
            ...
            var success = EndQuest(quest, -1, false);
        ...
        private bool EndQuest(Quest quest, int rewardGroup, bool owl)
        {
            ...
            if (quest.Data.Type == QuestType.Deliver)
                ModifyPtjTrackRecord(quest.Data.PtjType, +1, result == QuestResult.Perfect ? +1 : 0);
            ...
            if (rewardGroup != -1)
            {
                ...
                if (rewards.Count == 0)
                    Log.Warning("CreatureQuests.EndQuest: No rewards for group '{0}' at result '{1}' in quest '{2}'.",
                        rewardGroup, result, quest.Id);
                else
                    GiveRewards(quest, rewards, owl);
            }
            ...
            if (quest.Data.Type == QuestType.Deliver)
            {
                var record = GetPtjTrackRecord(quest.Data.PtjType);
        ...
        {
            var record = GetPtjTrackRecord(type);

            record.Done += done;
            record.Success += success;
            record.LastChange = DateTime.Now;

            PtjTrackRecordChanged.Raise(_creature, record);
        }
        ...
        {
            return Get(a => a.Data.Type == QuestType.Deliver && a.State != QuestState.Complete);
        }
