...
namespace Aura.Channel.World.Entities.Creatures
{
    ...
    {
        private readonly Dictionary<SkillId, DateTime> _autoCancel;
        private readonly Dictionary<SkillId, Action> _callbacks;
        private readonly Creature _creature;
        private readonly Dictionary<SkillId, Skill> _skills;
        ...
        public bool Add(Skill skill)
        {
            ...
            if (Has(skill.Info.Id, skill.Info.Rank))
                return false;

            var oldSkill = Get(skill.Info.Id);
        ...
        public bool Add(SkillId skillId, SkillRank skillRank, int raceId)
        {
            ...
            return Add(new Skill(_creature, skillId, skillRank, raceId));
        }
        ...
        public void Give(SkillId id, SkillRank rank)
        {
            ...
            if (skill == null)
            {
                Add(skill = new Skill(_creature, id, rank, _creature.RaceId));
            ...
            Send.StatUpdate(_creature, StatUpdateType.Private,
                ...
                Stat.Life, Stat.LifeInjured, Stat.LifeMaxMod, Stat.LifeMax, Stat.Mana, Stat.ManaMaxMod, Stat.ManaMax,
                Stat.Stamina, Stat.Hunger, Stat.StaminaMaxMod, Stat.StaminaMax
            );
            Send.StatUpdate(_creature, StatUpdateType.Public, Stat.Life, Stat.LifeInjured, Stat.LifeMaxMod,
                Stat.LifeMax);

            RankChanged.Raise(_creature, skill);
        }
        ...
        public void AddBonuses(Skill skill)
        {
            ...
            {
                _creature.StatMods.Add(Stat.LifeMaxMod, skill.RankData.Var3, StatModSource.SkillRank,
                    (int) skill.Info.Id);
                _creature.Life += skill.RankData.Var3;
            }
            else if (skill.Info.Id == SkillId.MagicMastery)
            {
                _creature.StatMods.Add(Stat.ManaMaxMod, skill.RankData.Var1, StatModSource.SkillRank,
                    (int) skill.Info.Id);
                _creature.Mana += skill.RankData.Var1;
            }
            else if (skill.Info.Id == SkillId.Defense)
            {
                _creature.StatMods.Add(Stat.DefenseBaseMod, skill.RankData.Var1, StatModSource.SkillRank,
                    (int) skill.Info.Id);
            }
        ...
        private void UpdateHighestSkills()
        {
            ...
            HighestSkillCp = highest;
            SecondHighestSkillCp = second;
        }
        ...
        public void CancelActiveSkill()
        {
            if (ActiveSkill == null)
            {
                Log.Warning("CancelActiveSkill: Player '{0}' tried to cancel a skill, without one being active.",
                    _creature.Name);
                return;
            }

            var handler = ChannelServer.Instance.SkillManager.GetHandler<ICancelable>(ActiveSkill.Info.Id);
            if (handler == null)
            {
                Log.Unimplemented("CancelActiveSkill: Skill handler or interface for '{0}'.", ActiveSkill.Info.Id);
                goto L_Cancel;
            }

            try
            {
                handler.Cancel(_creature, ActiveSkill);
            }
            catch (NotImplementedException)
            {
                Log.Unimplemented("CancelActiveSkill: Skill cancel method for '{0}'.", ActiveSkill.Info.Id);
            }

            L_Cancel:
            ActiveSkill.Stacks = 0;

            Send.SkillCancel(_creature);

            ActiveSkill.State = SkillState.Canceled;
            ActiveSkill = null;
        ...
        {
            return ActiveSkill != null && ActiveSkill.Info.Id == skillId;
        }
        ...
        {
            return IsActive(skillId) && ActiveSkill.State == SkillState.Ready;
        }
                ...
                foreach (var ac in _autoCancel)
                {
                    ...
                    else if (handler is IStoppable)
                    {
                        ...
                        if (skill.Has(SkillFlags.InUse))
                        {
                            ...
                            stopHandler.Stop(_creature, skill, new Packet(0, 0));
                        }
                    }
                    else
                    {
