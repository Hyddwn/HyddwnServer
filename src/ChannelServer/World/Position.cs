...
namespace Aura.Channel.World
{
    ...
    public struct Position
    {
        ...
        {
            X = (short) (locationId >> 16) * 20;
            Y = (short) (locationId >> 00) * 20;
        }
        ...
        {
            var x = rnd.Next(X - width / 2, X + width / 2 + 1);
            var y = rnd.Next(Y - height / 2, Y + height / 2 + 1);
        ...
        public Position GetRelative(Position other, int distance)
        {
            ...
            var deltaX = (double) other.X - X;
            var deltaY = (double) other.Y - Y;
        ...
        public Position GetRelative(double radian, int distance)
        {
            ...
            var newX = X + distance / deltaXY * deltaX;
            var newY = Y + distance / deltaXY * deltaY;
        ...
        public bool InPolygon(params Point[] points)
        {
            ...
            for (int i = 0, j = points.Length - 1; i < points.Length; j = i++)
                if (points[i].Y > Y != points[j].Y > Y && X <
                    (points[j].X - points[i].X) * (Y - points[i].Y) / (points[j].Y - points[i].Y) + points[i].X)
                    result = !result;
