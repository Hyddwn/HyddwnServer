...
namespace Aura.Channel.World
{
    public class WorldManager
    {
        ...
        public WorldManager()
        {
            ...
            DynamicRegions = new DynamicRegionManager();
            SpawnManager = new SpawnManager();
        }

        public DungeonManager DungeonManager { get; }
        public DynamicRegionManager DynamicRegions { get; }
        ...
        {
            Parallel.ForEach(AuraData.RegionDb.Entries.Values, region => AddRegion(region.Id));
        }
        ...
        private void SetUpHeartbeat()
        {
            ...
            _heartbeatTimer = new Timer(Heartbeat, null, HeartbeatTime - now.Ticks / 10000 % HeartbeatTime,
                HeartbeatTime);
        }
        ...
        public PlayerCreature GetPlayer(string name)
        {
            lock (_regions)
            {
                return _regions.Values.Select(region => region.GetPlayer(name))
        ...
        public Creature GetCreature(long entityId)
        {
            lock (_regions)
            {
                return _regions.Values.Select(region => region.GetCreature(entityId))
                    .FirstOrDefault(creature => creature != null);
            }
        }
        ...
        public Creature GetCreature(string name)
        {
            lock (_regions)
            {
                return _regions.Values.Select(region => region.GetCreature(name))
                    .FirstOrDefault(creature => creature != null);
            }
        }
        ...
        public NPC GetNpc(long entityId)
        {
            lock (_regions)
            {
                return _regions.Values.Select(region => region.GetNpc(entityId))
                    .FirstOrDefault(creature => creature != null);
            }
        }
        ...
        public NPC GetNpc(string name)
        {
            lock (_regions)
            {
                return (NPC) _regions.Values.Select(region => region.GetCreature(name))
                    .FirstOrDefault(creature => creature != null && creature is NPC);
            }
        }
