...
namespace Aura.Channel.World.Inventory
{
    ...
    public class InventoryPocketNormal : InventoryPocket
    {
        ...
        public override IEnumerable<Item> Items => _items.Values;
        ...
        public override bool TryAdd(Item newItem, byte targetX, byte targetY, out Item collidingItem)
        {
            ...
            var collidingItems = GetCollidingItems(targetX, targetY, newItem);
            ...
            if (collidingItem != null && (
                    ...
                    collidingItem.Data.StackType == StackType.Sac && collidingItem.Data.StackItemId != 0 &&
                    (collidingItem.Data.StackItemId == newItem.Info.Id ||
                     collidingItem.Data.StackItemId == newItem.Data.StackItemId) ||
            ...
            if (collidingItem != null)
            {
                ...
                ClearFromMap(collidingItem);
            }

            _items.Add(newItem.EntityId, newItem);
            newItem.Move(Pocket, targetX, targetY);
            AddToMap(newItem);
        ...
        {
            var count = 0;
            var max = item.Data.Width * item.Data.Height;
        ...
        public override bool Add(Item item)
        {
            ...
            for (byte x = 0; x <= _width - item.Data.Width; ++x)
            {
                ...
                if (GetCollidingItems(x, y, item).Count == 0)
                {
        ...
        public override bool FillStacks(Item item, out List<Item> changed)
        {
            ...
            for (var x = 0; x < _width; ++x)
            {
                ...
                if (item.Info.Id == invItem.Info.Id || invItem.Data.StackItemId == item.Info.Id)
                {
                    ...
                    if (invItem.Info.Amount + (uint) item.Info.Amount <= invItem.Data.StackMax)
                    {
        ...
        public override int Remove(int itemId, int amount, ref List<Item> changed)
        {
            ...
            for (var x = _width - 1; x >= 0; --x)
            {
                ...
                // Sacs/Stackables
                if (item.Data.StackItemId == itemId ||
                    item.Info.Id == itemId && item.Data.StackType == StackType.Stackable)
        ...
        public override int CountItem(string tag)
        {
            return _items.Values.Where(item =>
                    item.HasTag(tag) || item.Data.StackItem != null && item.Data.StackItem.HasTag(tag))
                .Aggregate(0, (current, item) => current + item.Info.Amount);
        }
    ...
    public class InventoryPocketSingle : InventoryPocket
    {
        ...
        public override int Count => _item != null ? 1 : 0;
        ...
        public override bool TryAdd(Item newItem, byte targetX, byte targetY, out Item collidingItem)
        {
            ...
            if (collidingItem != null &&
                (collidingItem.Data.StackType == StackType.Sac &&
                 (collidingItem.Data.StackItemId == newItem.Info.Id ||
                  collidingItem.Data.StackItemId == newItem.Data.StackItemId) ||
                 newItem.Data.StackType == StackType.Stackable && newItem.Info.Id == collidingItem.Info.Id))
                if (collidingItem.Info.Amount < collidingItem.Data.StackMax)
