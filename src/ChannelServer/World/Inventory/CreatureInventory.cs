...
namespace Aura.Channel.World.Inventory
{
    ...
    public class CreatureInventory
    {
        ...
        private readonly Dictionary<Pocket, InventoryPocket> _pockets;
        private Dictionary<UpgradeCheckType, int> _upgradeCheckTypeCache = new Dictionary<UpgradeCheckType, int>();

        private readonly object _upgradeEffectSyncLock = new object();

        private readonly Dictionary<long, WUUpgrades> _wuUpgrades;
        ...
        public CreatureInventory(Creature creature)
        {
            ...
            Add(new InventoryPocketStack(Pocket.Temporary));
            Add(new InventoryPocketStack(Pocket.Quests));
            Add(new InventoryPocketSingle(Pocket.Cursor));
            Add(new InventoryPocketNormal(Pocket.Trade, 10, 5));
            Add(new InventoryPocketSingle(Pocket.EntrustmentItem1));
            Add(new InventoryPocketSingle(Pocket.EntrustmentItem2));
            Add(new InventoryPocketNormal(Pocket.EntrustmentReward, 10, 5));

            Add(new InventoryPocketStack(Pocket.ExtraEquipSlotKits));

            // Equipment
            for (var i = Pocket.Face; i <= Pocket.Accessory2; ++i)
                Add(new InventoryPocketSingle(i));
            ...
            Add(new InventoryPocketSingle(Pocket.TailStyle));
        }

        /// <summary>
        ///     These pockets are checked by Safe methoods.
        /// </summary>
        public static ISet<Pocket> AccessiblePockets { get; }
        ...
        /// </summary>
        public Pocket RightHandPocket => WeaponSet == WeaponSet.First ? Pocket.RightHand1 : Pocket.RightHand2;

        /// <summary>
        ///     The currently active left hand pocket (off hand).
        /// </summary>
        public Pocket LeftHandPocket => WeaponSet == WeaponSet.First ? Pocket.LeftHand1 : Pocket.LeftHand2;

        /// <summary>
        ///     The currently active magazine pocket (ammunition).
        /// </summary>
        public Pocket MagazinePocket => WeaponSet == WeaponSet.First ? Pocket.Magazine1 : Pocket.Magazine2;
        ...
        public int Gold
        {
            ...
            set
            {
                ...
                if (newAmount < curAmount)
                    Remove(itemId, curAmount - newAmount);
                else if (newAmount > curAmount)
                    InsertStacks(itemId, newAmount - curAmount);
            }
        }
        ...
        public void AddMainInventory()
        {
            if (_creature.RaceData == null)
                Log.Warning("Race for creature '{0}' ({1:X16}) not loaded before initializing main inventory.",
                    _creature.Name, _creature.EntityId);
            ...
            // TODO: Race check
            Add(new InventoryPocketNormal(Pocket.Inventory, width, height));
            Add(new InventoryPocketNormal(Pocket.PersonalInventory, width, height));
            Add(new InventoryPocketNormal(Pocket.VIPInventory, width, height));
        }
        ...
        public void StartLiveUpdate()
        {
            ...
            _creature.LeveledUp += OnCreatureLeveledUp;
            _creature.Titles.Changed += OnCreatureChangedTitles;
            _creature.Skills.RankChanged += OnCreatureSkillRankChanged;
            _creature.Conditions.Changed += OnCreatureConditionsChanged;
            ChannelServer.Instance.Events.HoursTimeTick += OnHoursTimeTick;
        }
        ...
        public Item[] GetMainEquipment()
        {
            ...
            {
                result = _pockets.Values
                    .Where(a => a.Pocket.IsMainEquip(WeaponSet))
                    .SelectMany(pocket => pocket.Items.Where(a => a != null))
                    .ToArray();
            }
        ...
        public Item GetItemSafe(long entityId)
        {
            ...
            if (!AccessiblePockets.Contains(result.Info.Pocket))
                throw new SevereViolation("Item 0x{0:X16} is located in inaccessible pocket {1}", entityId,
                    result.Info.Pocket);
        ...
        {
            var freePocket = GetFreePocketId();
            if (freePocket == Pocket.None)
                return false;

            item.OptionInfo.LinkedPocketId = freePocket;

            Add(new InventoryPocketNormal(freePocket, item.Data.BagWidth, item.Data.BagHeight));
        ...
        public bool Move(Item item, Pocket target, byte targetX, byte targetY)
        {
            ...
            lock (_pockets)
            {
                ...
                if (collidingItem != null &&
                    (item.Info.Amount != amount || item.Info.Amount == 0 && item.Data.Type != ItemType.Sac))
                {
            ...
            // Check movement
            CheckLeftHand(item, source, target);
            CheckRightHand(item, source, target);

            // Equip handling
            if (target.IsEquip())
            {
                UpdateEquipReferences();
                OnEquip(item);
                if (collidingItem != null)
                    OnUnequip(collidingItem);
                UpdateEquipStats();
            ...
            if (source >= Pocket.EntrustmentItem1 && source <= Pocket.EntrustmentReward)
                _creature.Temp.ActiveEntrustment.RemoveItem(item, source);
        ...
        {
            if (!Has(target) || !other.Inventory.Has(target))
                return false;
            ...
            lock (_pockets)
            {
                if (!other.Inventory._pockets[target]
                    .TryAdd(newItem, (byte) targetX, (byte) targetY, out collidingItem))
                    return false;
        ...
        public bool PickUp(Item item)
        {
            ...
            var success = Insert(newItem, false);
        ...
        {
            var unequipRightHand = RightHand;
            var unequipLeftHand = LeftHand;
            var unequipMagazine = Magazine;

            WeaponSet = set;
            UpdateEquipReferences();

            if (unequipRightHand != null) OnUnequip(unequipRightHand);
            if (unequipLeftHand != null) OnUnequip(unequipLeftHand);
            if (unequipMagazine != null) OnUnequip(unequipMagazine);

            if (RightHand != null) OnEquip(RightHand);
            if (LeftHand != null) OnEquip(LeftHand);
            if (Magazine != null) OnEquip(Magazine);
            ...
            // Check touchability
            UnequipIfCantBeTouched(RightHand);
            UnequipIfCantBeTouched(LeftHand);
            UnequipIfCantBeTouched(Magazine);
        }
            ...
            if (success)
            {
                ...
                if (_creature.IsPlayer && pocket != Pocket.Temporary)
                {
                    ...
                    if (item.Data.StackType == StackType.Sac)
                        ChannelServer.Instance.Events.OnPlayerReceivesItem(_creature, item.Data.StackItemId,
                            item.Info.Amount);
                }
        ...
        {
            var success = TryAutoAdd(item, tempFallback);
        ...
        {
            List<Item> changed;
            return Insert(item, tempFallback, out changed);
        }
        ...
        {
            return InsertStacks(GoldItemId, amount);
        }
        ...
        public bool Remove(Item item)
        {
            lock (_pockets)
            {
                if (_pockets.Values.Any(pocket => pocket.Remove(item)))
                {
                    ...
                    if (item.Info.Pocket.IsEquip())
                    {
                        CheckLeftHand(item, item.Info.Pocket, Pocket.None);
                        CheckRightHand(item, item.Info.Pocket, Pocket.None);
        ...
        {
            return Remove(GoldItemId, amount);
        }
        ...
        {
            if (!Has(item) || item.Info.Amount == 0 || item.Info.Amount < amount)
                return false;
        ...
        private void OnEquip(Item item)
        {
            ...
            if (item.Info.Pocket.IsMainEquip(WeaponSet))
            {
                ApplyDefenseBonuses(item);
                ApplyUpgrades(item);
                ApplyUpgradeEffects(item);
                HandleWUUpgrades(item);
            }
        }
        ...
        public void UpdateStatBonuses()
        {
            lock (_upgradeEffectSyncLock)
            {
                ...
                // and reapply them.
                foreach (var item in GetMainEquipment())
                {
                    _creature.StatMods.Remove(StatModSource.Equipment, item.EntityId);
                    ApplyDefenseBonuses(item);
                    ApplyUpgradeEffects(item);
        ...
        private void ApplyDefenseBonuses(Item item)
        {
            if (item.OptionInfo.Defense != 0)
                _creature.StatMods.Add(Stat.DefenseBaseMod, item.OptionInfo.Defense, StatModSource.Equipment,
                    item.EntityId);
            if (item.OptionInfo.Protection != 0)
                _creature.StatMods.Add(Stat.ProtectionBaseMod, item.OptionInfo.Protection, StatModSource.Equipment,
                    item.EntityId);
        }
        ...
        private void ApplyUpgrades(Item item)
        {
            ...
            if (magicDefenseMod != 0)
                _creature.StatMods.Add(Stat.MagicDefenseMod, magicDefenseMod, StatModSource.Equipment, item.EntityId);
            if (magicProtectionMod != 0)
                _creature.StatMods.Add(Stat.MagicProtectionMod, magicProtectionMod, StatModSource.Equipment,
                    item.EntityId);
        }
        ...
        private void ApplyUpgradeEffects(Item item)
        {
            foreach (var effect in item.GetUpgradeEffects())
            {
                ...
                else if (effect.CheckType >= UpgradeCheckType.GreaterThan && effect.CheckType <= UpgradeCheckType.Equal)
                {
                    ...
                    {
                        case UpgradeStat.Level:
                            valueToCheck = _creature.Level;
                            break;
                        case UpgradeStat.TotalLevel:
                            valueToCheck = _creature.TotalLevel;
                            break;
                        case UpgradeStat.ExplorationLevel:
                            valueToCheck = 0;
                            break; // TODO: Set once we have exploration levels.
                        case UpgradeStat.Age:
                            valueToCheck = _creature.Age;
                            break;
                    ...
                    {
                        case UpgradeCheckType.Equal:
                            fulfilled = valueToCheck == effect.CheckValue;
                            break;
                        case UpgradeCheckType.GreaterThan:
                            fulfilled = valueToCheck > effect.CheckValue;
                            break;
                        case UpgradeCheckType.GreaterEqualThan:
                            fulfilled = valueToCheck >= effect.CheckValue;
                            break;
                        case UpgradeCheckType.LowerThan:
                            fulfilled = valueToCheck < effect.CheckValue;
                            break;
                        case UpgradeCheckType.LowerEqualThan:
                            fulfilled = valueToCheck <= effect.CheckValue;
                            break;
                    }
                }
                // Skill rank >, <, ==
                else if (effect.CheckType >= UpgradeCheckType.SkillRankEqual &&
                         effect.CheckType <= UpgradeCheckType.SkillRankLowerThan)
                {
                    ...
                    if (skill != null)
                        switch (effect.CheckType)
                        {
                            case UpgradeCheckType.SkillRankEqual:
                                fulfilled = skill.Info.Rank == effect.CheckSkillRank;
                                break;
                            case UpgradeCheckType.SkillRankGreaterThan:
                                fulfilled = skill.Info.Rank >= effect.CheckSkillRank;
                                break;
                            case UpgradeCheckType.SkillRankLowerThan:
                                fulfilled = skill.Info.Rank < effect.CheckSkillRank;
                                break;
                        }
                }
                ...
                {
                    fulfilled = effect.CheckBroken && item.Durability == 0 ||
                                !effect.CheckBroken && item.Durability != 0;
                }
                // Title
                ...
                else if (effect.CheckType == UpgradeCheckType.WhileSummoned)
                {
                    switch (effect.CheckStat)
                    {
                        case UpgradeStat.Pet:
                            fulfilled = _creature.Pet != null;
                            break;
                        case UpgradeStat.Golem:
                            fulfilled = false;
                            break; // TODO: Set once we have golems.
                        case UpgradeStat.BarrierSpikes:
                            fulfilled = false;
                            break; // TODO: Set once we have barrier spikes.
        ...
        private void UpdateEquipReferences()
        {
            lock (_pockets)
            {
                RightHand = _pockets[RightHandPocket].GetItemAt(0, 0);
                LeftHand = _pockets[LeftHandPocket].GetItemAt(0, 0);
                Magazine = _pockets[MagazinePocket].GetItemAt(0, 0);
            }
        }
        ...
        private void OnItemLeavesInventory(Item item)
        {
            ...
            {
                Remove(item.OptionInfo.LinkedPocketId);
        ...
        {
            if (!Has(item) || ChannelServer.Instance.Conf.World.NoDurabilityLoss)
                return;
        ...
        {
            var items = GetItems(a => pockets.Contains(a.Info.Pocket));
            foreach (var item in items)
            {
                Remove(item);
                Add(item, true);
