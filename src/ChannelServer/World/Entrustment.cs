...
namespace Aura.Channel.World
{
    public class Entrustment
    {
        ...
        public Entrustment(Creature creature1, Creature creature2)
        {
            ...
            Creature1.Temp.ActiveEntrustment = this;
            Creature2.Temp.ActiveEntrustment = this;

            var skill = Creature2.Skills.Get(SkillId.Enchant);
            if (skill != null)
                EnchantRank = skill.Info.Rank;
        }
        ...
        /// </summary>
        public SkillRank EnchantRank { get; }
        ...
        /// </summary>
        public bool IsReady => CheckItems(Creature1);
        ...
        {
            Send.EntrustmentRequest(Creature2, Creature1.EntityId, 0);
        }
        ...
        public void Cancel()
        {
            ...
            if (Status == EntrustmentStatus.Canceled)
                return;
            Status = EntrustmentStatus.Canceled;

            Creature1.Inventory.MoveItemsToInvFrom(Pocket.EntrustmentItem1, Pocket.EntrustmentItem2,
                Pocket.EntrustmentReward);

            Send.Notice(Creature1, Localization.Get("The entrusting for enchantment has been cancelled."));
            Send.Notice(Creature2, Localization.Get("The entrusting for enchantment has been cancelled."));

            Send.EntrustmentClose(Creature1);
            Send.EntrustmentClose(Creature2);
        }
        ...
        {
            Send.EntrustmentAddItem(Creature2, pocket, item);

            // Update chance
            var chance = GetChance();
            Send.EntrustmentChanceUpdate(Creature1, chance, EnchantRank);
            Send.EntrustmentChanceUpdate(Creature2, chance, EnchantRank);

            // Update request button
            if (CheckItems(Creature1))
                Send.EntrustmentEnableRequest(Creature1);
            else
                Send.EntrustmentDisableRequest(Creature1);
        }
        ...
        {
            Send.EntrustmentRemoveItem(Creature2, pocket, item.EntityId);
        ...
        {
            var item2 = GetItem2();
            var optionSetId = GetOptionSetId(item2);
            var chance = 0f;

            // Get chance
            if (optionSetId != 0)
            {
                var powder = GetMagicPowder(Creature1);
                var optionSetData = AuraData.OptionSetDb.Find(optionSetId);

                if (powder != null && optionSetData != null)
                    chance = Enchant.GetChance(Creature2, powder, SkillId.Enchant, optionSetData);
            }
        ...
        public Item GetMagicPowder(Creature creature)
        {
            ...
            if (rhItem != null && CheckPowder(rhItem))
                return rhItem;

            // Check all pockets afterwards
            var items = creature.Inventory.GetItems(a => CheckPowder(a), StartAt.BottomRight);
            if (items.Count == 0)
                return null;
        ...
        public void Ready()
        {
            Status = EntrustmentStatus.Ready;
            Send.EntrustmentRequestFinalized(Creature2);
        }
        ...
        {
            Status = EntrustmentStatus.Ready;
            Send.EntrustmentFinalizing(Creature1);
            Send.Notice(Creature1, Localization.Get("The player has accepted the request."));
        }
        ...
        public void End()
        {
            var items = Creature1.Inventory.GetItems(a => a.Info.Pocket == Pocket.EntrustmentReward);
            foreach (var item in items)
            {
                Creature1.Inventory.Remove(item);
                Creature2.Inventory.Add(new Item(item), true);

                Send.SystemMessageFrom(Creature1, Localization.Get("<ENCHANT_ENTRUST>"),
                    Localization.Get("Gave the item  as a reward."), Localization.Get(item.Data.Name));
                Send.SystemMessageFrom(Creature1, Localization.Get("<ENCHANT_ENTRUST>"),
                    Localization.Get("Received the item {0} as a reward."), Localization.Get(item.Data.Name));
            }
            ...
            if (item1 != null)
            {
                Creature1.Inventory.Remove(item1);
                Creature1.Inventory.Add(item1, true);
            }
            if (item2 != null)
                Creature1.Inventory.Remove(item2);

            Send.EntrustmentEnd(Creature1);
            Send.EntrustmentEnd(Creature2);

            Creature1.Temp.ActiveEntrustment = null;
            Creature2.Temp.ActiveEntrustment = null;
        }
        ...
        {
            return Creature1.Inventory.GetItemAt(Pocket.EntrustmentItem1, 0, 0);
        }
        ...
        {
            return Creature1.Inventory.GetItemAt(Pocket.EntrustmentItem2, 0, 0);
        }
    }
