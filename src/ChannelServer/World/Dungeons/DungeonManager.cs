...
namespace Aura.Channel.World.Dungeons
{
    ...
    public class DungeonManager
    {
        ...
        ///     a nano second later.
        /// </summary>
        private readonly object _createAndCleanUpLock = new object();

        private readonly Dictionary<long, Dungeon> _dungeons;
        ...
        private Dungeon CreateDungeon(string dungeonName, int itemId, Creature creature)
        {
            ...
            else
            {
                ...
                var existing = Get(a => a.Name == dungeonName && a.ItemId == itemId);
        ...
        public int GetRegionId()
        {
            ...
            lock (_syncLock)
            {
                for (var i = MabiId.DungeonRegions; i < MabiId.DynamicRegions; ++i)
        ...
        public bool CheckDrop(Creature creature, Item item)
        {
            ...
            else
            {
                ...
                {
                    Log.Warning("DungeonManager.CheckDrop: No dungeon name found in altar event '{0:X16}'.",
            ...
            return CreateDungeonAndWarp(dungeonName, item.Info.Id, creature);
        }
        ...
        public bool CreateDungeonAndWarp(string dungeonName, int itemId, Creature leader)
        {
            lock (_createAndCleanUpLock)
            {
                try
                {
                    var dungeon = CreateDungeon(dungeonName, itemId, leader);
                    ...
                    if (dungeon.HasRoles)
                    {
                        ...
                        {
                            Send.Notice(leader,
                                Localization.Get("Your party has too few members for this role-playing dungeon."));
                            return false;
                        }
                        ...
                        for (int i = 0, j = 1; i < creators.Count; ++i)
                        {
                            ...
                            {
                                Log.Error("DungeonManager.CreateDungeonAndWarp: Failed to create RP character '{0}'.",
                                    role);
                                throw;
                            }
                        }
