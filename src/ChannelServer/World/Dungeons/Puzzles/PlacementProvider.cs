...
namespace Aura.Channel.World.Dungeons.Puzzles
{
    ...
    public class PlacementProvider
    {
        ...
        private readonly Queue<int[]> _positionQueue;
        private readonly Placement _positionType;
        private readonly int _radius;
        private readonly List<Position> _reservedPositions;
        ...
        public PlacementProvider(Placement positionType, int radius = 600)
        {
            ...
            foreach (var i in shuffle)
                _positionQueue.Enqueue(new[]
                    {(int) (offsets[i, 0] * radius), (int) (offsets[i, 1] * radius), (int) offsets[i, 2]});
        }
        ...
        public int[] GetPosition()
        {
            ...
            {
                var rnd = RandomProvider.Get();
                var pos = new[] {rnd.Next(-_radius, _radius), rnd.Next(-_radius, _radius), rnd.Next(360)};
            ...
            if (_positionType == Placement.Herb)
            {
                ...
                for (var i = 0; i < 10 && !emptySpotFound; ++i)
                {
