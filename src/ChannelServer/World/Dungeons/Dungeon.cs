...
namespace Aura.Channel.World.Dungeons
{
    ...
    public class Dungeon
    {
        ...
        private readonly HashSet<int> _clearedSections;

        private bool _manualCompletion;
        private bool _partyEnterEventFired;

        private readonly object _partyEnterSyncLock = new object();

        private readonly Dictionary<int, string> _roles = new Dictionary<int, string>();

        private readonly List<Prop> _treasureChests;
        private readonly PlacementProvider _treasurePlacementProvider;
        ...
        public Dungeon(long instanceId, string dungeonName, int itemId, int seed, int floorPlan, Creature creature)
        {
            ...
            Log.Debug("Dungeon: Creating '{0}', item id: {1}, seed: {2}, floorPlan: {3}", dungeonName, itemId, seed,
                floorPlan);

            // Get data
            Data = AuraData.DungeonDb.Find(dungeonName);
            if (Data == null)
            ...
            Seed = seed;
            FloorPlan = floorPlan;
            Options = XElement.Parse("<option />");

            Creators = new List<long>();
            RpCharacters = new List<long>();
            PartyLeader = creature;

            // Only creatures who actually ENTER the dungeon at creation are considered "dungeon founders".
            Creators.AddRange(creature.Party.GetCreaturesOnAltar(creature.RegionId).Select(a => a.EntityId));

            // Add the creator to the list if something went wrong on the altar check.
            if (Creators.Count == 0)
                Creators.Add(creature.EntityId);

            // Get script
            Script = ChannelServer.Instance.ScriptManager.DungeonScripts.Get(Name);
            if (Script == null)
                Log.Warning("Dungeon: No script found for '{0}'.", Name);

            // Generate floors
            Generator = new DungeonGenerator(Name, ItemId, Seed, FloorPlan, Options.ToString());

            // Prepare puzzles
            for (var iFloor = 0; iFloor < Generator.Floors.Count; ++iFloor)
                GeneratePuzzles(iFloor);
        ...
        /// </remarks>
        public Creature PartyLeader { get; }
        ...
        /// </summary>
        public int RemainingBosses => _bossesRemaining;
        ...
        private void GenerateRegions()
        {
            ...
            var lobbyRegion = new DungeonLobbyRegion(lobbyRegionId, Data.LobbyRegionId, this);
            //lobbyRegion.PlayerEnters += this.OnPlayerEntersLobby;
            Regions.Add(lobbyRegion);

            // Create floors
            for (var iFloor = 0; iFloor < Generator.Floors.Count; ++iFloor)
            {
                var dungeonFloor = Generator.Floors[iFloor];
                ...
                Regions.Add(floorRegion);
            }

            // Add everything to world
            foreach (var floor in Regions)
        ...
        {
            var floorData = Generator.Data.Floors[iFloor];
            var rng = Generator.RngPuzzles;
            var sections = Generator.Floors[iFloor].Sections;
            ...
            for (var section = 0; section < sections.Count; ++section)
            {
                ...
                for (var p = 0; p < puzzleCount; ++p)
                {
                    ...
                    try
                    {
                        puzzle = sections[section].NewPuzzle(this, floorData,
                            floorData.Sections[section].Puzzles[randomPuzzle], puzzleScript);
        ...
        {
            for (var iRegion = 0; iRegion < Regions.Count; ++iRegion)
                // Lobby
                if (iRegion == 0)
                    InitLobbyRegion(iRegion);
                // Floors
                else
                    InitFloorRegion(iRegion);
        }
        ...
        public void InitLobbyRegion(int iRegion)
        {
            ...
            var stairs = region.GetPropById(Data.StairsPropId);
            if (stairs == null)
                throw new Exception("Missing stairs prop '" + Data.StairsPropId + "'.");

            var statue = region.GetProp(a =>
                a.Parameters.Any(x => x.SignalType == SignalType.Touch && x.EventType == EventType.Confirmation));
            if (statue == null)
                throw new Exception("Missing statue prop '" + Data.LastStatuePropId + "'.");

            stairs.Behavior = (cr, pr) =>
            {
                // Indoor_RDungeon_SB marks the start position for the next floor.
                var clientEvent = Regions[1].GetClientEvent("Indoor_RDungeon_SB");
                if (clientEvent == null)
                {
                    Log.Error("Event 'Indoor_RDungeon_SB' not found while trying to warp to '{0}'.", Regions[1].Name);
                    return;
                }

                // Warp to the second region, the 1st floor.
                var regionId = Regions[1].Id;
        ...
        {
            CreatePuzzles(iRegion);

            var region = Regions[iRegion];
            var floor = Generator.Floors[iRegion - 1];
            var gen = floor.MazeGenerator;
            var floorData = Data.Floors[iRegion - 1];

            var iPrevRegion = iRegion - 1;
            var iNextRegion = iRegion + 1;

            var startTile = gen.StartPos;
            var startPos = new Generation.Position(startTile.X * TileSize + TileSize / 2,
                startTile.Y * TileSize + TileSize / 2);
            ...
            var endPos =
                new Generation.Position(endTile.X * TileSize + TileSize / 2, endTile.Y * TileSize + TileSize / 2);
            ...
            var stairsBlock = Data.Style.Get(DungeonBlockType.StairsUp, startRoomIncomingDirection);
            var stairs = new Prop(stairsBlock.PropId, region.Id, startPos.X, startPos.Y,
                MabiMath.DegreeToRadian(stairsBlock.Rotation), 1, 0, "single");
            ...
            var portalBlock = Data.Style.Get(DungeonBlockType.PortalUp, startRoomIncomingDirection);
            var portal = new Prop(portalBlock.PropId, region.Id, startPos.X, startPos.Y,
                MabiMath.DegreeToRadian(portalBlock.Rotation), 1, 0, "single", "_upstairs",
                Localization.Get("<mini>TO</mini> Upstairs"));
            ...
            portal.Behavior = (cr, pr) =>
            {
                ...
                var clientEvent = Regions[iPrevRegion].GetClientEvent("Indoor_RDungeon_EB");
                if (clientEvent == null)
                {
                    Log.Error("Event 'Indoor_RDungeon_EB' not found while trying to warp to '{0}'.",
                        Regions[iPrevRegion].Name);
                    return;
                }

                // Warp to prev region
                var regionId = Regions[iPrevRegion].Id;
                var x = (int) clientEvent.Data.X;
                var y = (int) clientEvent.Data.Y;
            ...
            if (floorData.Statue)
            {
                var saveStatue = new Prop(Data.SaveStatuePropId, region.Id, startPos.X, startPos.Y,
                    MabiMath.DegreeToRadian(stairsBlock.Rotation + 180), 1, 0, "single");
                ...
                saveStatue.Behavior = (cr, pr) =>
                {
                    ...
                    // Scroll message
                    var msg = string.Format("You're currently on Floor {0} of {1}. ", iRegion, Data.EngName);
                    Send.Notice(cr, NoticeType.Top, ScrollMessageDuration, msg + GetPlayerListScrollMessage());
                };
            ...
            if (floor.IsLastFloor)
            {
                ...
                _bossExitDoor = new Prop(Data.BossExitDoorId, region.Id, endPos.X, endPos.Y + TileSize / 2,
                    Rotation(Direction.Up), 1, 0, "closed");
                ...
                {
                    Log.Warning("Dungeon.InitFloorRegion: No locked place in last section of '{0}'.", Name);

                    _bossDoor = new Door(Data.BossDoorId, region.Id, endPos.X, endPos.Y - TileSize, Direction.Up,
                        DungeonBlockType.BossDoor, "", "closed");
                    ...
                    _bossDoor.Behavior += BossDoorBehavior;
                    _bossDoor.UpdateShapes();
                    endRoomTrait.SetPuzzleDoor(_bossDoor,
                        Direction.Down); // making sure another open dummy door won't be added here
                ...
                else
                {
                    ...
                    {
                        Log.Warning("Dungeon.InitFloorRegion: Boss door was left open, closing. Dungeon: '{0}'.", Name);
                        _bossDoor.Close(endRoomTrait.X, endRoomTrait.Y);
                    }
                }

                // Create exit statue
                var exitStatue = new Prop(Data.LastStatuePropId, region.Id, endPos.X, endPos.Y + TileSize * 2,
                    Rotation(Direction.Up), 1, 0, "single");
                exitStatue.Info.Color1 = floorData.Color1;
                exitStatue.Info.Color2 = floorData.Color1;
                exitStatue.Info.Color3 = floorData.Color3;
                exitStatue.Extensions.AddSilent(new ConfirmationPropExtension("GotoLobby",
                    "_LT[code.standard.msg.dungeon_exit_notice_msg]",
                    "_LT[code.standard.msg.dungeon_exit_notice_title]", "haskey(chest)"));
            ...
            else
            {
                ...
                var stairsDownBlock = Data.Style.Get(DungeonBlockType.StairsDown, endRoomDirection);
                var stairsDown = new Prop(stairsDownBlock.PropId, region.Id, endPos.X, endPos.Y,
                    MabiMath.DegreeToRadian(stairsDownBlock.Rotation), 1, 0, "single");
                ...
                var portalDownBlock = Data.Style.Get(DungeonBlockType.PortalDown, endRoomDirection);
                var portalDown = new Prop(portalDownBlock.PropId, region.Id, endPos.X, endPos.Y,
                    MabiMath.DegreeToRadian(portalDownBlock.Rotation), 1, 0, "single", "_downstairs",
                    Localization.Get("<mini>TO</mini> Downstairs"));
                ...
                portalDown.Behavior = (cr, pr) =>
                {
                    ...
                    var clientEvent = Regions[iNextRegion].GetClientEvent("Indoor_RDungeon_SB");
                    if (clientEvent == null)
                    {
                        Log.Error("Event 'Indoor_RDungeon_SB' not found while trying to warp to '{0}'.",
                            Regions[iNextRegion].Name);
                        return;
                    }

                    // Warp to next floor
                    var regionId = Regions[iNextRegion].Id;
                    var x = (int) clientEvent.Data.X;
                    var y = (int) clientEvent.Data.Y;
            ...
            for (var y = 0; y < floor.MazeGenerator.Height; ++y)
            {
                ...
                for (var dir = 0; dir < 4; ++dir)
                {
                    ...
                    {
                        var doorX = x * TileSize + TileSize / 2;
                        var doorY = y * TileSize + TileSize / 2;

                        var doorBlock = Data.Style.Get(DungeonBlockType.Door, dir);
                        var doorProp = new Prop(doorBlock.PropId, region.Id, doorX, doorY,
                            MabiMath.DegreeToRadian(doorBlock.Rotation), state: "open");
            ...
            region.PlayerEntered += (creature, prevRegionId) => Script.OnPlayerEnteredFloor(this, creature, iRegion);
        }
        ...
        private void CreatePuzzles(int iRegion)
        {
            ...
            var sections = Generator.Floors[iRegion - 1].Sections;
        ...
        public void AddBoss(int raceId, int amount = 1)
        {
            ...
            var end = Generator.Floors.Last().MazeGenerator.EndPos;
            var endX = end.X * TileSize + TileSize / 2;
            var endY = end.Y * TileSize + TileSize / 2;
            var regionId = Regions.Last().Id;
        ...
        private void OnBossDeath(Creature creature, Creature killer)
        {
            ...
            // Call OnBossDeath
            if (Script != null)
                Script.OnBossDeath(this, creature, killer);
        ...
        {
            Regions.ForEach(a => a.RemoveAllMonsters());
        }
        ...
        private void SpawnTreasureChests()
        {
            ...
            for (var i = 0; i < _treasureChests.Count; ++i)
            {
                var pos = new Generation.Position(Generator.Floors.Last().MazeGenerator.EndPos);

                pos.X *= TileSize;
                pos.Y *= TileSize;

                pos.X += TileSize / 2;
                pos.Y += (int) (TileSize * 2.5f);
        ...
        {
            return Regions.Sum(a => a.CountPlayers());
        }
        ...
        {
            PlayCutscene(cutsceneName, null);
        }
        ...
        {
            var viewers = GetRpCharactersOrCreators().ToArray();
            if (viewers.Length == 0)
                return;

            var leader = PartyLeader;
            if (!IsInside(leader.EntityId))
        ...
        public void OnPlayerEntersLobby(Creature creature)
        {
            ...
            var isCreator = Creators.Contains(actualCreature.EntityId);
            ...
            Send.Notice(creature, NoticeType.Top, ScrollMessageDuration, msg + GetPlayerListScrollMessage());

            // Enter events
            Script.OnPlayerEntered(this, creature);
            lock (_partyEnterSyncLock)
            {
                if (!_partyEnterEventFired && CountPlayers() == Creators.Count)
                {
                    _partyEnterEventFired = true;
                    Script.OnPartyEntered(this, creature);
                }
            }
        }
        ...
        public string GetPlayerListScrollMessage()
        {
            ...
            {
                var floorString =
                    i == 0 ? Localization.Get("Entrance") : string.Format(Localization.Get("Floor {0}"), i);

                foreach (var player in Regions[i].GetAllPlayers())
                {
        ...
        public void CheckSectionClear()
        {
            ...
            for (var i = 0; i < Regions.Count; ++i)
            {
                var floorRegion = Regions[i] as DungeonFloorRegion;
                if (floorRegion == null)
                    continue;

                for (var j = 1; j <= floorRegion.Floor.Sections.Count; ++j)
                {
                    var id = i * 1000 + j;
                    ...
                    if (floorRegion.Floor.Sections[j - 1].HasBeenCleared)
                    {
                        ...
                        Script.OnSectionCleared(this, i, j);
                    }
                }
            }
        }
        ...
        public List<Creature> GetCreators()
        {
            ...
            var creatures = HasRoles ? RpCharacters : Creators;
        ...
        public List<Creature> GetRpCharacters()
        {
            ...
            foreach (var entityId in RpCharacters)
            {
                var creature = GetCreature(entityId);
                if (creature != null)
                    result.Add(creature);
        ...
        {
            return Regions.Any(a => a.CreatureExists(entityId));
        }
        ...
        {
            var players = Regions.SelectMany(a => a.GetCreatures(b => b.IsPlayer));
            foreach (var player in players)
                player.Warp(Data.Exit);
        }
        ...
        {
            var region = Regions.Last();
            var endPos = Generator.Floors.Last().MazeGenerator.EndPos;
        ...
        {
            var location = GetBossRoomCenter();
