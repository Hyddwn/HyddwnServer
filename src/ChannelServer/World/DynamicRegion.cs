...
namespace Aura.Channel.World
{
    public class DynamicRegion : Region
    {
            ...
            : base(baseRegionId)
        {
            ...
            var baseRegionInfoData = AuraData.RegionInfoDb.Find(BaseId);
            if (baseRegionInfoData == null)
                throw new Exception("DynamicRegion: No region info data found for '" + BaseId + "'.");

            var baseRegionData = AuraData.RegionDb.Find(BaseId);
            if (baseRegionData == null)
                throw new Exception("DynamicRegion: No region data found for '" + BaseId + "'.");

            var baseRegion = ChannelServer.Instance.World.GetRegion(baseRegionId);
            if (baseRegion != null)
                Properties.Load(baseRegion.Properties.GetList());

            BaseName = baseRegionInfoData.Name;
            Id = ChannelServer.Instance.World.DynamicRegions.GetFreeDynamicRegionId();
            Name = "DynamicRegion" + Id;
            Variation = variationFile;
            Mode = mode;
            IsIndoor = baseRegionData.Indoor;

            Data = CreateVariation(baseRegionInfoData, Id, variationFile);
        ...
        /// <param name="variationFile"></param>
        /// <returns></returns>
        private static RegionInfoData CreateVariation(RegionInfoData baseRegionInfoData, int newRegionId,
            string variationFile)
        {
        ...
        public override void RemoveCreature(Creature creature)
        {
            ...
            // region again.
            Send.RemoveDynamicRegion(creature, Id);

            // Remove empty region from world when last *player* was removed
            if (creature.IsPlayer && CountPlayers() == 0 && Mode == RegionMode.RemoveWhenEmpty)
            {
                ChannelServer.Instance.World.RemoveRegion(Id);
                ChannelServer.Instance.World.DynamicRegions.Remove(Id);
            }
        }
    }
}