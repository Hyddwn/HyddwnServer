...
namespace Aura.Channel.World.Guilds
{
    ...
    {
        private readonly Dictionary<long, Prop> _stones;
        ...
        protected override AuraDb Database => ChannelServer.Instance.Database;
        ...
        public override void Initialize()
        {
            ...
            ChannelServer.Instance.Events.CreatureConnected += OnCreatureConnected;
        }
        ...
        protected override void OnSyncGuildAdded(Guild guild)
        {
            ...
            foreach (var member in members)
                OnSyncGuildMemberAdded(guild, member);
        }
        ...
        protected override void OnSyncGuildRemoved(Guild guild)
        {
            var members = guild.GetMembers();
            foreach (var member in members)
                OnSyncGuildMemberRemoved(guild, member);
        ...
        {
            var guild = FindGuildWithMember(character.EntityId);
            if (guild == null)
                return;
        ...
        public void ConvertPlayPoints(Creature creature, Guild guild)
        {
            ...
            Send.GuildMessage(creature, guild,
                Localization.GetPlural("Added {0:n0} Point.", "Added {0:n0} Points.", points), points);
        }
        ...
        {
            AddGold(guild, amount);
            creature.Inventory.RemoveGold(amount);

            Send.GuildMessage(creature, guild,
                Localization.GetPlural("You have donated {0:n0} Gold.", "You have donated {0:n0} Gold.", amount),
                amount);
        }
        ...
        public void Apply(Creature creature, Guild guild, string application)
        {
            if (guild.HasMember(creature.EntityId))
                throw new ArgumentException("Character is already a member of this guild. (" + creature.Name + ", " +
                                            guild.Name + ")");
        ...
        public void ChangeStone(Guild guild, GuildStoneType stoneType)
        {
            ...
            {
                default:
                case GuildStoneType.Normal:
                    guild.Stone.PropId = GuildStonePropId.Normal;
                    break;
                case GuildStoneType.Hope:
                    guild.Stone.PropId = GuildStonePropId.Hope;
                    break;
                case GuildStoneType.Courage:
                    guild.Stone.PropId = GuildStonePropId.Courage;
                    break;
            }
        ...
        public void CreateGuild(Party party, string name, GuildType type, GuildVisibility visibility)
        {
            ...
            lock (_syncLock)
            {
                ...
                guild.RejectionMessage =
                    string.Format(Localization.Get("You have been denied admission to the {0} guild."), guild.Name);
