...
namespace Aura.Channel.World
{
    ...
    {
        private readonly Dictionary<long, List<LinePath>> _reference;
        private readonly Quadtree<LinePath> _tree;
        ...
        public bool Any(Position from, Position to)
        {
            ...
            return Find(from, to, out intersection);
        }
        ...
        public bool AnyInRange(Position pos, int range)
        {
            Position intersection;
            return
                Find(new Position(pos.X - range, pos.Y), new Position(pos.X + range, pos.Y), out intersection) ||
                Find(new Position(pos.X, pos.Y - range), new Position(pos.X, pos.Y + range), out intersection) ||
                Find(new Position(pos.X - range, pos.Y - range), new Position(pos.X + range, pos.Y + range),
                    out intersection) ||
                Find(new Position(pos.X - range, pos.Y + range), new Position(pos.X + range, pos.Y - range),
                    out intersection);
        }
        ...
        public bool Find(Position from, Position to, out Position intersection)
        {
            ...
            foreach (var line in lines)
            {
                ...
                Position inter;
                if (FindIntersection(x1, y1, x2, y2, x3, y3, x4, y4, out inter))
        ...
        /// </summary>
        private bool FindIntersection(double x1, double y1, double x2, double y2, double x3, double y3, double x4,
            double y4, out Position intersection)
        {
